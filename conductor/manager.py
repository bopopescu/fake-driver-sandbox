...
class ConductorManager(manager.Manager):
    ...
    def compute_node_update(self, context, node, values):
        ...
        LOG.info(_('Compute node id=%(id)s updated'), dict(id=node['id']))
        return jsonutils.to_primitive(result)

    def compute_node_update_pf9_stats(self, context, node, values):
        LOG.info(_('PF9 stats for host=%(id)s updated'), dict(id=node['host']))
        result = self.db.compute_node_update_pf9_stats(context, node['host'],
                                                       values)
        return jsonutils.to_primitive(result)
    ...
    def service_update(self, context, service, values):
        ...
        if service.get('binary') == 'nova-compute':
            LOG.info(_('nova-compute service id=%(id)s host=%(host)s updated'),
                 dict(id=service['id'], host=service['host']))
        return jsonutils.to_primitive(svc)
    ...
    def instance_register(self, context, host, updates):
        self.compute_api.instance_register(context, host, updates)

    def instance_not_found(self, context, host, uuids):
        for uuid in uuids:
            try:
                instance = objects.Instance.get_by_uuid(context, uuid)
                instance.vm_state = vm_states.ERROR
                instance.save()
                fault = exception.InstanceNotFoundOnHypervisorPF9(instance_id=uuid)
                compute_utils.add_instance_fault_from_exc(context, instance, fault)

            except exception.InstanceNotFound:
                LOG.info(_('Instance %(uuid)s not found in DB either'), dict(uuid=uuid))

    def log_warning_pf9(self, context, msg):
...
class _ConductorManagerV3Proxy(object):
    ...
    # PF9 functions
    def compute_node_update_pf9_stats(self, context, node, values):
        return self.manager.compute_node_update_pf9_stats(context, node, values)

    def instance_register(self, context, host, updates):
        return self.manager.instance_register(context, host, updates)

    def instance_not_found(self, context, host, uuids):
        return self.manager.instance_not_found(context, host, uuids)

    def log_warning_pf9(self, context, msg):
        return self.manager.log_warning_pf9(context, msg)
    # PF9 end
