...
class ConductorAPI(object):
    ...
    * 3.0  - Drop backwards compatibility
    ...
    def instance_register(self, context, host, updates):
        ...
        kwargs = {'host': host,
                  'updates': updates}
        #kwargs = {'host': host, 'updates': []}
        cctxt.cast(context, 'instance_register', **kwargs)
    ...
    def instance_not_found(self, context, host, instances):
        instances_p = jsonutils.to_primitive(instances)
        cctxt = self.client.prepare()
        kwargs = dict(host=host, uuids=instances_p)
        cctxt.cast(context, 'instance_not_found', **kwargs)

    def log_warning_pf9(self, context, message):
        cctxt = self.client.prepare()
        kwargs = dict(msg=message)
        cctxt.cast(context, 'log_warning_pf9', **kwargs)

    def compute_node_update_pf9_stats(self, context, node, values):
        node_p = jsonutils.to_primitive(node)
        cctxt = self.client.prepare()
        return cctxt.call(context, 'compute_node_update_pf9_stats',
