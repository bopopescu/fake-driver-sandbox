...
class LocalAPI(object):
    ...
    def instance_register(self, context, host, updates):
        self._manager.instance_register(context, host, updates)

    def instance_destroy(self, context, instance):
        return self._manager.instance_destroy(context, instance)

    def instance_not_found(self, context, host, uuids):
        self._manager.instance_not_found(context, host, uuids)

    def log_warning_pf9(self, context, msg):
        LOG.warning(_LW('%s'), msg)

    def compute_node_update_pf9_stats(self, context, node, values):
        LOG.info(_LI('compute_node_update_pf9_stats: begin'))
        retval = self._manager.compute_node_update_pf9_stats(context, node,
                                                           values)
        LOG.info(_LI('compute_node_update_pf9_stats: end'))
        return retval
    # PF9 end
...
class API(LocalAPI):
    ...
    def log_warning_pf9(self, context, msg):
        self._manager.log_warning_pf9(context, msg)
    # PF9 end
