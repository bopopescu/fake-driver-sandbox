...
pf9_opts = [
    cfg.BoolOpt(name='snapshot_to_template', default=False,
                help='A boolean option to determine whether to snapshot the VMs'
                     ' to a template or use the default OpenStack '
                     'snapshot mechanism')
]
...
class VMwareVMOps(object):
    ...
    def _extend_virtual_disk(self, instance, requested_size, name, dc_ref):
        ...
        vmdk_extend_task = self._session._call_method(
                ...
                newCapacityKb=long(requested_size),
                eagerZero=False)
        try:
    ...
    def _extend_if_required(self, dc_info, image_info, instance,
        ...
        # PF9: The disk size from flavor instead of the instance object.
        if hasattr(instance, 'flavor') and hasattr(instance.flavor, 'root_gb'):
            root_gb = instance.flavor.root_gb
        else:
            root_gb = instance.root_gb
        if root_gb * units.Gi > image_info.file_size:
            size_in_kb = root_gb * units.Mi
    ...
    def build_virtual_machine(self, instance, image_info,
        ...
        config_spec = vm_util.get_vm_create_spec(client_factory,
                                                 ...
                                                 metadata=metadata)
        # Start Pf9 addition
        if hasattr(image_info, 'is_ova') and image_info.is_ova:
            # Create a template from ova and use if for any future provisioning
            template_ref = None
            try:
                template_ref = vm_util.get_template_ref(self, image_info.name)
            except exception.InstanceNotFound:
                # Template does not exist. So create a template
                ova_folder_name = 'pf9_ova'
                try:
                    if not self.check_if_folder_exists_pf9(ova_folder_name, dc_info.vmFolder):
                        LOG.info("Creating folder: %s" % ova_folder_name)
                        self._session.invoke_api(self._session.vim,
                                                 'CreateFolder',
                                                 dc_info.vmFolder,
                                                 name=ova_folder_name)
                except:
                    LOG.warn("Creating pf9_ova folder failed. " + \
                                "Might have been created by another ostackhost process")
                cluster_name = vutil.get_object_properties(\
                                self._session.vim, self._cluster, ['name'])[0].propSet[0].val
                # Image location for glance ova is in the form:
                # vsphere://vc/folder/pf9/openstack/image_uuid?dcPath=dc&dsName=datastore_name
                image_locations = IMAGE_API().get(context, image_info.image_id,
                                                  True)['locations'][0]['url']
                # Datastore locator for ovftool is of the form:
                # https://vc:443/folder/pf9/openstack/image_uuid?dcPath=dc&dsName=datastore_name 
                source = self.get_ovf_locator('https') + ":443/folder" + \
                                        image_locations.split("folder")[1]
                target = self.get_ovf_locator() + '/{datacenter}/host/{cluster}'.\
                                       format(datacenter=dc_info.name, cluster=cluster_name)
                opts = '--noSSLVerify --sourceType=ova'
                name_folder_ds = '--name="{name}" -vf="{folder}" -ds="{ds_name}"'.\
                                        format(name=image_info.name,
                                               folder=ova_folder_name,
                                               ds_name=datastore.name)
                cmd = 'ovftool {opts} {name_folder_ds} "{source}" "{target}"' \
                        .format(opts=opts, name_folder_ds=name_folder_ds,
                                source=source, target=target)
                try:
                    child = pexpect.spawn(cmd)
                    child.expect('Username:')
                    child.sendline(CONF.vmware.host_username)
                    child.expect('Password:')
                    child.sendline(CONF.vmware.host_password)
                    output = child.read()
                    res = child.wait()
                    LOG.info("Deploy VM from OVA succeeded: " + str(output))
                except:
                    LOG.error("Deploying base template VM for OVA failed")
                    raise
                if res != 0:
                    LOG.error("Deploying ova using ovftool failed")
                    raise
                template_ref = vm_util.get_template_ref(self, image_info.name)

                # Mark as Template
                self._session._call_method(self._session.vim,
                                           "MarkAsTemplate", template_ref)
            vm_ref = vm_util.clone_vm_pf9(self._session, instance,
                                          client_factory,
                                          datastore, dc_info,
                                          self._root_resource_pool,
                                          template_ref, config_spec)

        elif image_info.is_vsphere_template == "1":
            # Clone the VM
            if hasattr(image_info, 'vsphere_template_moref') and \
                    image_info.vsphere_template_moref is not None:
                template_ref = vutil.get_moref(image_info.vsphere_template_moref,
                                               "VirtualMachine")
            else:
                template_ref = vm_util.get_template_ref(
                        self, image_info.vsphere_template_name)
            vm_ref = vm_util.clone_vm_pf9(self._session, instance,
                                          client_factory,
                                          datastore, dc_info,
                                          self._root_resource_pool,
    ...
    def check_if_folder_exists_pf9(self, folder_name, dc_vmfolder):
        # PF9 method
        # Get list of child entities for the parent folder
        prop_val = self._session._call_method(vutil, 'get_object_property',
                                              dc_vmfolder,'childEntity')
        if prop_val and hasattr(prop_val, 'ManagedObjectReference'):
            ...
            for child_entity in child_entities:
                if child_entity._type != 'Folder':
                    continue
                child_entity_name = self._session._call_method(vutil, 'get_object_property',
                                                               child_entity, 'name')
                if (child_entity_name
                    and (urllib.unquote_plus(child_entity_name) == folder_name)):
                    LOG.debug("Child folder: %s exists.", folder_name)
                    return True
        return False

    def get_ovf_locator(self, protocol="vi"):
        # PF9 method
        if protocol == "vi":
            return "{protocol}://{user}:{password}@{vcva}".format(
                        protocol=protocol,
                        user=urllib.quote_plus(CONF.vmware.host_username),
                        password=urllib.quote_plus(CONF.vmware.host_password),
                        vcva=CONF.vmware.host_ip)
        else:
            return "{protocol}://{vcva}".format(protocol=protocol,
                                                vcva=CONF.vmware.host_ip)
    ...
    def _get_vm_config_info(self, instance, image_info,
        ...
        if hasattr(instance, 'flavor') and hasattr(instance.flavor, 'root_gb'):
            ...
            root_gb = instance.root_gb
        if (root_gb != 0 and
                image_info.file_size > root_gb * units.Gi):
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        # instance uuid. Use PF9 function to add instance ref to cache since
        # cache structure has been modified by PF9
        vm_util.add_vm_to_cache(self, instance['uuid'], {'name': instance['uuid'], 'vmref': vm_ref, "template" : "false"}, True)
        ...
        # Start Pf9 addition
        if (hasattr(image_info, 'is_vsphere_template') and \
                image_info.is_vsphere_template == "1") or \
                (hasattr(image_info, 'is_ova') and image_info.is_ova) :
            # If it's a template, extend the disk if it's size is smaller than specified, and return
            dest_vmdk_name = "%s/%s.vmdk" % (instance['uuid'], instance['uuid'])
            dest_vmdk_path = "[%s] %s" % (vi.datastore.name, dest_vmdk_name)
            self._extend_if_required(vi.dc_info, image_info, instance, dest_vmdk_path)
            if configdrive.required_by(instance):
                self._configure_config_drive(
                        instance, vm_ref, vi.dc_info, vi.datastore,
                        injected_files, admin_password, network_info=network_info)
    ...
    # PF9 functions start
    def _get_actual_disk_size(self, devices):
        total_size = 0
        if hasattr(devices, 'VirtualDevice'):
            devices = devices.VirtualDevice
        for device in devices:
            if device.__class__.__name__ == 'VirtualDisk' and \
                    hasattr(device, 'capacityInKB'):
                total_size += int(device.capacityInKB) * 1024
        # If the size is reported as 0 glance driver will try to query the
        # backend to get the disk size which will fail. Prevent that by
        # returning disk size as at least 1
        if not total_size:
            total_size = 1
        return total_size

    def _create_template_from_vm(self, context, instance, vm_ref, datastore_name, os_type, image_id):
        client_factory = self._session.vim.client.factory
        # Following dict will be used for naming and setting other information
        # of the template.
        template_name = images.get_image_name_from_id_pf9(context, image_id)
        template_dict = {'vcpus': instance['vcpus'],
                         'memory_mb': instance['memory_mb'],
                         'uuid': image_id,
                         'hostname': template_name,
                         # ID needs to be set and passed in as 0 so that the
                         # check of ID fails and no ID appended to the
                         # template name
                         'id': 0}
        # From the liberty release get_vm_create_spec does not accept dict and
        # requires an objects.instance.Instance object. Create a temporary
        # object for that purpose. Also the object is deleted right after it
        # has been used since any operations e.g (save) can update database.
        template_obj = objects.instance.Instance(**template_dict)
        extra_specs = vm_util.ExtraSpecs()
        ...
        # Get datastore to store the template
        datastore = ds_util.get_datastore(self._session, self._cluster,
                                          self._datastore_regex)
        dc_info = self.get_datacenter_ref_and_name(datastore.ref)
        folder_ref = dc_info.vmFolder
        respool = self._root_resource_pool

        template_ref = vm_util.clone_to_template_pf9(
                                      self._session, client_factory, vm_ref,
                                      datastore.ref, folder_ref, respool,
                                      config_spec, image_id)
        template_info = self._session._call_method(vutil,
                                                   ...
                                                   ["summary.config.instanceUuid",
                                                    ...
                                                    "config.hardware.device"])
        del template_obj
        template_uuid = template_info['summary.config.instanceUuid']
        template_path = template_info['summary.config.vmPathName']
        disk_size = self._get_actual_disk_size(template_info['config.hardware.device'])

        images.update_snapshot_template_metadata_pf9(
            context, image_id, template_uuid=template_uuid,
            template_ref=template_ref.value, disk_size=disk_size,
...
# PF9 functions end
    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        def _get_vm_and_vmdk_attribs():
            ...
            # PF9: send instance_uuid as None so that first disk is used as
            # the root disk for snapshot information. If snapshotting to
            # template this will have no effect but if snapshotting to VMDK
            # vCenter driver anyway copies only the first disk as snapshot
            ...
            # PF9: We need the datastore name as well to create the config spec
            # for template VMs. Get the VMDK file path of VM and parse
            # datastore from it.
            ...
            datastore_name = ds_obj.DatastorePath.parse(
                    props['summary.config.vmPathName']).datastore
            return (vmdk, datastores, os_type, datastore_name)

        vmdk, datastores, os_type, datastore_name = _get_vm_and_vmdk_attribs()
        ds_ref = datastores.ManagedObjectReference[0]

        if CONF.PF9.snapshot_to_template:
            LOG.info(_('Snapshotting {vm} to template as per config '
                       'options'.format(vm=instance.uuid)))
            self._create_template_from_vm(context, instance, vm_ref, 
                                          datastore_name, os_type, image_id)
            # Manager function _snapshot_instance which invokes this function
            # expects the task state at end of this function to be image uploading.
        ...
        try:
            ...
            images.upload_image_stream_optimized(
                ...
                vmdk_size=vmdk.capacity_in_bytes, os_type=os_type,
                pf9_vpshere_template=0)
        finally:
            if snapshot_vm_ref:
    ...
    def power_off(self, instance):
        ...
        """
        # PF9 change - Don't power off instance if it's already powered off
        # (vm power states does not include CRASHED here)
        vm_info = self.get_info(instance)
        if vm_info.state != power_state.SHUTDOWN:
            vm_util.power_off_instance(self._session, instance)

    def power_on(self, instance):
        # PF9 change - Don't power on instance if it's already powered on
        vm_info = self.get_info(instance)
