...
# PF9 change, explicitly disable urllib3 warnings
...
class VMwareVCDriver(driver.ComputeDriver):
    ...
    def get_host_stats_pf9(self, res_types, refresh=False, nodename=None):
        return vm_utils_pf9.get_host_stats_vc(self, res_types, nodename)

    def get_instance_info(self, instance_uuid):
        return vm_util.get_instance_info(self, instance_uuid)
    ...
    def _register_openstack_extension(self):
        ...
        # PF9 change: ignore failures to register an extension and also use
        # Platform9 specific extension key
        try:
            LOG.debug('Registering extension %s with vCenter',
                      constants.PF9_EXTENSION_KEY)
            ...
            if os_extension is None:
                ...
                self._session._call_method(vim_util, 'register_extension',
                                           ...
                                           constants.EXTENSION_TYPE_INSTANCE,
                                           'Platform9 OpenStack',
                                           'Platform9 OpenStack Compute Service')
            vm_util.EXTENSION_REGISTERED = True
        except:
            vm_util.EXTENSION_REGISTERED = False
            LOG.debug('Failed to register the extension. Ignoring.')
    ...
    def get_all_networks_pf9(self, node=None):
        return vm_utils_pf9.get_all_cluster_networks(self, node)

    def get_all_ip_mapping_pf9(self, needed_uuids=None):
        return vm_util.get_network_information_from_vcenter(self, needed_uuids)

    def list_instances(self):
        """List VM instances """
        return vm_util.list_vm_names_on_vcenter(self)

    def list_instance_uuids(self, node=None, template_uuids=None, force=False):
        return [x for x in
                vm_util.list_instance_uuids_on_vcenter(
                    self, node, template_uuids=template_uuids, force=force)
                if uuidutils.is_uuid_like(x)]
# >>>>>>> atherton
    ...
    # PF9: add cluster_name to node name
    def _create_nodename(self, mo_id, cluster_name):
        ...
        PF9: The node name needs to be -
        <mo id>(cluster name) e.g.
          domain-c25(dogfood-cluster)
        to keep it consistent with existing
        deployments and simplify upgrades
        """

        return '%s(%s)' % (mo_id, cluster_name)
    ...
    def _get_available_resources(self, host_stats):
        return {'vcpus': host_stats['vcpus'],
               ...
               'hypervisor_hostname': host_stats['hypervisor_hostname'],
                ...
                # model of the "host"
                # PF9: We still need the hypervisor CPU information. Report it
                # as it was being reported in Juno.
                # 'cpu_info': None,
               'cpu_info': jsonutils.dumps(host_stats['cpu_info'], sort_keys=True),
    ...
    def destroy(self, context, instance, network_info, block_device_info=None,
        ...
        # PF9 change: remove the instance UUID from cache to keep it clean
        vm_util.remove_uuid_from_cache(instance['uuid'])
