...
class VMwareImage(object):
    ...
    def from_image(cls, image_id, image_meta):
        ...
        props_map = {
            ...
            # Start Pf9 addition
            'pf9_vsphere_template': 'is_vsphere_template',
            'pf9_vsphere_template_name': 'vsphere_template_name',
            # End Pf9 addition
        ...
        if 'pf9_vsphere_template_moref' in properties:
            props_map['pf9_vsphere_template_moref'] = 'vsphere_template_moref'
        # End Pf9 addition

        for k, v in six.iteritems(props_map):
            # PF9 change: check if k is present in properties to support new
            # image properties added by PF9
            if k in properties and properties.obj_attr_is_set(k):
...
# PF9 functions related to snapshot to template functionality
def update_snapshot_template_metadata_pf9(context, image, **kwargs):
    ...
    image_metadata = {"disk_format": "vmdk",
                      ...
                      "size": kwargs.get('disk_size', 0),
                      "properties": {
                          "pf9_vsphere_template": "1",
                          "pf9_vsphere_template_uuid": kwargs.get('template_uuid'),
                          "pf9_vsphere_template_name": kwargs.get('template_name',
                                                                  image),
                          "pf9_vsphere_template_moref": kwargs.get('template_ref'),
                          "pf9_virtual_size": kwargs.get('disk_size', 0),
                          "pf9_path": kwargs.get('template_path')},
                      # Add location data so that we can delete the images later.
                      # If template_loc is not present create a URL like
                      # vsphere://<<template_uuid>>. Ability to understand this URL
                      # is added in vmware_datastore.py in glance_store
                      "location": "vsphere://template-%s?dsName=DSNAME&dcPath=DCNAME" \
                              % kwargs.get('template_loc',
                                kwargs.get('template_uuid'))}
    IMAGE_API.update(context, metadata['id'], image_metadata)


def get_image_name_from_id_pf9(context, image):
    metadata = IMAGE_API.get(context, image)
