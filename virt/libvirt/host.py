...
class Host(object):
    ...
    def get_domain(self, instance):
        ...
        """
        # PF9 change to support searching VM refs based on name or display
        # name. Needed for discovered VMs. The changes have been implemented as
        # per older libvirt/driver.py:_lookup_by_instance_function
        instance_names = []
        for key in ['name', 'display_name']:
            try:
                instance_names.append(instance[key])
            except (KeyError, TypeError, exception.ObjectActionError):
                continue
        saved_exception = exception.InstanceNotFound(instance_id='unknown')
        for instance_name in instance_names:
            try:
                instance_ref = self._get_domain_by_name(instance_name)
                return instance_ref
            except exception.InstanceNotFound as ex:
                saved_exception = ex
        # Lookup by UUID
        try:
            instance_ref = self._get_domain_by_uuid(instance['uuid'])
            return instance_ref
        except exception.InstanceNotFound as ex:
            saved_exception = ex
        # Catching AttributeError here to fix unit tests failing due to lookupByUUID being absent
        # This is temporary fix for https://platform9.atlassian.net/browse/IAAS-5488
    ...
    def _get_domain_by_uuid(self, instance_uuid):
        ...
        try:
            conn = self.get_connection()
            return conn.lookupByUUIDString(instance_uuid)
        except libvirt.libvirtError as ex:
    ...
    def list_instance_domains(self, only_running=True, only_guests=True):
        ...
        doms = []
        for dom in alldoms:
            # PF9 change: skip if this is a guestfs appliance domain
            # We identify this based on the instance name prefix being 'guestfs'
            if hasattr(dom, 'XMLDesc'):
                xml = ElementTree.fromstring(dom.XMLDesc(0))
                tags = {child.tag: child for child in list(xml)}
                if ElementTree.tostring(tags['name']).startswith('<name>guestfs-'):
                    LOG.warn(_LW("Skipping guestfs appliance domain %s"), ElementTree.tostring(tags['name']))
                    continue
            # PF9 end
            if only_guests and dom.ID() == 0:
                continue
            doms.append(dom)
        LOG.debug("Reporting %d vms when only_running=%s and only_guests=%s" \
                  % (len(doms), only_running, only_guests))
        return doms
    ...
    @staticmethod
    def _parse_cpu_info_pf9(output):
        """Parse lscpu output and return sockets, cores and threads."""
        required_fields = {'Socket(s)': 'sockets',
        ...
        for line in lines:
            items = line.split(":")
            if items[0] in required_fields.keys():
                res[required_fields[items[0]]] = int(items[1].strip())
        return res
    ...
    def get_capabilities(self):
        ...
        if not self._caps:
            ...
            try:
                ...
                if set(['cores','sockets','threads']) == set(lscpu_cpu_info.keys()):
