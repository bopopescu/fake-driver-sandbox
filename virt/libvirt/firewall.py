...
class NWFilterFirewall(base_firewall.FirewallDriver):
    ...
    # PF9 begin
    def _rule_references_ip_var(self, rule):
        for child in rule:
    ...
    def _add_rules_without_ip_var(self, filter, rules):
        filter_xml = self._conn.nwfilterLookupByName(filter).XMLDesc(0)
        filter_et = xml.etree.ElementTree.fromstring(filter_xml)
        for subfilter in filter_et:
            if subfilter.tag == 'filterref':
                self._add_rules_without_ip_var(subfilter.get('filter'), rules)
            elif subfilter.tag == 'rule' and not self._rule_references_ip_var(subfilter):
                subfilter_xml = xml.etree.ElementTree.tostring(subfilter)
                rules.append(subfilter_xml)

    def _ip_variable_in_parameters(self, parameters):
        """
        (Platform9) Due to a bug in libvirt 0.9.8 (which is included by
        default in the Ubuntu12.04 repos), if the 'IP' variable is not
        defined as a parameter of the instance filter, then we do not
        include any rules in the filter that reference the 'IP' variable.
        libvirt 0.9.12 has been tested and does not have this issue.
        """
        for parameter in parameters:
            parameter = xml.etree.ElementTree.fromstring(parameter)
    ...
    def _get_instance_filter_xml(self, instance, filters, vif):
        ...
        # (PF9) libvirt 0.9.12 has been tested and successfully uses DHCP
        # when referencing the $IP variable before it is defined.
        if not libvirt or libvirt.getVersion() >= 9012 or self._ip_variable_in_parameters(parameters):
            for f in filters:
            ...
            rules = []
            for f in filters:
                self._add_rules_without_ip_var(f, rules)
            xml += "".join(rules)
        xml += '</filter>'
        return xml
