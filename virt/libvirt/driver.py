...
# PF9 import
from nova.virt import pf9_utils
...
# PF9 change
from nova.virt.libvirt import pf9_ip_discovery
from nova.virt import resource_types
from psutil import cpu_percent, virtual_memory, disk_usage, \
    net_io_counters as nw_counters
...
pf9_virt_opts = [
    cfg.ListOpt('ip_discovery_methods',
                default=['nwfilter','arp','sniffing'],
                help='IP discovery methods, in order of increasing precedence')
    ]

CONF = cfg.CONF
CONF.register_opts(libvirt_opts, 'libvirt')

pf9_virt_opt_group = cfg.OptGroup(name='PF9', title='PF9 specific options')
CONF.register_group(pf9_virt_opt_group)
CONF.register_opts(pf9_virt_opts, group=pf9_virt_opt_group)
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def get_all_networks_pf9(self, node=None):
        return pf9_utils.get_all_networks_pf9()

    def get_all_ip_mapping_pf9(self, needed_uuids=None):
        """
        For the libvirt driver we presently will ignore the needed_uuids
        argument. A very small efficiency could be gained by considering them,
        as it saves one call to libvirt, but it would mean that potential ARP
        mappings might be lost.
        """
        ip_add_mapping = self._find_all_mac_ip()
        uuids = self.list_instance_uuids()
        all_instance_info = dict()
        for uuid in uuids:
            info = self.get_instance_info(uuid)
            instance_ip_info = []
            if info and info['virtual_interfaces']:
                for vif_info in info['virtual_interfaces']:
                    vif_ip_info = dict()
                    vif_ip_info['mac_address'] = vif_info['mac_address']
                    bridge = None
                    if 'bridge' in vif_info:
                        bridge = vif_info['bridge']
                    vif_ip_info['bridge'] = bridge
                    ip_address = None
                    if vif_info['mac_address'] in ip_add_mapping:
                        ip_address = ip_add_mapping[vif_info['mac_address']]
                    vif_ip_info['ip_address'] = ip_address
                    instance_ip_info.append(vif_ip_info)
            all_instance_info[uuid] = instance_ip_info
        return all_instance_info

    def _find_all_mac_ip(self):
        results = {}
        for method in CONF.PF9.ip_discovery_methods:
            if method.lower() == 'arp':
                arp_results = pf9_ip_discovery.discover_ips_from_arp(processutils.execute)
                for r in arp_results:
                    results[r] = arp_results[r]
            elif method.lower() == 'nwfilter':
                nwf_results = pf9_ip_discovery.discover_ips_from_nwfilter(self._conn)
                for r in nwf_results:
                    results[r] = nwf_results[r]
            elif method.lower() == 'sniffing':
                sniffing_results = pf9_ip_discovery.discover_ips_from_sniffer()
                for r in sniffing_results:
                    results[r] = sniffing_results[r]
    ...
    # PF9 addition
    def instance_exists_ex(self, instance):
        try:
            self._host.get_guest(instance)
            return True
        except exception.NovaException as ex:
            LOG.exception(_('Exception while checking for instance: %s'), ex)
            return False
    # PF9 end
    ...
    def list_instance_uuids(self, node=None, template_uuids=None, force=False):
        # PF9 Change: The node parameter not used by libvirt, because there is
        # only 1 node / host.
        # node and template_uuids are  mainly needed for VMwareVCDriver
        uuids = []
        for dom in self._host.list_instance_domains(only_running=False):
    ...
    def get_instance_info(self, instance_uuid):
        """
        :param instance_uuid: Instance uuid for which to get information
        """
        virt_dom = self._conn.lookupByUUIDString(instance_uuid)
        xml_desc = virt_dom.XMLDesc(0)
        info = libvirt_utils.parse_instance_xml(xml_desc)
        state = libvirt_guest.LIBVIRT_POWER_STATE[virt_dom.info()[0]]
    ...
    def cleanup(self, context, instance, network_info, block_device_info=None,
        ...
        instance_files = []
        if destroy_disks:
            # PF9: Get file information before undefine
            instance_files = self._get_instance_files_pf9(instance)
        if destroy_vifs:
        ...
        if destroy_disks or (
            ...
            attempts = int(instance.system_metadata.get('clean_attempts',
                                                        '0'))
            # PF9: Delete file backings for discovered instances
            self._delete_instance_files_pf9(instance_files)
            # PF9: end
    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        with utils.tempdir(dir=snapshot_directory) as tmpdir:
            ...
            with libvirt_utils.file_open(out_path) as image_file:
                # PF9: We are setting purge_props to False here so
                # that metadata from the initial image creation do
                # not get marked deleted. Previously, pf9_description
                # gets marked deleted and is never shown in the UI.
                #
                # The side effect is that there are more properties
                # returned when querying the list of images. This
                # includes pf9_description, instance_type* properties
                # carried over from Nova.
    ...
    def _ok_to_poweron_pf9(self, guest):
        """Check if we have a valid domain in powered off state"""
        state = libvirt_guest.LIBVIRT_POWER_STATE[guest.get_power_state(
                self._host)]
        if state in [power_state.CRASHED, power_state.SHUTDOWN]:
            return True
        return False

    def _power_on_pf9(self, dom):
        self._create_domain(domain=dom)
    ...
    def power_on(self, context, instance, network_info,
        ...
        # PF9 begin -- Persistent domains must never hard reboot
        guest = self._host.get_guest(instance)
        if self._ok_to_poweron_pf9(guest):
            self._power_on_pf9(guest._domain)
        else:
    ...
    def get_all_ip_addr(self):
        """
        Get additional IPs assigned to hosts
        """
        ...
        for interface in interfaces:
            ...
            try:
                ...
                if not regex.match(ip):
                    ips.append({'interface_name': interface,
                                'ip': ip})
            except KeyError:
                pass
    ...
    def _create_image(self, context, instance,
        ...
        # File injection only if needed
            ...
            target_partition = self._inject_data(
                instance, network_info, admin_pass, files, suffix)

            # PF9 -- begin change
            # Key has already been injected once. No need to inject again
            key = None

            def get_injected_network_files(guest_os_info):
                """
                A nested callback function that generates
                guest OS-specific network files for injection.
                Note: This is a closure over 'network_info'.
                """
                return netutils.get_injected_network_files(network_info,
                                                           guest_os_info)
            metadata = instance.get('metadata')
            net = get_injected_network_files if network_info else None
            ...
            if any((key, net, metadata, admin_pass, files)):
                ...
                injection_image = self.image_backend.image(instance,
                                                           'disk' + suffix)
                injection_path = injection_image.get_model(self._conn)
                img_id = instance['image_ref']
    ...
    def _get_host_sysinfo_serial_auto(self):
        # PF9 change: Return hardware serial on NovaException instead of checking for file existence
        try:
    ...
    def get_pf9_hostid(self):
        ...
        """
        if not self._pf9_hostid:
            default_value = self._conn.getHostname()
            self._pf9_hostid = pf9_utils.get_pf9_hostid(LOG, default_value)
    ...
    def _get_cpu_rate_pf9(self):
        """Get CPU clock rate information of physical computer

        :returns: CPU core and Mhz information as dict
        """
        info = self._conn.getInfo()
        return [{'cores': info[2], 'mhz': info[3]}]
    ...
    def _get_cpu_info(self):
        ...
        # PF9: libvirt host_capabilities call does not report clock rate
        cpu_info['clock_rate'] = self._get_cpu_rate_pf9()
    ...
    def _live_migration_operation(self, context, instance, dest,
        ...
        # PF9: We use hostid for registering compute nodes instead of
        # OpenStack way of using hostnames. So the dest here does not
        # resolve correctly to a host. Fetch the compute record from
        # conductor and get the IP address of the compute node.
        compute_address = dest
        try:
            compute_node = objects.ComputeNodeList.get_all_by_host(context, dest)
            if len(compute_node) > 0:
                if len(compute_node) > 1:
                    LOG.warn('Got multiple compute nodes for {dest} ID'.format(
                        dest=dest))
                    LOG.warn('using the 0th record for migration')
                compute_node = compute_node[0]
                compute_address = compute_node.host_ip
            else:
                # Although we know migration is going to fail we cannot return
                # from here directly as there is cleanup that needs to be
                # completed on failed migrations. Same for exception part below
                LOG.warn('Did not get any compute node for {dest} ID'.format(
                    dest=dest))
        except Exception as e:
            LOG.exception('There was an exception getting IP address from '
                          'destination host ID - {err}'.format(err=e.message))
        ...
        try:
            ...
            if (migratable_flag is None or
                ...
                self._check_graphics_addresses_can_live_migrate(listen_addrs)
                self._verify_serial_console_is_disabled()
                # PF9: change dest to compute_address
                dom.migrateToURI(CONF.libvirt.live_migration_uri % compute_address,
                                 logical_sum,
                                 None,
                ...
                try:
                    # PF9: change dest to compute_address
                    dom.migrateToURI2(CONF.libvirt.live_migration_uri % compute_address,
                                      None,
                                      new_xml_str,
                                      logical_sum,
                                      None,
    ...
    def _get_instance_disk_info(self, instance_name, xml,
        ...
        for cnt, disk_node in enumerate(disk_nodes):
            ...
            target = disk_node.find('target').get('dev')
            disk_driver_type = disk_node.find('driver').get('type')
            path = None
            source = disk_node.find('source')
            # Source element may not always be present in disk_node info
            ...
            disk_info.append({'type': disk_driver_type,
                              'path': path,
    ...
    def _get_instance_files_pf9(self, instance):
        virt_dom = None

        try:
            virt_dom = self._host.get_guest(instance)
        except exception.InstanceNotFound:
            return []

        if not hasattr(virt_dom, 'XMLDesc'):
            return []

        instance_xml = virt_dom.XMLDesc(0)
        domain = etree.fromstring(instance_xml)
        ...
        for disk in disks:
            ...
            if file_path:
                # This device has backing of type disk
                files.append(file_path)

        return files

    def _delete_instance_files_pf9(self, files):
        nova_instance_path = CONF.instances_path

        for f in files:
            if not f.startswith(nova_instance_path):
                LOG.debug('Removing %s', f)
                libvirt_utils.delete_file_pf9(f, throw=False)
    ...
    # PF9 functions -
    pf9_resource_hooks = {
        resource_types.CPU: cpu_percent,
        resource_types.MEMORY: lambda: virtual_memory().percent,
        resource_types.DISK_TOTAL: lambda: disk_usage(CONF.instances_path).total
                                        / (1024 ** 3),
        resource_types.DISK_USED: lambda: disk_usage(CONF.instances_path).used
                                        / (1024 ** 3),
        resource_types.NETWORK_RECV_PKTS: lambda: nw_counters().packets_recv
                                        / 1000,
        resource_types.NETWORK_SENT_PKTS: lambda: nw_counters().packets_sent
                                        / 1000,
        resource_types.NETWORK_SEND_RATE: lambda: 8.0 * nw_counters().bytes_sent
                                        / (1000 ** 2),
        resource_types.NETWORK_RECV_RATE: lambda: 8.0 * nw_counters().bytes_recv
                                        / (1000 ** 2),
    }
    ...
    def _update_stats_pf9(self, resource_type):
        ...
        data = dict()
        if self.pf9_resource_hooks:
            data = self.pf9_resource_hooks[resource_type]()
