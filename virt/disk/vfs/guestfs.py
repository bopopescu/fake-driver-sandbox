...
class VFSGuestFS(vfs.VFS):
    ...
    def get_guest_os(self):
        return self.gos_type, self.gos_distro, self.gos_major_version
        # PF9: end
    ...
    def setup_os_inspect(self):
        ...
        # PF9 change
        self.gos_type = self.handle.inspect_get_type(roots[0])
        self.gos_distro = self.handle.inspect_get_distro(roots[0])
        self.gos_major_version = self.handle.inspect_get_major_version(roots[0])

        # Newer CentOS 7 images' /etc/redhat-release file has
        # changed and libguestfs versions readily available in
        # repos as of this writing cannot handle this change.
        # Earliest version that can support this is v1.29.45
        # which was release on June 2nd, 2015.  Once this
        # version is generally available in repos, we should
        # remove this workaround.
        #
        # This particular section parses /etc/centos-release
        # to determine if it's a CentOS distribution and to
        # get the major version of the distribution--which we
        # check before building RHEL style network config.
        if self.gos_distro == "redhat-based":
            try:
                release_file = self.handle.cat("/etc/centos-release")
                LOG.debug("OS is redhat-based: {0}".format(release_file))
                base_string = "^CentOS.* release (?P<version>.*) \("
                regex = re.compile(base_string)
                the_match = regex.match(release_file)
                the_version = StrictVersion(the_match.group("version"))
                self.gos_major_version = the_version.version[0]
            except RuntimeError as e:
                LOG.error(_("OS is redhat-based and not CentOS: %s"), e)
        # PF9 end
