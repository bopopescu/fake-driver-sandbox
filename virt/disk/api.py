...
def _inject_net_into_fs(net, fs):
    """ PF9: Change -- Inject network-related files into the filesystem rooted
    at fs.
    net is a callback function that generates the files to inject
    """
    guest_os_info = fs.get_guest_os()
    files = net(guest_os_info)
    for path, contents in files:
        dirname = os.path.dirname(path)
        fs.make_path(dirname)
        # PF9 change
        #fs.set_ownership(dirname, 'root', 'root')
        #fs.set_permissions(dirname, 0o744)
        _inject_file_into_fs(fs, path, contents)
