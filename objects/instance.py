...
class _BaseInstance(base.NovaPersistentObject, base.NovaObject,
    ...
    fields = {
        ...
        'root_gb': fields.FloatField(nullable=True),
    ...
    def _get_vmw_name_pf9(self):
        global PF9_IS_VMWARE
        try:
            if PF9_IS_VMWARE is None:
                # instance.host property is not present when querying name during
                # creation, so check hypervisor_type of any of the compute_nodes
                LOG.debug('Platform9: Loading some compute node to determine the'
                          ' type of hypervisor')
                compute_node = objects.ComputeNodeList.get_all(self._context)[0]
                PF9_IS_VMWARE = compute_node.hypervisor_type == \
                    'VMware vCenter Server'
            if not PF9_IS_VMWARE:
                return None

            # Definitely VMware DU (not a discovered instance)
            return '{name}-{id}'.format(name=self.hostname,
                                            id=self.id)
        except:
            return None
    ...
    @property
    def name(self):
        # PF9: Read name_on_host value from system_metadata, if present.
        try:
            if 'name_on_host' in self['system_metadata']:
                return self['system_metadata']['name_on_host']
        # Not catching just AttributeError here to avoid unit test failures
        except Exception:
            pass

        # If name is not found in sysmeta, use display_name for discovered vms
        if hasattr(self, 'display_description') and self.display_description == 'Discovered Instance':
            return self.display_name

        # Get the correct name on host in case of VMware deployments
        vmw_name = self._get_vmw_name_pf9()
        if vmw_name:
            return vmw_name
        # PF9 end
    ...
    def _load_flavor(self):
        ...
        # PF9 change: Because we upgraded directly from Juno to Liberty we
        # might still be dealing with unmigrated flavor data. Migrate it now.
        # This path should be removed with IAAS-5145
        try:
            ...
            LOG.warn(_LW("PF9: Migrating flavor data for instance - "
                         "{uuid}".format(uuid=self.uuid)))
            orig_read_deleted = self._context.read_deleted
            self._context.read_deleted = "yes"
            self.flavor = objects.Flavor.get_by_id(self._context,
                                                   self.instance_type_id)
            self._context.read_deleted = orig_read_deleted
            self.old_flavor = None
            self.new_flavor = None
            self.save()
...
class _BaseInstanceList(base.ObjectListBase, base.NovaObject):
    ...
    @base.remotable_classmethod
    def get_by_network_id_pf9(cls, context, network_id):
        db_inst = db.instance_get_all_by_network_pf9(context, network_id)
        return _make_instance_list(context, cls(), db_inst, ['info_cache', 'system_metadata'])
