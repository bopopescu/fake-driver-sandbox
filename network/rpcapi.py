...
class NetworkAPI(object):
    ...
    def add_discovered_ip_addresses(self, ctxt, instance_ip_info):
        self.client.cast(ctxt, 'add_discovered_ip_addresses', instance_ip_info=instance_ip_info)

    def add_discovered_networks(self, ctxt, node, all_nets):
        return self.client.call(ctxt, 'add_discovered_networks',
                                node=node, all_nets=all_nets)

    def add_discovered_networks_for_node_pf9(self, ctxt, host, all_nets, node):
        return self.client.call(ctxt, 'add_discovered_networks_for_node_pf9',
                                host=host, all_nets=all_nets, node=node)
    ...
    def update_network(self, ctxt, uuid, **kwargs):
        return self.client.call(ctxt, 'update_network',
                                uuid=uuid, **kwargs)
