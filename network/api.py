...
class API(base_api.NetworkAPI):
    ...
    @wrap_check_policy
    def update(self, context, network_uuid, **kwargs):
        return self.network_rpcapi.update_network(context, network_uuid, **kwargs)
    ...
    def add_discovered_ip_addresses(self, context, instance_ip_info):
        args = {'instance_ip_info': instance_ip_info}
        self.network_rpcapi.add_discovered_ip_addresses(context, **args)

    def add_discovered_networks(self, context, node, all_nets):
        args = {'all_nets': all_nets, 'node': node}
        self.network_rpcapi.add_discovered_networks(context, **args)

    def add_discovered_networks_for_node_pf9(self, context, host, all_nets, node):
        args = {'all_nets': all_nets, 'host': host, 'node':node}
        self.network_rpcapi.add_discovered_networks_for_node_pf9(context, **args)
    ...
    def get_access_pf9(self, context, network_id):
        """Get project_ids with access to network"""
        #TODO: enforce policy
        project_ids = self.db.network_get_access_pf9(context, network_id)
        return project_ids

    def add_access_pf9(self, context, network_id, project_id):
        """Add project as user of a private network"""
        #TODO: enforce policy
        self.db.network_add_access_pf9(context, network_id, project_id)

    def remove_access_pf9(self, context, network_id, project_id):
        """Remove project as authorized user of a private network"""
        #TODO: enforce policy
        self.db.network_remove_access_pf9(context, network_id, project_id)

    @wrap_check_policy
    def delete_vif_for_instance_pf9(self, context, instance_uuid, vif_id):
        """
        PF9 function. Remove vif from an instance
        :param contest:
        :param instance_uuid: UUID of instance
        :param vif_id:
        :return:
        """
        vifs = objects.VirtualInterfaceList.get_by_instance_uuid(context, instance_uuid)
        vif = filter(lambda v: v.uuid == vif_id, vifs)

        if vif:
            vif = vif[0]
            vif.delete(context)
        else:
            raise exception.NotFound(_('vif={vif_id}'.format(vif_id=vif_id)))
