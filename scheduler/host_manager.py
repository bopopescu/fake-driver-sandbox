...
class HostState(object):
    ...
    def update_from_network_info(self, networks):
        # Pf9: networks are models.Network object
        self.network_uuids = set([net['uuid'] for net in networks])
...
class HostManager(object):
    ...
    def get_filtered_hosts(self, hosts, filter_properties,
            filter_class_names=None, index=0, filter_errors={}):
        ...
        return self.filter_handler.get_filtered_objects(filters,
                hosts, filter_properties, index, filter_errors=filter_errors)
    ...
    def get_all_host_states(self, context):
        ...
        for compute in compute_nodes:
            ...
            # Pf9: Get networks available on compute node
            network_ids = [n['network_id'] for n in
                           db.compute_node_get_all_networks_pf9(context,
                                                               compute['id'])]
            networks = []
            if network_ids:
                all_networks = db.network_get_all(context)
                networks = filter(lambda n: n['id'] in network_ids,
                                  all_networks)
            else:
                LOG.info(_LI("No networks on host {host}".\
                           format(host=compute['id'])))
            # PF9 end
            ...
            # PF9 change
            host_state.update_from_network_info(networks)
    ...
    def get_filtering_errors(self, filter_errors):
        return self.filter_handler.get_error_description(filter_errors)
