...
class FilterScheduler(driver.Scheduler):
    ...
    def select_destinations(self, context, request_spec, filter_properties):
        ...
        # PF9: Get scheduling error, if any.
        selected_hosts, msg = self._schedule(context, request_spec,
                                             filter_properties)
        ...
        if len(selected_hosts) < num_instances:
            ...
            # PF9: Bubble the error message instead of a generic message
            # reason = _('There are not enough hosts available.')
            # raise exception.NoValidHost(reason=reason)
            # PF9: Raise with any error msg
    ...
    def _get_exception(self, exc_record):
            """
            PF9: Get exception from RPC traceback
            """
            exc_type = None
            regex = re.compile('.*exception\.(.+)\(.*')
            for rec in exc_record:
                if not exc_type:
                    match = regex.findall(rec)
                    if match:
                        exc_type = match[0]
                else:
                    if rec.startswith(exc_type):
                        return rec

            LOG.debug('No exception found in {exc}'.format(exc=exc_record))
            return ''
    ...
    def _schedule(self, context, request_spec, filter_properties):
        ...
        if 'retry' in filter_properties:
            ...
            error_msg = self._get_exception(retry['exc']) if 'exc' in retry else ''
        else:
            error_msg = ''
        filter_errors = {}
        # PF9 end
        ...
        for num in range(num_instances):
            ...
            hosts = self.host_manager.get_filtered_hosts(hosts,
                    filter_properties, index=num, filter_errors=filter_errors)
            if not hosts:
                # Can't get any more locally.
                # PF9
                if filter_errors:
                    error_msg = self.host_manager.get_filtering_errors(filter_errors)
                break
        ...
        # PF9: Return error msg, if any
        return selected_hosts, error_msg
