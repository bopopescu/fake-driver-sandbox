...
from nova.virt.stats_collector import Stat
...
class ResourceTracker(object):
    ...
    @classmethod
    def get_resources_pf9(cls):
        #FIXME: Make duration configurable
        return [
            Stat('% CPU Used', resource_types.CPU, 'avg', 5, 'percent'),
            Stat('% Memory Used', resource_types.MEMORY, 'avg', 5, 'percent'),
            Stat('Disk Total', resource_types.DISK_TOTAL, 'latest', 60, 'GB'),
            Stat('Disk Used', resource_types.DISK_USED, 'latest', 60, 'GB'),
            Stat('Network Received Mbps', resource_types.NETWORK_RECV_RATE,
                 'delta_avg', 10, 'Mbps'),
            Stat('Network Sent Mbps', resource_types.NETWORK_SEND_RATE,
                 'delta_avg', 10, 'Mbps'),
            Stat('Network Received Pkts', resource_types.NETWORK_RECV_PKTS,
                 'delta', 10, 'K'),
            Stat('Network Sent Pkts', resource_types.NETWORK_SENT_PKTS,
                 'delta', 10, 'K')
        ]
    ...
    def _get_all_ips(self):
        try:
            return self.driver.get_all_ip_addr()
        except NotImplementedError:
            return []

    def _get_pf9_id(self):
        try:
            return self.driver.get_pf9_hostid()
        except NotImplementedError:
            return ""
    ...
    def update_available_resource(self, context):
        ...
        resources['pf9_ip_info'] = self._get_all_ips()
        resources['pf9_ext'] = [self._get_pf9_id()]
    ...
    def _update_pf9_stats(self, context, values):
        self.conductor_api.compute_node_update_pf9_stats(
    ...
    def update_resource_usage_pf9(self, context):
        """Refresh physical usage information of various resources on host
        """
        current_time = time()
        stat_update = {}
        res_types = [x.type for x in self.resources_pf9]
        # This class is instantiated per node,
        # Since a nova compute host can have multiple nodes,
        # we need to report stats only for the give node
        stats = self.driver.get_host_stats_pf9(res_types, refresh=True, nodename=self.nodename)
        for res in self.resources_pf9:
            if res.is_right_iteration(current_time):
                if stats and (res.type in stats.keys()):
                    val = res.add_sample(stats[res.type], current_time)
                    if res.did_rollover(current_time):
                        stat_update[res.type] = val
        if stat_update:
            LOG.debug('Sending stats: %s', stat_update)
            self._update_pf9_stats(context, stat_update)
