...
from nova.objects import network_request as net_request_obj
from nova.objects import network as net_obj
...
pf9_opts = [
    cfg.StrOpt('pf9_project_id', default=None, help='ID of PF9 system project'),
    cfg.StrOpt('pf9_user_id', default=None, help='User ID of PF9 system user'),
    cfg.StrOpt('pf9_flavor', default='m1.medium',
               help="Name of PF9 system flavor")
]
...
CONF.register_opts(pf9_opts, group=pf9_opt_group)
...
class API(base.Base):
    ...
    def _record_action_finish(self, context, instance):
        objects.InstanceAction.action_finish(context,
                                             instance['uuid'],
                                             want_result=False)
    ...
    def _check_num_instances_quota(self, context, instance_type, min_count,
                                   max_count, quota_overrides=dict(),
                                   project_id=None, user_id=None):
        """Enforce quota limits on number of instances created."""

        # PF9: Begin change
        # use quota overrides here so as to update quota usages as per
        # the actual usage by the instance. This is needed for discovered
        # instances which are discovered pf9.unknown (1 cpu, 1MB ram and
        # 0GB disk)
        req_cores = max_count * quota_overrides.get('vcpus',
                                                    instance_type['vcpus'])
        vram_mb = int(instance_type.get('extra_specs', {}).get(VIDEO_RAM, 0))
        req_ram = max_count * quota_overrides.get('memory_mb',
                                                  instance_type['memory_mb'])
        req_root_gb = max_count * quota_overrides.get('root_gb',
                                                      instance_type['root_gb'])
        # PF9 FIXME: quota reservations and usage entries can only store
        # integers, so we round up here. For more precision, consider switching
        # the storage quota to mb. As it stands, small VMs will eat a full gig
        # of quota.
        req_root_gb = int(ceil(req_root_gb))

        # Check the quota
        try:
            quotas = objects.Quotas(context)
            quotas.reserve(instances=max_count,
                           cores=req_cores, ram=req_ram, root_gb=req_root_gb,
            ...
            # PF9 FIXME: Reduce 'allowed' instances in line with the root_gb (0
            # sized flavors????)
            if instance_type['root_gb']:
                allowed = min(allowed,
                              headroom['root_gb'] // instance_type['root_gb'])
            ...
            requested = dict(instances=num_instances, cores=req_cores,
                             ram=req_ram, root_gb=req_root_gb)
    ...
    def _validate_and_build_base_options(self, context, instance_type,
        ...
        # PF9 TODO: Juno/Icehouse converted everything to objects.
        if requested_networks:
            if isinstance(requested_networks, net_request_obj.NetworkRequestList):
                net_uuids = [r.to_tuple()[0] for r in requested_networks]
            else:
                net_uuids = [n[0] for n in requested_networks]
    ...
    def _build_filter_properties(self, context, scheduler_hints, forced_host,
                                 forced_node, instance_type, requested_networks=None):
        ...
        # PF9 Change
        if (requested_networks and isinstance(requested_networks, objects.NetworkRequestList)):
            # FIXME: As of Juno, tuple is not needed anymore
            requested_networks = [(net.network_id, net.network_id)
    ...
    def pf9_get_total_disk_size(self, boot_meta, instance_type, bdm_data):

            total_disk_size_bytes = 0

            # PF9: When creating instance from 0 disk flavor, set the disk
            # size to virtual size of the image
            if boot_meta and not instance_type['root_gb']:
                image_properties = boot_meta['properties']
                if 'pf9_virtual_size' in image_properties.keys():
                    total_disk_size_bytes = int(image_properties['pf9_virtual_size'])
                    LOG.info('Zero disk flavor. Setting instance disk size to '
                             'match virtual size of image (%d)B', total_disk_size_bytes)
                else:
                    LOG.info('Expected pf9_virtual_size property missing '
                             'from image metadata. Instance disk size will be set '
                             'to size reported in block_device_mapping_v2 field')
            # If the instance is provisioned from non-0 disk flavor take
            # root_gb from the flavor
            elif instance_type['root_gb']:
                total_disk_size_bytes = instance_type['root_gb'] * (1024.0 ** 3)

            # BDM data should be present only in case of discovered VMs
            if bdm_data:
                for bdm in bdm_data:
                    if bdm.has_key('virtual_size') and bdm['virtual_size'] > 0:
                        total_disk_size_bytes += bdm['virtual_size']

            # PF9: This code is supposed to save disk size in GB. The DB field
            # has limited precision, and cannot faithfully scale up to real
            # bytes value when nova-compute checks it again. The logic below
            # addresses two issues
            # (1) Make sure once GB value is stored in DB is scaled down to bytes,
            #     the instance disk size is enough to accommodate image
            # (2) Make sure the value will align on MB boundary, otherwise
            # resize does not work.
            # See libvirt/driver.py:_create_image() function which uses this
            # value to ensure the instance disk can accommodate image.

            # (1) Compute disk size in MB, round up the float, so that even if
            # precision is lost when saving the value in DB, the disk size will
            # be large enough to accommodate virtual size of image
            total_disk_size = ceil(total_disk_size_bytes / (1024.0 * 1024.0))

            # (2) Scale down the MB value to GB.
            total_disk_size /= 1024.0
    ...
    def _create_instance(self, context, instance_type,
        ...
        # pacharya: Liberty removed BDM arg in
        # _validate_and_build_base_options. We were using bdm_data to
        # update the root_gb value. Explicitly handling it here now.
        base_options['root_gb'] = self.pf9_get_total_disk_size(boot_meta, instance_type, block_device_mapping)
    ...
    @staticmethod
    def _validate_register_instance_params(update_data):
        expected_keys = ['name', 'instance_uuid', 'vcpus', 'memory_mb']

        for key in expected_keys:
            if key not in update_data.keys():
                raise exception.InvalidParameterValue(key)

    @staticmethod
    def _map_power_state_to_vm_state(cur_power_state):
        power_state_to_vm_state = {power_state.RUNNING: vm_states.ACTIVE,
                                   power_state.SHUTDOWN: vm_states.STOPPED,
                                   power_state.CRASHED: vm_states.STOPPED,
                                   power_state.NOSTATE: vm_states.ERROR,
                                   power_state.PAUSED: vm_states.PAUSED,
                                   power_state.SUSPENDED: vm_states.SUSPENDED,
                                   }
        if power_state_to_vm_state.has_key(cur_power_state):
            return power_state_to_vm_state[cur_power_state]

        return vm_states.ERROR

    def _instance_register(self, context, host, update_data):
        self._validate_register_instance_params(update_data)
        # Create default parameters, may change in future
        reservation_id = utils.generate_uid('r')
        security_groups = []
        instance_type = flavors.get_flavor_by_name(CONF.PF9.pf9_flavor,
                                                   ctxt=context)
        image_id = None
        access_ip_v4 = update_data.get('access_ip_v4', None)
        access_ip_v6 = update_data.get('access_ip_v6', None)
        display_name = update_data['name']

        # TODO: Localize
        display_description = 'Discovered Instance'
        config_drive = None
        kernel_id = None
        ramdisk_id = None

        boot_meta = {}
        legacy_bdm = not(update_data['block_device_mapping_v2'])
        bdm_data = update_data.get('block_device_mapping_v2', []) \
                   or update_data.get('block_device_mapping', [])

        get_bdm_data = self._get_bdm_image_metadata

        try:
            boot_meta['properties'] = get_bdm_data(context, bdm_data,
                                                   legacy_bdm)
            # Should not overwrite "_", it is a function in nova.i18n
            # and breaks logging in this function
            base_options, ignore_this_ret_val =\
                self._validate_and_build_base_options(context, instance_type,
                                                      boot_meta, None,
                                                      image_id, kernel_id,
                                                      ramdisk_id, display_name,
            ...
            # _validate_and_build_base_options. We were using bdm_data to
            # update the root_gb value. Explicitly handling it here now.
            base_options['root_gb'] = self.pf9_get_total_disk_size(boot_meta, instance_type, bdm_data)

            base_options['uuid'] = update_data['instance_uuid']

            # PF9: replace quantities derived from (fake) flavor with real values
            # sent from nova-compute's instance discovery.
            base_options['memory_mb'] = update_data['memory_mb']
            base_options['vcpus'] = update_data['vcpus']

            bdm = self._check_and_transform_bdm(context, base_options, instance_type,
                                                boot_meta,
                                                1, 1,
                                                bdm_data or [], legacy_bdm)

            inst = self._provision_instances(context, instance_type, 1, 1,
                                             base_options, boot_meta,
                                             security_groups, bdm, False,
                                             None, False)[0]

            self._record_action_start(context, inst, instance_actions.REGISTER)
        except exception.InvalidBDM as inval_bdm:
            LOG.warning(_("Skipping instance %s, error %s"),
                          update_data['instance_uuid'], inval_bdm)
            return None

        except (exception.FlavorNotFound, exception.InstanceUserDataMalformed):
            LOG.warning(_("Skipping instance %s, invalid instance information"),
                        update_data['instance_uuid'])
            return None

        notifications.send_update_with_states(context, inst, vm_states.BUILDING,
                                              vm_states.ACTIVE, None, None,
                                              service="api")

        current_vm_state =\
            self._map_power_state_to_vm_state(update_data['power_state'])
        old_ref, inst = \
            self.db.instance_update_and_get_original(
                context, inst.uuid,
                {'host': host, 'node': update_data['node'],
                 'task_state': None,
                 'hostname': display_name,
                 'system_metadata': {'name_on_host': update_data['name']},
                 'vcpus': update_data['vcpus'],
                 'memory_mb': update_data['memory_mb'],
                 'vm_state': current_vm_state,
                 'project_id': CONF.PF9.pf9_project_id,
                 'user_id': CONF.PF9.pf9_user_id,
                 'power_state': update_data['power_state'],
                 'launched_at': timeutils.utcnow()},
                 # PF9 change: join system_metadata for storing name_on_host
                 columns_to_join = ['system_metadata'])
        self._record_action_finish(context, old_ref)
        LOG.info('Registered new instance: %s' % inst.uuid)


    def _instance_undelete_on_register(self, context, instance):
        """
        PF9: restore an instance that was previously marked deleted, reserving
        and commiting quota resources.
        """
        ...
        quota_overrides = {'root_gb': instance['root_gb'],
                           'memory_mb': instance['memory_mb'],
                           'vcpus': instance['vcpus']}
        LOG.info('undeleting instance %s, quotas: %s', instance['uuid'],
                quota_overrides)
        num_instances, quotas = self._check_num_instances_quota(context,
                instance_type, 1, 1, quota_overrides, project_id=project_id,
                user_id=user_id)
        self.db.instance_restore(context, instance['uuid'])
        quotas.commit()

    def _check_and_fix_vm_state(self, context, instance, update):
        """
        PF9 function: Mark the VM as active again if needed
        :param context:
        :param instance:
        :return:
        """
        if instance.vm_state == vm_states.ERROR:
            instance_faults = objects.InstanceFaultList.get_by_instance_uuids(context,
                                                                          [instance.uuid])
            len_instance_faults = len(instance_faults);
            if len_instance_faults == 0:
                LOG.debug('Instance is in error state, but no faults found', instance=instance)
                return

            len_pf9_faults = 0;
            for fault in instance_faults:
                if fault.code == exception.InstanceNotFoundOnHypervisorPF9.code:
                    len_pf9_faults += 1
                    fault.update(dict(deleted=True))

            # Check if other faults are present for this VM
            if len_instance_faults > len_pf9_faults:
                LOG.info('Instance has other associated faults, not marking active',
                         instance=instance)
                return

        instance.vm_state = self._map_power_state_to_vm_state(update['power_state'])
        if instance.vm_state == vm_states.ACTIVE:
            instance.terminated_at = None
        instance.save()

    def instance_register(self, context, host, updates):
        """
        This API is internal, and for pf9-only. Set the context right so
        that instances get registered with correct system tenant
        """

        # In addition to existing, lookup deleted instances in openstack DB.
        # It may happen that an instance is marked 'deleted' by openstack but it
        # exists on hypervisor. Do not re-add such instance
        LOG.info('instance_register called with updates: %s' % str(updates))
        context.read_deleted = 'yes'
        context.project_id = CONF.PF9.pf9_project_id
        context.user_id = CONF.PF9.pf9_user_id
        for update in updates:
            if 'instance_uuid' not in update or update['instance_uuid'] is None:
                LOG.error(_('UUID missing in instance update'))
                continue
            # Skip servers already known
            try:
                uuid = update['instance_uuid']
                old_instance = objects.Instance.get_by_uuid(context, uuid, expected_attrs=['system_metadata'])
                if old_instance.deleted:
                    self._instance_undelete_on_register(context, old_instance)
                    # IAAS-3179 In case the instance is reported with a new host,
                    # as happens when the appliance is changed on VMware
                    if host != old_instance['host']:
                        self.db.instance_update_and_get_original(context, uuid, {'host': host})
                if old_instance.vm_state == vm_states.ERROR or old_instance.vm_state == vm_states.DELETED:
                    # Check and correct vm_state
                    self._check_and_fix_vm_state(context, old_instance, update)
                LOG.debug('sysmeta db: %s, actual name: %s' % (str(old_instance.system_metadata), update['name']))
                if 'name_on_host' not in old_instance.system_metadata or \
                   old_instance.system_metadata['name_on_host'] != update['name']:
                    # Update instance name in db based on the actual name on hypervisor
                    old_instance.system_metadata['name_on_host'] = update['name']
                    old_instance.save()

            except exception.InstanceNotFound:
                try:
                    self._instance_register(context, host, update)
                except Exception as e:
                    # Catch a general exception, and skip instance,
                    # otherwise an instance registration error will block rest of discovery
                    LOG.error(_("Skipping instance: %s, error: %s, exception type: %s")
                              % (update['instance_uuid'], e, e.__class__.__name__))
    ...
    def _validate_bdm(self, context, instance, instance_type, all_mappings):
        ...
        # PF9: Ignore boot index sequence if its a discovered instance since it
        # is already working properly we just want to add it to database
        # assumption: pf9.unknown flavor is not visible / usable by user hence
        # no VMs other than discovered ones can have that flavor
        # Reason (pushkar): It is easy to create VMs with disks that are out of
        # boot order - Create a VM with 3 disks and delete the
        # middle disk. Now VM has disks with boot indices 0 and 2.
        # Further also removing the check for boot_index 0 in discovered VMs, since
        # we can have discovered VMs with no boot_index 0 that function correctly
        # Reason (amrish): Boot_index 0 (and strict ordering) is needed mainly for nova-boot.
        # Since discovered VMs have no bearing on nova-boot, we don't need to check for either.
        ignore_indices = instance_type.get('name', None) == CONF.PF9.pf9_flavor

        if 0 not in boot_indexes or not _subsequent_list(boot_indexes):
            if ignore_indices:
                LOG.warn(_("One or more disks of {uuid} instance are out of "
                           "order, or boot_index 0 is missing".format(uuid=instance.uuid)))
                # PF9: end
            else:
    ...
    def create_db_entry_for_new_instance(self, context, instance_type, image,
        ...
        # PF9 change: populate instance name based on template in sysmeta
        # Do this after instance create, so that its id is known
        system_meta = {'name_on_host': instance.name}
        instance.system_metadata.update(system_meta)
    ...
    def _delete(self, context, instance, delete_type, cb, **instance_attrs):
        ...
        try:
            ...
            try:
                # PF9 change: Always check if service is up and throw exception if it isn't
                service = objects.Service.get_by_compute_host(
                        context.elevated(), instance.host)
                is_up = self.servicegroup_api.service_is_up(service)
                if not shelved_offloaded:
                    is_local_delete = not is_up
                # PF9 end
		...
		# PF9 change
		if not is_up:
		    # If compute node is down, raise error
		    raise exception.ComputeServiceUnavailable(host=instance.host)
		# PF9 end
    ...
    def _create_reservations(self, context, instance, original_task_state,
        ...
        # FIXME - see pf9 comment on ceil in _check_num_instances_quota
        instance_root_gb = ceil(instance.root_gb)
...
class HostAPI(base.Base):
    ...
    def discovery_task_state_pf9(self, context, host_name):
        """Returns state of Pf9 discovery task (running or not)."""
        host_name = self._assert_host_exists(context, host_name,
                         must_be_up=True)
        return self.rpcapi.discovery_task_state_pf9(context, host=host_name)

    def trigger_discovery_task_pf9(self, context, host_name):
        """Triggers Pf9 discovery task to run outside of periodic interval."""
        host_name = self._assert_host_exists(context, host_name,
                         must_be_up=True)
        return self.rpcapi.trigger_discovery_task_pf9(context, host=host_name)
