...
# PF9: FIXME: Need to move functionality from template reporter to
#             another service
from template_reporter_pf9 import TemplateReporter
...
interval_opts = [
    ...
    # PF9 change
    cfg.IntOpt('sync_nw_info_interval',
               default=60,
               help='interval to sync IP info between compute & '
                    'conductor'),
...
pf9_compute_opts = [
    cfg.IntOpt('storage_recency_hours',
               default=24*7,
               help="Maximum hours elapsed in which use of storage is "
                    " considered recent"),
    cfg.IntOpt('update_resource_utilization_pf9_interval',
               default=1800,
               help="Seconds between resource utilization updates"),
    cfg.IntOpt('sync_network_info_interval',
               default=600,
               help="Seconds between network discovery updates"),
    cfg.IntOpt('remove_instances_not_on_driver_interval',
               default=600,
               help="Seconds between hypervisor sync updates"),
    cfg.IntOpt('find_orphaned_vms_interval',
               default=600,
               help="Seconds between instance discovery updates"),
    cfg.IntOpt('sync_ip_interval',
               default=60,
               help="Seconds between IP synchronizations"),
    cfg.IntOpt('sync_ip_skips',
               default=10,
               help="Skips between full IP synchronization"),
    cfg.IntOpt('sync_ip_recency',
               default=3600,
               help="Recently created instances timeout")
    ]

CONF = cfg.CONF

# PF9 change
pf9_compute_opt_group = cfg.OptGroup(name='PF9', title='PF9 specific options')
CONF.register_group(pf9_compute_opt_group)
CONF.register_opts(pf9_compute_opts, group=pf9_compute_opt_group)
# PF9 end
...
class ComputeManager(manager.Manager):
    ...
    def __init__(self, compute_driver=None, *args, **kwargs):
        ...
        # PF9: Begin
        self._known_vms = dict()
        self._vm_cache_mutex = Lock()
        self._check_discovery_task_running = Lock()
        self.is_discovery_task_running = False
        self._known_vms_ip = {}
        self._vm_ip_cache_mutex = Lock()
        self._discovered_nw_cache = dict()
        self._sync_ip_counter = 0
        self._instance_dirty_flag = False
        self._pf9_cached_instances = dict()
        self._sync_ip_thread_lock = Lock()
        self._sync_ip_thread_running = False
        # PF9: End
    ...
    def _clean_known_vm_cache(self, driver_uuids):
        """
        PF9: Clean up known vms cache
        """
        to_purge_keys = set()
        with self._vm_cache_mutex:
            for known_uuid in self._known_vms:
                if known_uuid not in driver_uuids:
                    to_purge_keys.add(known_uuid)

        return to_purge_keys

    def _clean_known_vm_ip_cache(self, driver_uuids):
        """
        PF9 function
        """
        to_purge_ip_keys = set()
        with self._vm_ip_cache_mutex:
            for known_uuid in self._known_vms_ip:
                if known_uuid not in driver_uuids:
                    to_purge_ip_keys.add(known_uuid)
        return to_purge_ip_keys

    def _can_cleanup_instance(self, db_instance, driver_uuids):
        # akapoor: Refreshing the instance before using it here is a
        # severe performance drain for environments at scale.
        # db_instance.refresh(use_subordinate=True)

        if db_instance.host != self.host:
            # Migration. Sending end of nova-compute
            LOG.info(_("During sync hypervisor state the "
        ...
        if db_instance.task_state is not None:
            LOG.info(_('During hypervisor state sync the instance has a pending task (%(task)s), skipping'),
        ...
        if uuid not in driver_uuids:
            current_time = timeutils.utcnow()
            if db_instance.launched_at and \
               timeutils.is_older_than(db_instance.launched_at, 10 * 60):
                LOG.info(_('Instance %s marked as not found') % uuid)
                return True
        return False

    @periodic_task.periodic_task(spacing=CONF.PF9.remove_instances_not_on_driver_interval, run_immediately=True)
    def _remove_instances_not_on_driver(self, context):
        """
        PF9 function
        Purge instance records for the instances found
        in database, but have been deleted from hypervisor
        """
        LOG.info("Running instances cleanup task")
        db_instances = objects.InstanceList.get_by_host(context,
                                                        self.host,
                                                        use_subordinate=True)

        try:
            driver_template_uuids = []
            driver_uuids = set(self.driver.list_instance_uuids(\
                                        template_uuids=driver_template_uuids))
            driver_uuids.update(driver_template_uuids)
        except NotImplementedError:
            # FIXME implement for non-uuid hypervisors
            return

        to_purge_keys = self._clean_known_vm_cache(driver_uuids)
        to_purge_ip_keys = self._clean_known_vm_ip_cache(driver_uuids)

        instances_in_error = []
        for db_instance in db_instances:
            LOG.debug("Checking if instance: %s can be cleaned up", db_instance)
            if self._can_cleanup_instance(db_instance, driver_uuids):
                LOG.info("Cleaning up instance: %s from db", db_instance)
                instances_in_error.append(db_instance['uuid'])

        if instances_in_error:
            self.conductor_api.instance_not_found(context, self.host, instances_in_error)

        # PF9: Update cache
        with self._vm_cache_mutex:
            for key in to_purge_keys:
                del self._known_vms[key]

        #PF9: Update vm ip cache
        with self._vm_ip_cache_mutex:
            for key in to_purge_ip_keys:
                del self._known_vms_ip[key]
    ...
    def _populate_retry(self, retry, ex_val):
        """
        PF9 function: Fill exception portion of retry structure so that it can be
        reused
        :param retry:
        :param ex_val:
        :return:
        """
        if not all([retry, ex_val]):
            LOG.info(_('Retry not part of request or no exception to log, not queuing recent errors.'))
            return

        pf9_error = retry.setdefault('pf9_error', [])

        if not isinstance(ex_val, list):
            ex_val = ex_val.split('\n')

        pf9_error.append((self.host, ex_val[-1]))
        LOG.debug('Original ex: {ex}, using ex: {ex2}'.format(ex=ex_val, ex2=ex_val[-1]))
    ...
    def _reschedule(self, context, request_spec, filter_properties,
        ...
        if exc_info:
            ...
            # PF9 change
            exception_message = traceback.format_exception_only(exc_info[0], exc_info[1])
            retry['exc'] = exception_message
            self._populate_retry(retry, exception_message)
            # PF9 end
...
# TODO: Pushkar cleanup this function
# =======
#     @object_compat
...
#         try:
#
#             # PF9: Next line is for optimization. If find_orphaned_vms finds a VMs
#             # UUID in known_vms then find_orphaned_vms will not query
#             # hypervisor to get information related to this VM.
#             # Information reated to this VM will be populated in this method
#             # PF9: end
#             self._known_vms[instance['uuid']] = {'template': False}
...
# >>>>>>> atherton
    ...
    def _do_build_and_run_instance(self, context, instance, image,
        ...
        try:
            ...
            self._instance_dirty_flag = True
        except exception.InstanceNotFound:
    ...
    @wrap_exception()
    def trigger_discovery_task_pf9(self, context):
        """Triggers Pf9 discovery task to run outside of periodic interval."""
        self.find_orphaned_vms(context, True)

    @wrap_exception()
    def discovery_task_state_pf9(self, context):
        """Returns the state of the Pf9 discovery task (running or not)."""
        return {"running": self.is_discovery_task_running}
    ...
    @periodic_task.periodic_task(spacing=CONF.PF9.sync_network_info_interval,
                                 run_immediately=True)
    def _sync_network_info(self, context):
        """
        PF9: This periodic task collects information about the networking configuration and
            reports to the nova-network
        TODO: This might be better off placed in some nova-network related code
        TODO: optimize this further and only upload what is not present
        TODO: we can also optimize it by batching both the network and ip
        discovery info in one call
        :param context:
        :return:
        """
        ...
        for node in nodenames:
            all_discovered = self.driver.get_all_networks_pf9(node)
            # Add discovered networks to in memory cache. Only send update
            # if there's a change. Does not require a lock to access this cache.
            # This is the only method that uses and changes the cache.
            hash_val = hash(json.dumps(all_discovered, sort_keys=True, ensure_ascii=True))
            if hash_val != self._discovered_nw_cache.get(node, None):
                LOG.info("Sending network information %s for node %s", all_discovered, node)
                self.network_api.add_discovered_networks_for_node_pf9(context, self.host,
                                                     all_discovered, node)
                self._discovered_nw_cache[node] = hash_val

    @periodic_task.periodic_task(spacing=CONF.PF9.sync_ip_interval,
                                 run_immediately=True)
    def _sync_ip_info(self, context):

        # Internal function - handler for worker thread that runs asynchronously
        # to the periodic tasks. Spawned initially, and sleeps every 60 seconds,
        # so as to not get blocked by other periodic tasks
        def _sync_ip_thread(context):
            try:
                while True:
                    full_discovery = self._sync_ip_counter % CONF.PF9.sync_ip_skips == 0
                    self._sync_ip_counter += 1
                    # Sleep at top, as loop can continue iteration for partial discovery
                    eventlet.greenthread.sleep(CONF.PF9.sync_ip_interval)

                    # moderately expensive. use cache whenever possible
                    if full_discovery or self._instance_dirty_flag:
                        self._instance_dirty_flag = False
                        self._pf9_cached_instances = \
                           obj_instance.InstanceList.get_by_host(context, self.host)

                    db_uuids = [x.uuid for x in self._pf9_cached_instances]

                    if full_discovery:
                        LOG.info("Full IP discovery")
                        all_instance_info = self.driver.get_all_ip_mapping_pf9()
                    else:

                        # Partial discovery IAAS-3987. This is run in between the 10-minute full discoveries,
                        # and especially on VMWare trying to limit the burden on vCenter.

                        now = timeutils.normalize_time(timeutils.utcnow())

                        # all instances created in the last hour
                        recent = set()
                        for inst in self._pf9_cached_instances:
                           if inst.created_at is not None and inst.vm_state != 'error':
                               created_at = timeutils.normalize_time(inst.created_at)
                               age = timeutils.delta_seconds(created_at, now)
                               if age < CONF.PF9.sync_ip_recency:
                                   recent.add(inst.uuid)

                        # filter out any that have known ips
                        needed = set()
                        with self._vm_ip_cache_mutex:
                            for uuid in recent:
                                if self._known_vms_ip.has_key(uuid):
                                    ips = self._known_vms_ip[uuid]['ips']
                                    if len(ips) == 0 or ips[0]['ip_address'] is None:
                                        needed.add(uuid)
                                else:
                                    needed.add(uuid)

                        LOG.info("partial IP discovery -> recent: %s needed: %s" % (str(recent), str(needed)))

                        if len(needed) == 0 and not full_discovery:
                            continue

                        all_instance_info = self.driver.get_all_ip_mapping_pf9(needed_uuids=needed)

                    # Add instance uuid & hash of value to memory cache if not already present.
                    # Only send instance_info for the ones that have changes.
                    for instance_uuid, ips in all_instance_info.items():
                        hash_val = hash(json.dumps(ips, sort_keys=True, ensure_ascii=True))
                        if (self._known_vms_ip.has_key(instance_uuid) and
                                        self._known_vms_ip[instance_uuid]['hash'] == hash_val) or \
                                (instance_uuid not in db_uuids):
                            del all_instance_info[instance_uuid]
                        else:
                            # Add to cache/update cache
                            with self._vm_ip_cache_mutex:
                                LOG.info("Updating IP: uuid(%s) ip(%s)", instance_uuid, str(ips))
                                self._known_vms_ip[instance_uuid] = { 'hash': hash_val, 'ips': ips }
                    if all_instance_info:
                        LOG.debug("Sending ip address information %s", all_instance_info)
                        try:
                            self.network_api.add_discovered_ip_addresses(context,
                                                                         all_instance_info)
                        except exception.InstanceNotFound:
                            pass 

            finally:
                with self._sync_ip_thread_lock:
                    self._sync_ip_thread_running = False

        # First time only, spawn this function in a new thread
        # Or if thread terminated and not running (due to exception, or interval expired)
        with self._sync_ip_thread_lock:
            if not self._sync_ip_thread_running:
                LOG.info("Spawning initial thread for sync_ip_info")
                self._sync_ip_thread_running = True
                eventlet.greenthread.spawn_n(_sync_ip_thread, context)
    ...
    def _sync_instance_power_state(self, context, db_instance, vm_power_state,
        ...
        # Pf9 change (akapoor) - These instance refresh calls go between the host <-> DU everytime,
        # and cause issues at scale (1000+ VMs).
        # The changes that we've made later in the code, treating the hypervisor as
        # the source of truth and updating the DB state to reflect the hypervisor state,
        # make this useless anyway (see if check on power states below)
        ...
        if vm_state in (vm_states.BUILDING,
            ...
            # PF9: IAAS - 2160
            # Reason (pushkar): The change was already part of Havana code base
            # but was removed in Icehouse release. Without this change if VM is
            # started directly on the hypervisor nova will shut it down.
            if vm_power_state in (power_state.RUNNING,
                                  power_state.SUSPENDED,
                                  power_state.PAUSED) and \
                    db_instance.task_state is None:
                db_instance.power_state = vm_power_state
                db_instance.vm_state = vm_states.ACTIVE
                db_instance.save()
            # PF9: end
            elif vm_power_state not in (power_state.NOSTATE,
                                      power_state.SHUTDOWN,
                                      power_state.CRASHED):
    ...
    @periodic_task.periodic_task(spacing=CONF.update_resources_interval,
                                 run_immediately=True)
    def update_resource_utilization_pf9(self, context):
        nodenames = set(self.driver.get_available_nodes())

        for nodename in nodenames:
            rt = self._get_resource_tracker(nodename)
            rt.update_resource_usage_pf9(context)

    @periodic_task.periodic_task(spacing=CONF.PF9.find_orphaned_vms_interval,
                                 run_immediately=True)
    def find_orphaned_vms(self, context, force=False):
        """
        Find the VMs that are part of the hypervisor layer but not the conductor
        :param context: security context
        """
        with self._check_discovery_task_running:
            if self.is_discovery_task_running:
                LOG.debug("Discovery task is already running, doing nothing and returning")
                return
            LOG.debug("Starting discovery task")
            self.is_discovery_task_running = True
        ...
        try:
            ...
            try:
                ...
                for node in nodenames:
                    template_uuids = []
                    driver_uuids = self.driver.list_instance_uuids(
                                                    node, template_uuids, force)
                    LOG.debug('Discovered {num_uuids} Instance UUIDs for '
                              '{node}.'.format(num_uuids=len(driver_uuids),
                                           node=node))
                    LOG.debug('Discovered {num_uuids} Template UUIDs for '
                              '{node}.'.format(num_uuids=len(template_uuids),
                                           node=node))
                    driver_uuids += template_uuids

                    # For each UUID now find the corresponding information

                    with self._vm_cache_mutex:
                        for uuid in driver_uuids:
                            if uuid not in self._known_vms:
                                # PF9: Fix for IAAS - 1603
                                # PF9: Check if we are dealing with template or VM
                                # FIXME: Once template reporter has been moved to new
                                #        service do not completely remove below code.
                                #        It is still needed to avoid reporting
                                #        templates as VMs.
                                info = self.driver.get_instance_info(uuid)
                                if uuid in template_uuids:
                                    images_info.append(info)
                                    self._known_vms[uuid] = {'template': True}
                                else:
                                    # Add node information needed for associating instance
                                    # with hypervisor
                                    info['node'] = node
                                    instance_info.append(info)
                                    self._known_vms[uuid] = {'template': False, 'name_on_host': info['name']}
                            else:
                                known_attrs = self._known_vms[uuid]
                                if uuid not in template_uuids and known_attrs['template']:
                                    # delete from glance and add to VMs
                                    info = self.driver.get_instance_info(uuid)
                                    LOG.info("{0} converted from template to VM".format(uuid))
                                    template_reporter.delete_template(uuid)
                                    info['node'] = node
                                    instance_info.append(info)
                                    self._known_vms[uuid] = {'template': False, 'name_on_host': info['name']}
                                elif uuid in template_uuids and not known_attrs['template']:
                                    # delete from VMs and add to Glance
                                    info = self.driver.get_instance_info(uuid)
                                    LOG.info("{0} converted from VM to template ".format(uuid))
                                    try:
                                        inst_obj = objects.Instance.get_by_uuid(context, uuid)
                                        self._complete_partial_deletion(context, inst_obj)
                                    except exception.InstanceNotFound:
                                        LOG.debug('Instance {uuid} already removed'
                                                  ' from database.'
                                                  ' Ignoring.'.format(uuid=uuid))
                                    images_info.append(info)
                                    self._known_vms[uuid] = {'template': True, 'name_on_host': info['name']}
                                else:
                                    info = self.driver.get_instance_info(uuid)
                                    if self._known_vms[uuid]['template']:
                                        images_info.append(info)
                                        self._known_vms[uuid]['name_on_host'] = info['name']
                                    else:
                                        if 'name_on_host' in self._known_vms[uuid] and \
                                           self._known_vms[uuid]['name_on_host'] == info['name']:
                                            LOG.debug('Instance {uuid} already reported'
                                                      ' and has not changed'.format(
                                                      uuid=uuid))
                                        else:
                                            info['node'] = node
                                            instance_info.append(info)
                                            self._known_vms[uuid]['name_on_host'] = info['name']
                                            LOG.info('Instance name for {uuid} seems to have changed'
                                                     ' on the host'.format(uuid=uuid))

            except NotImplementedError:
                LOG.warning("List instance uuids not supported by the underlying driver")

            LOG.debug('Instance and template discovery complete for all nodes')
            # PF9: FIXME: To be removed alongwith template reporter
            if images_info:
                LOG.debug("Reporting templates as images - %s" % images_info)
                template_reporter.convert_and_report_template(images_info)

            if instance_info:
                LOG.debug("Calling conductor with host %s,"
                          "instance_register : %s",  self.host, str(instance_info))
                self.conductor_api.instance_register(context, self.host,
                                                 instance_info)
        except Exception as e:
            # Catching a generic exception, because if any unknown exception is thrown,
            # we won't be able to reset the discovery_task_running flag
            LOG.error('Discover task failed to complete, clearing known VMs cache. '
                    'Error was {error_msg}'.format(error_msg=e.message))
            self._clean_known_vm_cache(driver_uuids)
        finally:
            with self._check_discovery_task_running:
                LOG.debug("Discovery task complete")
                self.is_discovery_task_running = False
        ...
        with utils.temporary_mutation(context, read_deleted="yes"):
            for instance in self._running_deleted_instances(context):
                if action == "log":
                    ...
                    # PF9: begin
                    msg = ("Detected instance %s with name label "
                           "'%s' which is marked as "
                           "DELETED but still present on host %s." % (instance.uuid, instance.name, self.host))
                    self.conductor_api.log_warning_pf9(context, msg)
                    # PF9: end
