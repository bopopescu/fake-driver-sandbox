...
def add_instance_fault_from_pf9_error(context, instance, pf9_errors):
    ...
    def _form_reasons(pf9_errors):
        ...
        for host, msg in pf9_errors:
            reasons.append('Host {host} failed with error: {msg}'.format(host=host, msg=msg))
...
class StdTableOutputParser(object):
    """
    ...
    def __init__(self, data_input, delimiter=None, headers=None,
                 columns_desired=1):
        """
        ...
        self._columns_desired = columns_desired
    ...
    def get_all(self):
        ...
        ret_data = []
        column_names = []
        lines = self._data_input.split("\n")
        index = 0
        for line in lines:
            # Ignore the first line
            if index == 0:
                if not self._headers:
                    column_names = line.split(self._delimiter)
                else:
                    column_names = self._headers
                index += 1
                continue
            index += 1
            ...
            for column in columns:
                row[column_names[count]] = column
                count += 1
                if self._columns_desired > 0 and self._columns_desired == count:
                    break
