...
# Pf9 exception thrown when a template cannot be located
class TemplateNotFoundPf9(NotFound):
    msg_fmt = _("Template %(template_name)s not found "
                "for deployment.")
...
class FixedIpRangeStartInvalid(Invalid):
    msg_fmt = _("Start of IP subrange %(address)s is invalid.")


class FixedIpRangeEndInvalid(Invalid):
    msg_fmt = _("End of IP subrange %(address)s is invalid.")
...
class NetworkAccessNotFound(NotFound):
    msg_fmt = _("Network access not found for %(network_id)s / "
                "%(project_id)s combination.")

class NetworkSharedPf9(NovaException):
    msg_fmt = _('Network %(network_id)s is shared with other projects,'
                'it cannot be deleted.')

class InstanceNetworksNotAccessiblePf9(NovaException):
    msg_fmt = _("The network %(network_id)s used by %(instance_id)s is not "
                "accessible from project %(project_id)s.")

class NetworkAssociationExists(NovaException):
    msg_fmt = _('Network %(network_id)s is associated with host '
                '%(compute_id)s already.')

class NetworkAssociationNotFound(NotFound):
    msg_fmt = _('Network %(network_id)s is not associated with host '
                '%(compute_id)s.')
...
class NetworkAccessExists(NovaException):
    msg_fmt = _("Network access already exists for network %(network_id)s "
                "and project %(project_id)s combination.")
...
# PF9
class InstanceNotFoundOnHypervisorPF9(NotFound):
    msg_fmt = _('Instance %(instance_id)s could not be found on hypervisor.')
    code = 94043
