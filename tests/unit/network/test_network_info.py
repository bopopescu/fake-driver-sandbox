...
# PF9: File injection on RHEL
GUEST_OS_RHEL6 = ('linux', 'rhel', 6)
...
class NetworkInfoTests(test.NoDBTestCase):
    ...
    def _setup_injected_network_scenario(self, should_inject=True,
                                         ...
                                         guest_os=None, external_dhcp=False):
    ...
    def _test_injected_network_template_debian(self, ninfo, should_inject,
                                               use_ipv4,
                                               use_ipv6, gateway,
                                               external_dhcp):
        ...
        if not should_inject:
            ...
            if use_ipv4:
                ...
                if external_dhcp:
                    self.assertTrue('iface eth0 inet dhcp' in template)
                    self.assertTrue('address' not in template)
                    self.assertTrue('netmask' not in template)
                    self.assertTrue('broadcast' not in template)
                    self.assertTrue('gateway' not in template)
                else:
            ...
            if use_ipv6:
                if external_dhcp:
                    self.assertTrue('iface eth0 inet6 dhcp' in template)
                    self.assertTrue('address' not in template)
                    self.assertTrue('netmask' not in template)
                    self.assertTrue('gateway' not in template)
                else:
...
# PF9: Begin functions
    def test_injection_static_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             external_dhcp=True)

    def test_injection_static_nogateway_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             gateway=False,
                                             external_dhcp=True)

    def test_injection_static_ipv6_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             use_ipv6=True,
                                             external_dhcp=True)

    def test_injection_static_ipv6_nogateway_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             use_ipv6=True,
                                             gateway=False,
                                             external_dhcp=True)

    def test_injection_static_with_ipv4_off_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             use_ipv4=False,
                                             external_dhcp=True)

    def test_injection_static_ipv6_with_ipv4_off_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             use_ipv6=True,
                                             use_ipv4=False,
                                             external_dhcp=True)

    def test_injection_static_ipv6_nogateway_with_ipv4_off_dhcp(self):
        self._test_injected_network_template(should_inject=True,
                                             use_ipv6=True,
                                             use_ipv4=False,
                                             gateway=False,
    ...
    def test_injection_static_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True)

    def test_injection_static_nogateway_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True, gateway=False)

    def test_injection_static_ipv6_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True, use_ipv6=True)

    def test_injection_static_ipv6_nogateway_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             use_ipv6=True,
                                             gateway=False)

    def test_injection_static_rhel6_dhcp(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             external_dhcp=True)

    def test_injection_static_nogateway_rhel6_dhcp(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True, gateway=False,
                                             external_dhcp=True)

    def test_injection_static_ipv6_rhel6_dhcp(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True, use_ipv6=True,
                                             external_dhcp=True)

    def test_injection_static_ipv6_nogateway_rhel6_dhcp(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             use_ipv6=True,
                                             gateway=False,
    ...
    def test_injection_dynamic_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=False)

    def test_injection_dynamic_nogateway_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=False,
                                             gateway=False)

    def test_injection_static_with_ipv4_off_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             use_ipv4=False)

    def test_injection_static_ipv6_with_ipv4_off_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             use_ipv6=True,
                                             use_ipv4=False)

    def test_injection_static_ipv6_nogateway_with_ipv4_off_rhel6(self):
        self._test_injected_network_template(guest_os=GUEST_OS_RHEL6,
                                             should_inject=True,
                                             use_ipv6=True,
                                             use_ipv4=False,
    ...
    def _test_injected_network_template(self, should_inject, use_ipv4=True,
        ...
        if not guest_os:
            self.\
                _test_injected_network_template_debian(ninfo=ninfo,
        ...
        gos_str = '%s_%s_%s' % guest_os
        # The only non-debian guest OS currently supported is RHEL/CentOS 6
        self.assertTrue(gos_str in ('linux_centos_6', 'linux_rhel_6'))
        self._test_injected_network_template_rhel6(ninfo=ninfo,
                                                   should_inject=should_inject,
                                                   use_ipv4=use_ipv4,
                                                   use_ipv6=use_ipv6,
                                                   gateway=gateway,
    ...
    def _test_injected_network_template_rhel6(self, ninfo, should_inject,
                                              use_ipv4,
                                              use_ipv6, gateway,
                                              external_dhcp):

        files = netutils.get_injected_network_files(network_info=ninfo,
                                                    guest_os_info=GUEST_OS_RHEL6,
                                                    use_ipv6=use_ipv6)
        if not should_inject:
            self.assertTrue(len(files) == 0)
            return

        vifs_with_at_least_one_subnet = 0
        for vif in ninfo:
            if vif['network']['subnets']:
                vifs_with_at_least_one_subnet += 1
        if not vifs_with_at_least_one_subnet:
            self.assertEqual(0, len(files))
            return
        self.assertEqual(vifs_with_at_least_one_subnet + 1, len(files))
        path, contents = files[0]
        self.assertEqual(path, '/etc/udev/rules.d/70-persistent-net.rules')
        self.assertEqual(contents, '# This file was generated by OpenStack Nova\n\n')

        self.assertEqual(2, len(files))
        path, contents = files[1]
        self.assertTrue('DEVICE=eth0' in contents)
        self.assertTrue('ONBOOT=yes' in contents)

        boot_proto = 'BOOTPROTO=dhcp' if external_dhcp else 'BOOTPROTO=static'
        self.assertTrue(boot_proto in contents)

        if use_ipv4:
            if external_dhcp:
                self.assertTrue('IPADDR' not in contents)
                self.assertTrue('NETMASK' not in contents)
                self.assertTrue('BROADCAST' not in contents)
            else:
                self.assertTrue('IPADDR=10.10.0.2' in contents)
                self.assertTrue('NETMASK=255.255.255.0' in contents)
                self.assertTrue('BROADCAST=10.10.0.255' in contents)
                if gateway:
                    self.assertTrue('GATEWAY=10.10.0.1' in contents)
                else:
                    self.assertTrue('GATEWAY' not in contents)
                self.assertTrue('DNS1=1.2.3.4' in contents)
                self.assertTrue('DNS2=2.3.4.5' in contents)
                self.assertTrue('PEERDNS=yes' in contents)
        if use_ipv6:
            self.assertTrue('IPV6INIT=yes' in contents)
            if external_dhcp:
                self.assertTrue('DHCPV6C=yes' in contents)
                self.assertTrue('IPV6ADDR' not in contents)
                self.assertTrue('IPV6_DEFAULTGW' not in contents)
            else:
                self.assertTrue('DHCPV6C' not in contents)
                self.assertTrue('IPV6ADDR=1234:567::2/48' in contents)
                if gateway:
                    self.assertTrue('IPV6_DEFAULTGW=1234:567::1' in contents)
                else:
                    self.assertTrue('IPV6_DEFAULTGW' not in contents)

    def _test_injected_network_template_debian(self, ninfo, should_inject, use_ipv4,
                                               use_ipv6, gateway,
                                               external_dhcp):
        template = netutils.get_injected_network_template(ninfo,
                                                          use_ipv6=use_ipv6)
    ...
    def _test_injected_hostname_get_netinfo(self):
        network = fake_network_cache_model.new_network({'subnets': []})
        subnet_dict = {'gateway': None}
        network.add_subnet(
            fake_network_cache_model.new_subnet(subnet_dict))
    ...
    def test_injected_hostname_rhel6(self):
        guest_os = ('linux', 'centos', '6')
        header = "# hostname script for RHEL/CentOS\n# " \
                 "Injected by Nova on instance boot\n\n"
        ninfo = self._test_injected_hostname_get_netinfo()
        files = netutils.get_injected_network_files(network_info=ninfo,
                                                    guest_os_info=guest_os,
                                                    hostname="test-vm-1")
        expected_tuple = ('/etc/sysconfig/network',
                          "%sNETWORKING=yes\nHOSTNAME=test-vm\n\n" % header)
        self.assertIn(expected_tuple, files)

    def test_no_injected_hostname_rhel6(self):
        guest_os = ('linux', 'centos', '6')
        header = "# hostname script for RHEL/CentOS\n# " \
                 "Injected by Nova on instance boot\n\n"
        # The only non-debian guest OS currently supported is RHEL/CentOS 6
        ...
        expected_tuple = ('/etc/sysconfig/network',
                          "%sNETWORKING=yes\nHOSTNAME=test-vm\n\n" % header)
        self.assertNotIn(expected_tuple, files)

    def test_injected_hostname_debian(self):
        ninfo = self._test_injected_hostname_get_netinfo()
        files = netutils.get_injected_network_files(network_info=ninfo,
                                                    hostname="test-vm-1")
        expected_tuple = ('/etc/hostname', 'test-vm')
        self.assertIn(expected_tuple, files)

    def test_no_injected_hostname_debian(self):
        ninfo = self._test_injected_hostname_get_netinfo()
        files = netutils.get_injected_network_files(network_info=ninfo)
        expected_tuple = ('/etc/hostname', 'test-vm')
        self.assertNotIn(expected_tuple, files)
