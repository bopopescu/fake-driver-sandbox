...
class UpdateNetworkTestCasePF9(test.TestCase):

    REQUIRES_LOCKING = True

    def setUp(self):
        super(UpdateNetworkTestCasePF9, self).setUp()
        self.useFixture(test.SampleNetworks())
    ...
    def _run_updates(self, nets):
        net_uuid = nets[1]['uuid']
        # Update two non-CIDR fields
        self.network.update_network(self.context, net_uuid,
                                    label='modified_test1',
                                    dns1='1.2.3.4')
        net = db.network_get(self.context, nets[1]['id'])
        self.assertEqual('modified_test1', net['label'])
        self.assertEqual('1.2.3.4', net['dns1'])
        # Ensure IP addresses still there
        ips = db.fixed_ip_get_all(self.context)
        for ip in ips:
            self.assertFalse(ip['deleted'])

        # Cause a change in CIDR, resulting in IP reallocation
        self.network.update_network(self.context, net_uuid,
                                    gateway='4.3.2.1',
                                    cidr='192.168.1.0/28')
        net = db.network_get_by_uuid(self.context, net_uuid)
        self.assertEqual('4.3.2.1', net['gateway'])
        # Ensure other fields did not change
        self.assertEqual('modified_test1', net['label'])
        self.assertEqual('1.2.3.4', net['dns1'])
        ips = db.fixed_ip_get_all(self.context)
        # Verify there are 16 more addresses
        self.assertEqual(32, len(ips))
        # The old ones belonging to the old subnet are marked as deleted
        for i in range(8, 16):
            self.assertTrue(ips[i]['deleted'])
        # Verify the new ones
        ip_net = netaddr.IPNetwork('192.168.1.0/28')
        for i in range(16, 32):
            self.assertEqual(str(ip_net[i - 16]), ips[i].address)
            self.assertEqual(net['id'], ips[i].network_id)

        # Set CIDR to empty
        # PF9: This does not work starting Juno. Empty CIDR is not valid
        # DB field
        """
        self.network.update_network(self.context, net_uuid, cidr='',
                                    label='foobar')
        net = db.network_get_by_uuid(self.context, net_uuid)
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(32, len(ips))
        ...
        for i in range(16, 32):
            self.assertTrue(ips[i]['deleted'])
        self.assertEqual('foobar', net['label'])
        # Ensure other fields did not change, except for gateway
        self.assertEqual('1.2.3.4', net['dns1'])
        self.assertEqual(None, net['gateway'])
        """
        # New IP range
        self.network.update_network(self.context, net_uuid,
                                    cidr='192.168.5.0/29',
                                    gateway='192.168.5.1')
        net = db.network_get_by_uuid(self.context, net_uuid)
        self.assertEqual('192.168.5.1', net['gateway'])
        self.assertEqual('1.2.3.4', net['dns1'])
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(40, len(ips))
        ip_net = netaddr.IPNetwork('192.168.5.0/29')
        for i in range(32, 40):
            self.assertEqual(str(ip_net[i - 32]), ips[i].address)
            self.assertEqual(net['id'], ips[i].network_id)

    def test_update_network(self):
        nets = db.network_get_all(self.context)
        self.assertEqual(2, len(nets))
        self.assertEqual('test_0', nets[0]['label'])
        self.assertEqual('test_1', nets[1]['label'])
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(16, len(ips))

        self._run_updates(nets)


    def test_network_update_with_sharing_pf9(self):
        """
        Test to verify that network sharing functionality does not
        affect network updates
        """
        nets = db.network_get_all(self.context)
        self.assertEqual(2, len(nets))
        self.assertEqual('test_0', nets[0]['label'])
        ...
        expected = [['proj1', 'proj2'], 'proj1']

        for i in range(0, 2):
            for proj in expected[i]:
                db.network_add_access_pf9(self.context, nets[i]['uuid'], proj)

        for i in range(0, 2):
            observed = db.network_get_access_pf9(self.context,
                                                 nets[i]['uuid'])
            self.assertEqual(sorted(expected[i]),
                             sorted(observed))
...
class NetworkSharingPf9TestCase(test.TestCase):

    REQUIRES_LOCKING = True

    def setUp(self):
        super(NetworkSharingPf9TestCase, self).setUp()
        self.useFixture(test.SampleNetworks())
        self.network = self.start_service('network')
        self.user_id = 'fake'
        ...
        self.context = context.RequestContext(self.user_id,
                                              self.project_id,
                                              is_admin=False)
        self.admin_id = 'fake-admin'
        self.admin_project_id = 'fake-admin-project'
        self.admin_context = context.RequestContext(self.admin_id,
                                                    self.admin_project_id,
                                                    is_admin=True)
        for net in db.network_get_all(self.admin_context):
            db.network_associate(self.admin_context,
                                 self.admin_project_id,
                                 network_id=net['id'],
                                 force=True)
    ...
    def test_network_sharing(self):
        ...
        nets = db.network_get_all(self.admin_context)
        self.assertEquals(2, len(nets))

        for net in nets:
            db.network_add_access_pf9(self.admin_context,
                                      net['uuid'],
                                      self.project_id)
...
class SubrangeTestCasePF9(test.TestCase):
    def setUp(self):
        super(SubrangeTestCasePF9, self).setUp()
        self.useFixture(test.SampleNetworks())

        # PF9 change
        self.network = self.start_service('network',
                                          manager='nova.network.manager.FlatManager')
        self.user_id = 'fake'
        self.project_id = 'fake'
        self.context = context.RequestContext(self.user_id,
                                              self.project_id,
    ...
    def test_update_subrange(self):
        ...
        net = nets[1]
        net_uuid = net['uuid']
        # Verify default subrange values
        self.assertEqual('10.0.0.11', net['range_start'])
        self.assertEqual('10.0.0.14', net['range_end'])

        # Specify invalid subrange
        self.assertRaises(exception.FixedIpRangeStartInvalid,
                          self.network.update_network,
                          self.context, net_uuid, range_start='10.0.0.254')
        self.assertRaises(exception.FixedIpRangeEndInvalid,
                          self.network.update_network,
                          self.context, net_uuid, range_end='10.0.0.2')

        # This should be a no-op
        self.network.update_network(self.context, net_uuid,
                                    range_start='10.0.0.11')
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(16, len(ips))

        # Shrink subrange to 3 IPs, should cause 1 IP to be deleted.
        # Also change the network label.
        self.network.update_network(self.context, net_uuid,
                                    label='foo',
                                    range_start='10.0.0.12')
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(16, len(ips))
        for i in range(8,16):
            self.assertEqual(i == 11, ips[i]['deleted'] > 0)
        net = db.network_get(self.context, net['id'])
        self.assertEqual(net['range_start'], '10.0.0.12')
        self.assertEqual(net['label'], 'foo')

        # Shrink subrange to 2 IPs, should cause another IP to be deleted
        self.network.update_network(self.context, net_uuid,
                                    range_end='10.0.0.13')
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(16, len(ips))
        for i in range(8, 16):
            self.assertEqual(i in (11, 14), ips[i]['deleted'] > 0)
        net = db.network_get(self.context, net['id'])
        self.assertEqual(net['range_end'], '10.0.0.13')

        # Expand back to 4 IPs, should create 2 new rows in the FixedIps table
        self.network.update_network(self.context, net_uuid,
                                    range_start='10.0.0.11',
                                    range_end='10.0.0.14')
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(18, len(ips))

        # verifies sameness of before-and-after for array of fixed ips
        def check_ip_tables_match(old_ips, new_ips, active_only):
            if active_only:
                old_ips = [entry for entry in old_ips if entry['deleted'] == 0]
                new_ips = [entry for entry in new_ips if entry['deleted'] == 0]
                # deletions will have changed the order of the tables
                old_ips = sorted(old_ips, key=lambda k: k['address'])
                new_ips = sorted(new_ips, key=lambda k: k['address'])
            self.assertEqual(len(old_ips), len(new_ips))
            for i in range(len(new_ips)):
                if active_only and new_ips[i]['deleted'] != 0:
                    continue
                for field in ['address', 'deleted', 'reserved']:
                    assert(new_ips[i][field] == old_ips[i][field])

        orig_ips = ips
        # Specify an invalid DNS server while changing range. It should not
        # extend/contract the fixed_ips table on this failure. IAAS-3868
        self.assertRaises(ValueError,
                          self.network.update_network,
                          self.context, net_uuid,
                          range_start='10.0.0.12',
                          range_end='10.0.0.14',
                          dns1='300.300.300.300')
        ips = db.fixed_ip_get_all(self.context)
        check_ip_tables_match(orig_ips, ips, False)


        # test failure of network write
        from nova.db.sqlalchemy import api as sqlalchemy_api
        orig_network_update = sqlalchemy_api.network_update

        class FakeNetworkUpdateError(RuntimeError):
            pass

        def fake_network_update(context, network_id, values):
            raise FakeNetworkUpdateError()

        # monkeypatch
        sqlalchemy_api.network_update = fake_network_update

        orig_ips = ips
        self.assertRaises(FakeNetworkUpdateError,
                          self.network.update_network,
                          self.context, net_uuid,
                          range_start='10.0.0.12',
                          range_end='10.0.0.14',
        ...
        # undo monkey
        sqlalchemy_api.network_update = orig_network_update

        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(19, len(ips))
        self.assertEqual(ips[18].deleted, 0)
        self.assertEqual(ips[18].address, '10.0.0.11')
        # though changed, the table must contain the same state as before
        check_ip_tables_match(orig_ips, ips, True)

        return

    def test_discovered_ip_with_reassignment(self):
        """
        PF9 test case
        Related Issue - IAAS-3488 and IAAS-3508
        """
        self.conductor = self.start_service(
            'conductor', manager=CONF.conductor.manager)
        self.compute = self.start_service('compute')
        self.network.add_discovered_networks(self.context,
                                             self.compute.host,
                                             [{'bridge': 'testbr0'}])
        # Create an instance in error state
        inst1 = db.instance_create(self.context, {'host': self.compute.host,
                                                  'display_name': HOST,
                                                  'instance_type_id': 1,
                                                  'vm_state': 'error'})
        inst2 = db.instance_create(self.context, {'host': self.compute.host,
                                                  'display_name': HOST,
                                                  'instance_type_id': 2})
        nets = db.network_get_all(self.context)
        for network in nets:
        ...
        # Insert one 'discovered' IP address
        instance_1_ip_info = {
            inst1['uuid']: [{
                'mac_address': '00:50:56:A9:0B:BD',
        ...
        instance_2_ip_info = {
            inst2['uuid']: [{
                'mac_address': '00:50:56:A9:0B:BE',
                'ip_address': ip_address,
                'bridge': 'testbr0'
            }]
        }
        self.network.add_discovered_ip_addresses(self.context,
                                                 instance_1_ip_info)
        ip_info_list = self.network.get_instance_uuids_by_ip_filter(
            self.context, {'ip': ip_address})
        uuid_found = False
        for ip_info in ip_info_list:
            if ip_info['instance_uuid'] == inst1['uuid']:
                uuid_found = True
                self.assertEqual(str(ip_info['ip']), ip_address)
        self.assertIs(True, uuid_found,
                      "instance 1 was not found in list of instance associated "
                      "with 192.168.1.52. list - {ip_list}".format(
                          ip_list=ip_info_list))
        self.network.add_discovered_ip_addresses(self.context,
                                                 instance_2_ip_info)
        ip_info_list = self.network.get_instance_uuids_by_ip_filter(
            self.context, {'ip': ip_address})
        uuid_found = False
        for ip_info in ip_info_list:
            if ip_info['instance_uuid'] == inst2['uuid']:
                uuid_found = True
                self.assertEqual(str(ip_info['ip']), ip_address)
        self.assertIs(True, uuid_found,
                      "instance 2 was not found in list of instance associated "
                      "with 192.168.1.52. list - {ip_list}".format(
                          ip_list=ip_info_list))

    def test_with_discovered_ip(self):
        """
        PF9 test case
        """
        self.conductor = self.start_service(
        ...
        self.network.add_discovered_ip_addresses(self.context,
                                                 instance_ip_info)
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(17, len(ips))

        # Update some settings while the CIDR is still unset.
        # The range_ settings should be ignored.
        self.network.update_network(self.context, net_uuid,
                                    label='discovered_net',
                                    description='Discovered Net',
                                    range_start='192.168.5.100',
                                    range_end='192.168.5.200')
        net = db.network_get(self.context, net_id)
        self.assertEqual('discovered_net', net['label'])
        self.assertEqual('Discovered Net', net['description'])
        self.assertEqual(None, net['range_start'])
        self.assertEqual(None, net['range_end'])

        # Assigning a CIDR without specifying a subrange fails due to conflict
        self.assertRaises(exception.FixedIpAlreadyInUse,
                          self.network.update_network,
                          self.context, net_uuid,
                          label='my_test_network',
                          cidr='192.168.1.0/24')
        net = db.network_get(self.context, net_id)
        self.assertNotEqual('my_test_network', net['label'])

        # This time, specify a subrange that does not overlap with discovered IP
        ...
        self.network.update_network(self.context, net_uuid,
                                    ...
                                    range_end=range_end)
        net = db.network_get(self.context, net_id)
        self.assertEquals('my_test_network', net['label'])

        # Num created IPs = 2 bottom reserved + 1 top reserved + 30 for subrange
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(17 + 33, len(ips))
        base_addr = netaddr.IPAddress(range_start)
        for i in range(30):
            self.assertEqual(str(base_addr + i), ips[19 + i]['address'])

        # Create 2 additional instances attached to the network
        project_id = self.context.project_id
        requested_networks = [(net_uuid, None)]
        insts = []
        nw_info = []
        range_start_addr = netaddr.IPAddress(range_start)
        for i in range(2):
            insts.append(db.instance_create(self.context,
                                            {'host': self.compute.host,
                                             'display_name': 'inst-%s' % i,
                                             'instance_type_id': 1}))
            inst = insts[i]
            nw_info.append(self.network.allocate_for_instance(self.context,
                instance_id=inst['id'], instance_uuid=inst['uuid'],
                host=inst['host'], vpn=None, rxtx_factor=3,
                requested_networks=requested_networks,
                project_id=project_id, macs=None))
            self.assertEquals(1, len(nw_info[i]))
            fixed_ip = nw_info[i].fixed_ips()[0]['address']
            self.assertTrue(netutils.is_valid_ipv4(fixed_ip))
            self.assertEqual(str(range_start_addr + i), fixed_ip)

        # Shift the range forward by 1 IP address and expanding it by 10.
        # Should fail due to 192.168.1.10 being in use
        self.assertRaises(exception.FixedIpAlreadyInUse,
                          self.network.update_network,
                          self.context, net_uuid,
        ...
        # Deallocate network for first instance, then try again.
        # Should succeed this time.
        self.network.deallocate_for_instance(self.context,
                                             instance_id=insts[0]['id'])
        self.network.update_network(self.context, net_uuid,
                                    range_start='192.168.1.11',
                                    range_end='192.168.1.50')

        # There should be 11 more IPs. Also verify one got marked as deleted.
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(17 + 33 + 11, len(ips))
        self.assertTrue(ips[19]['deleted'] > 0)

        # Expand the range by another 10 IPs. Should fail due to discovered IP.
        self.assertRaises(exception.FixedIpAlreadyInUse,
                          self.network.update_network,
                          self.context, net_uuid,
                          range_end='192.168.1.60')

        # Deallocate network for the instance with the discovered IP.
        # Expanding the range should now succeed.
        self.network.deallocate_for_instance(self.context,
                                             instance_id=inst1['id'])
        self.network.update_network(self.context, net_uuid,
                                    range_end='192.168.1.60')
        # Verify there are 10 more IPs, and discovered IP got marked as deleted.
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual(17 + 33 + 11 + 10, len(ips))
        self.assertTrue(ips[16]['deleted'] > 0)

        # Reset network to DHCP. Should fail due to instance at 192.168.1.11
        self.assertRaises(exception.FixedIpAlreadyInUse,
                          self.network.update_network,
                          self.context, net_uuid,
                          cidr='')

        # Deallocate network for instance, then try again.
        # Should succeed this time.
        # PF9: FIXME Setting empty CIDR does not work starting Juno
        """
        self.network.deallocate_for_instance(self.context,
                                             instance_id=insts[1]['id'])
        self.network.update_network(self.context, net_uuid,
                                    cidr='')
        net = db.network_get(self.context, net_id)
        self.assertEqual('', net['cidr'])
        self.assertEqual(None, net['range_start'])
        self.assertEqual(None, net['range_end'])
        self.assertEqual(None, net['gateway'])
        ips = db.fixed_ip_get_all(self.context)
        ...
        # Assign a new 6-bit subnet and a subrange
        self.network.update_network(self.context, net_uuid,
                                    cidr='10.10.50.0/26',
                                    label='10.10.50.0_net',
                                    description='10.10.50.0 Network',
                                    range_start='10.10.50.40',
                                    range_end='10.10.50.49')
        net = db.network_get(self.context, net_id)
        ips = db.fixed_ip_get_all(self.context)
        self.assertEqual('10.10.50.63', net['broadcast'])
        self.assertEqual('10.10.50.0 Network', net['description'])
        base = 17 + 33 + 11 + 10
        # There are 13 (2 bottom reserved + 1 top reserved + 10 usable) new IPs
        self.assertEqual(base + 13, len(ips))
        for i in range(13):
            self.assertEqual(i in (0, 1, 12), ips[base + i]['reserved'])

        # Double the size of the network, without specifying a new subrange.
        # The new network is a superset of the old one.
        # Verify that one previously 'reserved' IP is no longer reserved.
        self.network.update_network(self.context, net_uuid,
                                    cidr='10.10.50.0/25')
        ips = db.fixed_ip_get_all(self.context)
        # There should be 128 - 14 + 1 new addresses
        # (+1 because the old entry with 10.10.10.63 was soft-deleted because
        # it had 'reserved' flag set, while 10.10.10.63 in the new network is
        # not reserved, and was therefore re-created)
        self.assertEqual(base + 14 + 128 - 14 + 1, len(ips))
        self.assertEqual('10.10.50.63', ips[base + 12]['address'])
        self.assertTrue(ips[base + 12]['reserved'])
        self.assertNotEqual(0, ips[base + 12]['deleted'])
        self.assertEqual('10.10.50.63', ips[base + 64]['address'])
        self.assertFalse(ips[base + 64]['reserved'])
        self.assertEqual(0, ips[base + 64]['deleted'])
        """

    def test_instance_network_mapping(self):
        # PF9 test
        ...
        self.network.add_discovered_ip_addresses(self.context,
                                                 instance_ip_info)
        mapped_nets = db.network_get_all_by_instance_pf9(self.context,
                                                         inst1['id'])
        self.assertTrue(mapped_nets)
        self.assertEqual(net_id, mapped_nets[0]['network_id'])
        self.assertRaises(exception.NetworkInUse,
                          db.network_delete_safe,
                          self.context, net_id)
