    ...
    def _stub_allocate_for_instance(self, net_idx=1, **kwargs):
        ...
        for request in ordered_networks:
            ...
            if request.port_id:
                ...
                # PF9 Begin - set instance name as DNS name in port for VM DNS name resolution
                port_req_body['port']['dns_name'] = self.instance.hostname
                # PF9 End
                if macs:
...
class TestNeutronv2(TestNeutronv2Base):
        ...
        for network in self.nets2:
            ...
            port_req_body = {
                'port': {
                    ...
                    'admin_state_up': True,
                    'tenant_id': self.instance.project_id,
                    # PF9 Begin - set hostname in port create request for DNS VM name resolution
    ...
    def test_validate_networks_not_specified(self):
        # PF9 Begin - IAAS 5415
        # validate_networks used to fail with AmbiguousNetworks exception if requested_networks was None
        # and the list of available networks from neutron net-list was > 1
        # We have changed behavior to always return immediately if requested is 0 so we don't
        # throw an exception, regardless of the available_networks Neutron reports
        requested_networks = []
        self.mox.ReplayAll()
