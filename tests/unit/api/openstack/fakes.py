...
def stub_out_instance_quota(stubs, allowed, quota, resource='instances'):
    def fake_reserve(context, **deltas):
        ...
        if requested > allowed:
            # PF9: add root_gb to support storage quotas
            quotas = dict(instances=1, cores=1, ram=1, root_gb=1)
            ...
            usages = dict(instances=dict(in_use=0, reserved=0),
                          ...
                          # PF9: add root_gb to support storage quota
                          root_gb=dict(in_use=0, reserved=0))
...
# Pf9 change - backporting fix for OpenStack bug 1359808
