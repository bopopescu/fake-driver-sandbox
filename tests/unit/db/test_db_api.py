...
class InstanceTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def _create_network_access(self, network_id, project_id):
        return db.network_add_access_pf9(self.ctxt, network_id, project_id)

    def _create_network(self, values):
        vals = self._get_base_values()
        vals.update(values)
        return db.network_create_safe(self.ctxt, values)
        # Pf9 end
    ...
    def test_instance_update_ownership_pf9(self):
        projects = ['prj1', 'prj2']
        users = ['user1', 'user2']
        inst = self.create_instance_with_args(project_id=projects[0],
                                              user_id=users[0])

        net = self._create_network(dict(project_id=projects[0]))

        db.instance_associate_networks_pf9(self.ctxt, inst['id'],
                                           [net['id']])
        self.assertRaises(exception.InstanceNetworksNotAccessiblePf9,
                          db.instance_update,
                          self.ctxt, inst['uuid'],
                          dict(project_id=projects[1]))
        db.network_add_access_pf9(self.ctxt, net['uuid'], projects[1])
        db.instance_update(self.ctxt, inst['uuid'],
                           dict(project_id=projects[1]))
...
class FixedIPTestCase(BaseInstanceTypeTestCase):
    ...
    def test_fixed_ip_delete_by_network(self):
        """
        PF9 function
        :return:
        """
        ...
        'reserved': True, 'deleted': 0, 'leased': True,
        'host': '192.168.133.1', 'address': '10.0.0.2',
        'allocated': True, 'instance_uuid': None,
        'network_id': network_id, 'virtual_interface_id': None,
        }
        param_2 = {
        'reserved': True, 'deleted': 0, 'leased': True,
        'host': '192.168.133.1', 'address': '10.0.0.3',
        'allocated': True, 'instance_uuid': None,
        'network_id': network_id, 'virtual_interface_id': None,
        }
        ...
        param_3 = {
        'reserved': True, 'deleted': 0, 'leased': True,
        'host': '192.168.133.1', 'address': '192.168.0.1',
        'allocated': True, 'instance_uuid': None,
        'network_id': network_id_2, 'virtual_interface_id': None,
        }
        fixed_ip_addrs = [db.fixed_ip_create(self.ctxt, param)['address']
                          for param in [param_1, param_2, param_3]]

        db.network_delete_fixed_ips(self.ctxt, network_id)
        for addr in fixed_ip_addrs[:2]:
            self.assertRaises(exception.FixedIpNotFoundForAddress,
                              db.fixed_ip_get_by_address, self.ctxt, addr)

        self.assertEqual(param_3['address'],
                         db.fixed_ip_get_by_address(self.ctxt, '192.168.0.1')['address'])


        db.network_delete_fixed_ips(self.ctxt, network_id_2)
        self.assertRaises(exception.FixedIpNotFoundForAddress,
                         db.fixed_ip_get_by_address, self.ctxt, '192.168.0.1')
...
class VirtualInterfaceTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def test_virtual_interface_delete(self):
        ...
        self.assertIsNotNone(vif['id'])
        vif_uuid = vif['uuid']
        db.virtual_interface_delete(self.ctxt, vif['id'])
        self.assertIsNone(db.virtual_interface_get_by_uuid(self.ctxt, vif_uuid))
...
class QuotaTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def test_quota_reserve_all_resources(self):
        ...
        usages = {'instances': 3, 'cores': 6, 'ram': 9, 'root_gb': 12} # PF9 change
        instances = []
        for i in range(3):
            instances.append(db.instance_create(self.ctxt,
                             {'vcpus': 2, 'memory_mb': 3, 'root_gb': 4, # PF9 change
                             'project_id': 'project1'}))
...
class ComputeNodeTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def setUp(self):
        ...
        self.pf9_ip_info = [dict(interface_name='eth0', ip='4.3.2.1')]
        self.pf9_stat = dict(net_sent_mib='22', disk_total_gb='600',
                             disk_used_gb='45', net_recvd_mb='44',
                             net_recvd_pkts_k='45000', net_sent_pkts_k='6300',
                             cpu_util_percent='23.33', mem_util_percent='55')
        self.pf9_networks = []
        self.compute_node_dict['pf9_ip_info'] = self.pf9_ip_info
        self.compute_node_dict['pf9_stat'] = self.pf9_stat
        self.compute_node_dict['stats'] = jsonutils.dumps(self.stats)
        self.flags(reserved_host_memory_mb=0)
        self.flags(reserved_host_disk_mb=0)
        # PF9 change: creating a copy of dict object since pf9 fields would contain db models after compute_node_create call
        self.item = db.compute_node_create(self.ctxt, self.compute_node_dict.copy())

    def _host_ips_as_dict(self, ips):
        d = {}
        ip = ips[0]
        d[ip['interface_name']] = ip['ip']
    ...
    def test_compute_node_get_all(self):
        ...
        self._assertEqualObjects(self.compute_node_dict, node,
                                 ignored_keys=self._ignored_keys +
                                              ['stats', 'service',
                                               # PF9 change- add ip_info, ext,
                                               # pf9_stat and pf9_networks
                                               'pf9_ip_info', 'ext',
                                               'pf9_stat', 'pf9_networks'])
    ...
    def test_compute_node_get_all_deleted_compute_node(self):
        ...
        for x in range(2, 5):
            ...
            compute_node_data['pf9_ip_info'] = [self.pf9_ip_info[0].copy()]
            compute_node_data['pf9_stat'] = self.pf9_stat
            ...
            # compute node stats, services and pf9 tables are wiped in a cascaded fashion
            db.compute_node_delete(self.ctxt, node['id'])

            # Clean up the service
            # PF9: We clean up service after node delete
            # db.service_destroy(self.ctxt, service['id'])


    def pf9_convert_db_obj_attr_to_dict(self, node):
            # PF9: Convert db objects to dict form
            pf9_ip_info = node['pf9_ip_info']
            node_ips = []
            for ip in pf9_ip_info:
                dict_ip = dict(ip)
                del dict_ip['ip_info']
                node_ips.append(dict_ip)
            node['pf9_ip_info'] = node_ips

            pf9_stat = node['pf9_stat']
            node_stats = []
            for stat in pf9_stat:
                dict_stat = dict(stat)
                del dict_stat['pf9_stat']
                node_stats.append(dict_stat)
            node['pf9_stat'] = node_stats
    ...
    def test_compute_node_get_all_mult_compute_nodes_one_service_entry(self):
        ...
        self.pf9_convert_db_obj_attr_to_dict(existing_node)
        ...
        for name in ['bm_node1', 'bm_node2']:
            ...
            # PF9 change - openstack master has this change. hypervisor_hostname
            # is different for the two compute nodes inserted.
            compute_node_data['hypervisor_hostname'] = name
            node = db.compute_node_create(self.ctxt, compute_node_data)

            node = dict(node)
            self.pf9_convert_db_obj_attr_to_dict(node)
            expected.append(node)

        result = sorted(db.compute_node_get_all(self.ctxt),
                        key=lambda n: n['hypervisor_hostname'])

        self._assertEqualListsOfObjects(expected, result, ignored_keys=['stats', 'pf9_networks'])
    ...
    def test_compute_node_get_all_by_host_with_distinct_hosts(self):
        ...
        self._assertEqualListsOfObjects([self.item], result, ignored_keys=['pf9_stat', 'pf9_ip_info'])
        result = db.compute_node_get_all_by_host(self.ctxt, 'host2', False)
        self._assertEqualListsOfObjects([node], result, ignored_keys=['pf9_stat', 'pf9_ip_info'])
    ...
    def test_compute_node_get_all_by_host_with_same_host(self):
        ...
        compute_node_same_host['pf9_ip_info'] = self.pf9_ip_info
        compute_node_same_host['pf9_stat'] = self.pf9_stat
        ...
        self._assertEqualListsOfObjects(expected, result,
                                        ignored_keys=['stats', 'pf9_ip_info', 'pf9_stat'])
    ...
    def test_compute_node_get_by_host_and_nodename(self):
        ...
        self._assertEqualObjects(expected, result,
                                 ignored_keys = ['pf9_ip_info', 'pf9_stat'])
    ...
    def test_compute_node_get(self):
        ...
        self._assertEqualObjects(self.compute_node_dict, node,
                        ignored_keys=self._ignored_keys + ['stats', 'service',
                                                           'pf9_ip_info', 'ext',
                                                           'pf9_stat', 'pf9_networks']) # pf9 change
        new_stats = jsonutils.loads(node['stats'])
        self.assertEqual(self.stats, new_stats)

    def _conv_compute_node_ip_info_to_dict(self, compute_node_ip_info_list):
        # Pf9 function
        ret = []
        for compute_node_ip_info in compute_node_ip_info_list:
            ret.append({'interface_name': compute_node_ip_info.interface_name,
                        'ip': compute_node_ip_info.ip})
        return ret
    ...
    def test_compute_node_update(self):
        ...
        # Pf9: Add ip info
        ip_info = [{'interface_name': u'virbr0', 'ip': u'192.168.1.12'},
            {'interface_name': u'eth0', 'ip': u'10.4.0.3'}]

        values = {
            'vcpus': 4,
            'stats': jsonutils.dumps(stats),
            'pf9_ip_info': ip_info # PF9 change
        }
        ...
        # PF9 Compare ip_info lists, sort by interface name before comparing
        self.assertEqual(sorted(ip_info, key=lambda x: x['interface_name']),
                         sorted(self._conv_compute_node_ip_info_to_dict(
                             item_updated.pf9_ip_info), key=lambda x: x['interface_name']))
...
class ArchiveTestCase(test.TestCase):
    ...
    def test_shadow_tables(self):
        ...
        for table_name in metadata.tables:
            ...
            if table_name.startswith("dump_") or table_name == 'sqlite_sequence':
                continue
...
class NetworkAccessPf9TestCase(test.TestCase, ModelsObjectComparatorMixin):
    def setUp(self):
        super(NetworkAccessPf9TestCase, self).setUp()
        self.ctxt = context.get_admin_context()
        self.counter = 0
    ...
    def _create_instance_with_args(self, project_id, **kwargs):
        args = {'reservation_id': 'a', 'image_ref': 1, 'host': 'host1',
                'node': 'node1', 'project_id': project_id,
                'vm_state': 'fake'}
        if 'context' in kwargs:
            ctxt = kwargs.pop('context')
            args['project_id'] = ctxt.project_id
    ...
    def test_get_network_access_pf9(self):
        access_list = {uuidutils.generate_uuid(): ['proj1', 'proj2'],
                       uuidutils.generate_uuid(): ['proj2']}

        for k in access_list.keys():
            net = self._create_network(values={'uuid': k})

            for proj in access_list[k]:
                self._create_network_access(net['uuid'], proj)


        for k in access_list.keys():
            ret = db.network_get_access_pf9(self.ctxt, k)
            self.assertEqual(access_list[k], ret)

    def test_add_network_access_pf9(self):
        access_list = (uuidutils.generate_uuid(), 'proj1')

        net = self._create_network(values={'uuid': access_list[0]})
        self._create_network_access(net['uuid'], access_list[1])

        new_proj = 'proj2'
        db.network_add_access_pf9(self.ctxt, access_list[0], new_proj)
        proj_list = db.network_get_access_pf9(self.ctxt, access_list[0])
        self.assertEqual(sorted([new_proj, access_list[1]]),
                         sorted(proj_list))

    def test_remove_network_access_pf9(self):
         access_list = {uuidutils.generate_uuid(): ['proj1', 'proj2'],
                        uuidutils.generate_uuid(): ['proj2']}
         ...
         proj_to_remove = 'proj2'
         for k in access_list.keys():
             db.network_remove_access_pf9(self.ctxt, k,
                                          proj_to_remove)
             proj = db.network_get_access_pf9(self.ctxt, k)
             self.assertNotIn(proj_to_remove, proj)

    def test_remove_network_access_with_instances(self):
        access_list = {uuidutils.generate_uuid(): ['proj1', 'proj2'],
                        uuidutils.generate_uuid(): ['proj2']}

        nets = []
        inst = self._create_instance_with_args('proj2')
        for k in access_list.keys():
            net = self._create_network(values={'uuid': k})
        ...
        db.instance_associate_networks_pf9(self.ctxt,
                                           inst['id'],
                                           [n['id'] for n in nets])
        for n in access_list:
            self.assertRaises(exception.NetworkInUse,
                              db.network_remove_access_pf9,
                              self.ctxt, n, 'proj2')


class HostNetworkPf9TestCase(test.TestCase, ModelsObjectComparatorMixin):
    def setUp(self):
        super(HostNetworkPf9TestCase, self).setUp()
        self.ctxt = context.get_admin_context()
        self.counter = 0

    def _get_service_base_values(self):
        return {
    ...
    def _create_service(self, values):
        v = self._get_service_base_values()
        v.update(values)
        return db.service_create(self.ctxt, v)
    ...
    def test_host_to_network_mapping(self):
        uuids = [uuidutils.generate_uuid() for i in range(0, 2)]
        networks = []
        for n in uuids:
            networks.append(self._create_network(values={'uuid': n}))
        ...
        for n in networks:
            db.compute_node_associate_network_pf9(self.ctxt, compute['id'],
                                                  n['id'])

        ret_networks = db.compute_node_get_all_networks_pf9(self.ctxt,
                                                       compute['id'])
        self.assertEquals(sorted([n['id'] for n in ret_networks]),
                          sorted([n['id'] for n in networks]))

        for n in networks:
            db.compute_node_disassociate_network_pf9(self.ctxt, compute['id'],
                                                     n['id'])

        ret_networks = db.compute_node_get_all_networks_pf9(self.ctxt,
                                                       compute['id'])
        self.assertFalse(ret_networks)
    ...
    def _create_compute_node(self):
        ...
        compute_values = dict(vcpus=2, memory_mb=1024, local_gb=2048,
                              ...
                              running_vms=0, current_workload=0,
                              service_id=service['id'])
        # PF9: OpenStack no longer associates compute node and service based on
        # ID but on host. Unsure why unit tests have not moved to that model.
        compute_values['host'] = service.host
        compute = db.compute_node_create(self.ctxt, compute_values)

        return compute, service

    def test_network_creation_with_host_mapping(self):
        nw_names = ['bridge-{id}'.format(id=id) for id in range(0, 3)]

        compute, service = self._create_compute_node()

        networks = []

        for name in nw_names:
            values = {'label': 'Network-{n}'.format(n=name),
                      'bridge': name}
            networks.append(
                db.network_create_safe(self.ctxt, values))
        for n in networks:
            db.compute_node_associate_network_pf9(self.ctxt, compute.id,
                                                  n['id'])
        ret_networks = db.compute_node_get_all_networks_pf9(self.ctxt,
                                                            compute['id'])
        self.assertEqual(len(ret_networks), len(nw_names))

        db.network_delete_safe(self.ctxt, networks[0]['id'])

        ret_networks = db.compute_node_get_all_networks_pf9(self.ctxt,
                                                            compute['id'])
        self.assertEqual(len(ret_networks), len(nw_names) - 1)

        # purge deleted network from list
        nw_names.remove('bridge-0')

        # Create another host, report same network from that host
        compute2, service2 = self._create_compute_node()

        for name in nw_names:
            db.compute_node_associate_network_pf9(self.ctxt, compute2.id,
                                                  name)
    ...
    def test_network_disassociation_with_host_mapping(self):
        nw_names = ['bridge-{id}'.format(id=id) for id in range(0, 2)]
        compute = [self._create_compute_node() for i in range(0, 2)]

        networks = []
        for name in nw_names:
            values = {'label': 'Network-{n}'.format(n=name),
                      'bridge': name}
            networks.append(db.network_create_safe(self.ctxt,values))

        for n in networks:
            db.compute_node_associate_network_pf9(self.ctxt, compute[1][0]['id'],
                                                  n['id'])

        # Report both networks
        for i in range(0, 2):
            db.network_cleanup(self.ctxt, compute[i][1]['host'],
                               nw_names)

        self.assertEqual(2, len(db.network_get_all(self.ctxt)))
        ...
        # IAAS-2158 Networks are not cleaned up automatically, but through Janitor service
        # self.assertEqual(1, len(db.network_get_all(self.ctxt)))


        # Do not report any networks
        for i in range(0, 2):
            db.network_cleanup(self.ctxt, compute[i][1]['host'], [])

        # self.assertRaises(exception.NoNetworksFound, db.network_get_all,
        #                  self.ctxt)

class InstanceNetworkPf9TestCase(test.TestCase, ModelsObjectComparatorMixin):
    def setUp(self):
        super(InstanceNetworkPf9TestCase, self).setUp()
        self.ctxt = context.get_admin_context()
        self.project_id = 'fake'
        self.user_id = 'fake'
        self.counter = 0

    def _create_instance_with_args(self, **kwargs):
        args = {'reservation_id': 'a', 'image_ref': 1, 'host': 'host1',
                'node': 'node1', 'project_id': self.project_id,
                'vm_state': 'fake'}
        if 'context' in kwargs:
    ...
    def _create_mapping(self):
        inst = self._create_instance_with_args()
        nets = []
        for i in range(0, 5):
    ...
    def test_instance_network_mapping(self):
        inst, nets = self._create_mapping()
        observed = db.network_get_all_by_instance_pf9(self.ctxt,
                                                      inst['id'])
        self.assertTrue(observed)

        observed_ids = [o['network_id'] for o in observed]
        network_ids = [n['id'] for n in nets]
        self.assertEqual(sorted(network_ids),
                         sorted(observed_ids))

    def test_instance_destroy(self):
        inst, networks = self._create_mapping()
        inst_uuid = inst['uuid']
        db.instance_destroy(self.ctxt, inst_uuid)
        observed = db.network_get_all_by_instance_pf9(self.ctxt,
                                                      inst['id'])
    ...
    def test_instance_create(self):
        ...
        inst =\
            self._create_instance_with_args(networks=[n['uuid'] for n in nets])
        observed = db.network_get_all_by_instance_pf9(self.ctxt,
                                                      inst['id'])
        self.assertEqual(sorted([n['id'] for n in nets]),
                         sorted([n['network_id'] for n in observed]))
    ...
    def test_network_delete(self):
        ...
        for i in range(0, 5):
            instances.append(
                self._create_instance_with_args(networks=[net['uuid']]))
        ...
        for inst in instances:
            db.instance_destroy(self.ctxt, inst['uuid'])

        db.network_delete_safe(self.ctxt, net['id'])
