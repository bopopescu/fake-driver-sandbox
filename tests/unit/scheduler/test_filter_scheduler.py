...
# PF9: changes related to bubbling scheduler errors to UI.
def fake_get_filtered_hosts(hosts, filter_properties, index, filter_errors={}):
    return list(hosts)


# PF9: changes related to hetereogenous network setup on hypervisor
...
class FilterSchedulerTestCase(test_scheduler.SchedulerTestCase):
    ...
    def test_schedule_happy_day(self, mock_get_extra, mock_get_all,
        ...
        self.mox.ReplayAll()
        # PF9: filter errors
        weighed_hosts, msg = self.driver._schedule(self.context, request_spec, {})
    ...
    def test_schedule_host_pool(self, mock_get_extra, mock_get_all,
        ...
        self.mox.ReplayAll()
        # PF9: filter errors
        hosts, msg = self.driver._schedule(self.context, request_spec,
    ...
    @mock.patch('nova.db.instance_extra_get_by_instance_uuid',
                return_value={'numa_topology': None,
                              'pci_requests': None})
    @mock.patch('nova.db.compute_node_get_all_networks_pf9',
                return_value=[])
    ...
    def test_select_destinations_no_valid_host_not_enough(self):
        ...
        # requested to build.
        pf9_msg = "2 hosts failed with error: Not enough memory available."
        consumed_hosts = [mock.MagicMock(), mock.MagicMock()]
        with mock.patch.object(self.driver, '_schedule',
                               return_value = (consumed_hosts, pf9_msg)):
            try:
