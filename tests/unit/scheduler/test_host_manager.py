...
class HostManagerTestCase(test.NoDBTestCase):
    ...
    def _mock_get_filtered_hosts(self, info):
        ...
        def fake_filter_one(_self, obj, filter_props, filter_errors={}):
    ...
    @mock.patch.object(nova.db, 'compute_node_get_all_networks_pf9')
    def test_get_all_host_states(self, mock_cn_nw_pf9, mock_get_by_host):
        mock_get_by_host.return_value = objects.InstanceList()
        mock_cn_nw_pf9.return_value = []
    ...
    def test_get_all_host_states_with_no_aggs(self, svc_get_by_binary,
                                              ...
                                              mock_get_by_host,
                                              mock_cn_gt_nw_pf9):
        mock_cn_gt_nw_pf9.return_value = []
        svc_get_by_binary.return_value = [objects.Service(host='fake')]
        # PF9: adding ID
        cn_get_all.return_value = [
            objects.ComputeNode(host='fake', hypervisor_hostname='fake', id=1)]
        mock_get_by_host.return_value = objects.InstanceList()
    ...
    def test_get_all_host_states_with_not_matching_aggs(self,
        ...
        # PF9: adding IDs to compute nodes
        cn_get_all.return_value = [
            objects.ComputeNode(host='fake', hypervisor_hostname='fake', id=1),
...
class HostManagerChangedNodesTestCase(test.NoDBTestCase):
    ...
    def test_get_all_host_states(self, mock_get_by_host,
                                 mock_compute_node_get_all_networks_pf9):
        mock_get_by_host.return_value = objects.InstanceList()
        mock_compute_node_get_all_networks_pf9.return_value = \
            []
        context = 'fake_context'
