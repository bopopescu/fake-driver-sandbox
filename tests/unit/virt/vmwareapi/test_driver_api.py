...
class VMwareAPIVMTestCase(test.NoDBTestCase):
    ...
    def test_cache_dir_disk_created(self, mock_from_image):
        ...
        root = ds_obj.DatastorePath(self.ds, 'vmware_base',
                                     self.fake_image_uuid,
                                    # PF9 change - 80.vmdk -> 80.0.vmdk
                                     '%s.80.0.vmdk' % self.fake_image_uuid)
        vmwareapi_fake.assertPathExists(self, str(path))
    ...
    def test_spawn_disk_extend_failed_partial_copy(self):
        ...
        # PF9 change 80.vmdk -> 80.0.vmdk
        cached_image = '[%s] vmware_base/%s/%s.80.0.vmdk' % (self.ds,
                                                           uuid, uuid)
    ...
    @testtools.skip('https://platform9.atlassian.net/browse/IAAS-2705')
    def test_snapshot_no_root_disk(self):
        # PF9 - Disabling this check, since snapshot volume fix has been rolled back
        # PF9 - Bug IAAS-2705 tracks this.
        self._iso_disk_type_created(instance_type='m1.micro')
    ...
    def _rescue(self, config_drive=False):
        ...
        self._check_vm_info(info, power_state.RUNNING)
        # PF9 change - We have a vm_ref cache in vm_utils_pf9 that is used
        # when spawning. This conflicts with the refs cached in vm_util - so
        # clear out the entry from vm_utils_pf9, so that it is forced to reload
        vm_util.remove_uuid_from_cache(self.uuid)
        # End Pf9 change
    ...
    def test_get_available_resource(self):
        ...
        self.assertEqual(2048.0, stats['memory_mb'])
        self.assertEqual(1000.0, stats['memory_mb_used'])
        ...
        # PF9 Liberty expects that cpu_info is None, reverting back to Juno
        # code
        # self.assertIsNone(stats['cpu_info'])
        self.assertEqual(stats['cpu_info'], json.dumps(cpu_info, sort_keys=True))
        self.assertEqual(
    ...
    def test_nodename(self):
        ...
        # PF9 change: The node name in Platform9 OpenStack does not contain
        # vCenter UUID to simplify upgrades
        self.assertEqual("%s(%s)" % (test_mor, test_cluster_name),
                         self.conn._create_nodename(test_mor, test_cluster_name),
