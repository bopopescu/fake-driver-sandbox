...
def reset():
    ...
    # PF9: This is an orphan resource pool. It messes up
    # VM listing logic when it looks for VMs in known clusters
    # create_res_pool()
    create_cluster('test_cluster', ds_ref1)
    create_cluster('test_cluster2', ds_ref2)
    # PF9: Add a vm folder
    create_vm_folder('fake-vm-folder')
...
class DataObject(object):
    ...
    # PF9 addition to support PF9 specific unit tests
    def __getitem__(self, key):
...
# PF9 addition
class GuestNic(ManagedObject):
    """Guest Nic Class"""

    def __init__(self):
        super(GuestNic, self).__init__()
        nic_info = DataObject()
        nic_info.ipAddress = ['111.222.333.444']
        nic_info.macAddress = 'de:ad:be:ef:be:ef'
        nic_info.network = 'Test-Network'
        self.GuestNicInfo = [nic_info]
...
class VirtualDisk(DataObject):
    ...
    def __init__(self, controllerKey=0, unitNumber=0):
        ...
        self.deviceInfo.label = "Test-Virtual-Disk"
        self.capacityInKB = 1024
        # End Pf9 changes
...
#PF9 change: Add VM Folder class
class Folder(ManagedObject):
    """VM Folder class"""

    def __init__(self, **kwargs):
        super(Folder, self).__init__('folder')
        self.set("name", kwargs.get("name", "fake-folder"))
#PF9 change end
...
class VirtualMachine(ManagedObject):
    ...
    def __init__(self, **kwargs):
        ...
        # PF9 changes
        self.set('summary.config.name', kwargs.get("name", "test-vm"))
        self.set('summary.guest.ipAddress', kwargs.get("ip_address", "123.123.123.123"))
        self.set("guest.net", kwargs.get("guest_nic", None))
        self.set('summary.config.vmPathName', kwargs.get("path", "[Datastore] test-vm/test-vm.vmx"))
        self.set('resourcePool', kwargs.get("resourcePool", _get_object_refs("ResourcePool")[0]))
        self.set('parent', kwargs.get("parent", _get_object_refs('Folder')[0]))
        # PF9 end
    ...
    def reconfig(self, factory, val):
        ...
        try:
            ...
            # PF9: Below modifications needed as part of testing get_instance_info.
            # It expects that deviceInfo should be present for each device
            # especially for disk and network and also for each controller.

            # Modify disk information, adding deviceInfo
            disk.deviceInfo = DeviceInfo()
            disk.deviceInfo.label = "Hard disk"

            # Modify network information, adding deviceInfo
            self.device[0].deviceInfo = DeviceInfo()
            self.device[0].deviceInfo.label  = "Network adapter"
            # mac address is located in incorrect location in comparison to
            # what vSphere API return. As it not possible to modify "DataObject"
            # class without affecting other tests so adding another field in
            # correct location for get_instance_info method
            self.device[0].macAddress = self.device[0].device.macAddress


            #Modify the controller being added
            controller.deviceInfo = DeviceInfo()
            controller.deviceInfo.label = "LSI controller"
...
class Network(ManagedObject):
    ...
    def __init__(self):
        ...
        # Pf9 changes
        self.set("name", "vmnet0")
        self.key = 0
        self.macAddress = "test-mac-address"
        self.deviceInfo = DeviceInfo()
        self.deviceInfo.label = "Test-Network"
        # End Pf9 changes
...
class PerformanceManager(ManagedObject):
    ...
    def __init__(self, name="test_PerformanceManager"):
        super(PerformanceManager, self).__init__("rm")
        self.set("name", name)
        groupInfo = DataObject()
        nameInfo = DataObject()
        groupInfo.key = "Group-Key"
        nameInfo.key = "Name-Key"

        counter = ManagedObject()
        counter.groupInfo = groupInfo
        counter.nameInfo = nameInfo
        counter.rollupType = "Test-Type"
        counter.key = "Counter-Key"
        counter_array = []
        counter_array.append(counter)
        self.set("perfCounter", [counter_array])
# End Pf9 added class
...
class ResourcePool(ManagedObject):
    ...
    def __init__(self, name="test_ResPool", value="resgroup-test"):
        ...
        # Pf9 changes
        self.set("runtime.memory", memory)
        self.set("runtime.cpu", cpu)
        # End Pf9 changes
...
class ClusterComputeResource(ManagedObject):
    ...
    def _add_root_resource_pool(self, r_pool):
        if r_pool:
            ...
            # PF9 Change:
            # Since resource pool has been added to this cluster change the
            # owner of the resource pool to this cluster
            r_pool_obj = _get_object(r_pool)
            r_pool_obj.set('owner', ManagedObjectReference(name=self.name,
                                                           value=self.mo_id))
...
class HostSystem(ManagedObject):
    ...
    def __init__(self, name="ha-host", connected=True, ds_ref=None,
        ...
        quickstats.overallCpuUsage = 1000
        ...
        # Pf9 changes
        create_datastore("ds-1", 1024, 500)
        ds_ref = _db_content["Datastore"][_db_content["Datastore"].keys()[0]].obj
        datastore_do = DataObject()
        datastore_do.ManagedObjectReference = [ds_ref]
        self.set("datastore", [[ds_ref]])
        # End Pf9 changes
...
# Pf9 created class
class Pf9Datacenter(ManagedObject):
    """Pf9 Datacenter class."""

    def __init__(self, **kwargs):
        super(Pf9Datacenter, self).__init__("dc")
        self.set("name", kwargs.get("name", 'test-dc'))
        self.set("vmFolder", "vm_folder_ref")
        self.set("network", kwargs.get("network", None))
# End Pf9 created class
...
class DeviceInfo(object):
    """ PF9: deviceInfo class for adding 'deviceInfo' for disks and networks"""
...
# Pf9 changes
def create_perf_manager():
    perf_manager = PerformanceManager()
    _create_object('PerformanceManager', perf_manager)
    return perf_manager.obj


def create_vm_folder(name):
    vm_folder = Folder(name=name)
    _create_object('Folder', vm_folder)
    return vm_folder.obj
# End Pf9 changes
...
def create_vm(uuid=None, name=None,
    ...
    vm_dict = {"name": name,
              ...
              "instanceUuid": uuid,
              "version": version,
              # PF9: pass in resource pool as well
...
class FakeVim(object):
    ...
    def __init__(self, protocol="https", host="localhost", trace=None):
        ...
        # Pf9 changes
        service_content.perfManager = create_perf_manager()
        # End Pf9 changes
    ...
    # Pf9 changes
    def QueryPerf(self, perf_manager, querySpec):
        return
    # End Pf9 changes
