...
class VMwareVMOpsTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        # PF9 change for supporting datacenter nested under folders
        fake_dc_ref = vmwareapi_fake.ManagedObjectReference('fake-dc')
    ...
    def test_spawn_non_root_block_device(self, from_image,
        ...
        with mock.patch.object(self._vmops, '_volumeops') as volumeops:
            ...
            build_virtual_machine.assert_called_once_with(self._instance,
                ...
                extra_specs, self._get_metadata(), self._context)
    ...
    def test_spawn_with_no_image_and_block_devices(self, from_image,
        ...
        with mock.patch.object(self._vmops, '_volumeops') as volumeops:
            ...
            build_virtual_machine.assert_called_once_with(self._instance,
                image_info, vi.dc_info, vi.datastore, [],
                extra_specs, self._get_metadata(is_image_used=False), self._context)
    ...
    def _test_spawn(self,
        ...
        with contextlib.nested(
            ...
            source_file = six.text_type('[fake_ds] vmware_base/%s/%s.vmdk' %
                          (self._image_id, self._image_id))
            # PF9: add .0 to VMDK name.
            # Note (Pushkar): This change seems to be carried over from havana
            # but the need for it is not known.
            dest_file = six.text_type('[fake_ds] vmware_base/%s/%s.%d.0.vmdk' %
                          (self._image_id, self._image_id,
            ...
            mock_copy_virtual_disk.assert_called_with(self._session,
                                                      # PF9 change to support
                                                      # datacenter nested under
                                                      # folders
                                                      self._dc_info.ref,
                                                      source_file,
                                                      dest_file)
