...
def fake_get_instance_files_pf9(instance):
    return []
...
""", flags=flags)
    ...
    @testtools.skip('PF9 skip. Need to fix this')
    ...
    def test_get_instance_disk_info_works_correctly(self):
        ...
        utils.execute('env', 'LC_ALL=C', 'LANG=C', 'qemu-img', 'info',
                      # PF9 change to run the command as root
                      '/test/disk.local', run_as_root=True).AndReturn((ret, ''))
    ...
    def _test_create_image_plain(self, os_type='', filename='', mkfs=False):
        ...
        wantFiles = [
            {'filename': '356a192b7913b04c54574d18c28d46e6395428ab',
             # PF9: 1GB disk size flavor is being used. Unsure why it is
             # expecting disk size to be 10GB
             'size': 1 * units.Gi},
            {'filename': filename,
             'size': 20 * units.Gi},
            ]
    ...
    def test_create_image_with_swap(self):
        ...
        wantFiles = [
            {'filename': '356a192b7913b04c54574d18c28d46e6395428ab',
             # PF9: Flavor being used is 1GB disk size. Unsure why it is
             # expecting 10GB
             'size': 1 * units.Gi},
            {'filename': self._EPHEMERAL_20_DEFAULT,
             'size': 20 * units.Gi},
    ...
    def test_destroy_undefines(self, mock_save, mock_delete_instance_files,
        ...
        drvr = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), False)
        # Mock pf9 functions
        self.stubs.Set(drvr, '_get_instance_files_pf9', fake_get_instance_files_pf9)
    ...
    def test_destroy_undefines_no_undefine_flags(self, mock_save):
        ...
        self.stubs.Set(drvr, 'delete_instance_files',
                       fake_delete_instance_files)
        # Mock pf9 functions
        self.stubs.Set(drvr, '_get_instance_files_pf9',
                       fake_get_instance_files_pf9)
        instance = objects.Instance(self.context, **self.test_instance)
        drvr.destroy(self.context, instance, [])
        mock_save.assert_called_once_with()
    ...
    # PF9 skipping unit test
    @testtools.skip('https://platform9.atlassian.net/browse/IAAS-1608')
    ...
    def test_get_all_networks_pf9(self, mock_execute):
        mock_execute.return_value =\
            "bridge name	bridge id		STP enabled	interfaces\n" \
            "virbr0		8000.525400d8f9ad	yes		virbr0-nic", ""
        conn = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        nets = conn.get_all_networks_pf9()
        self.assertEqual('virbr0', nets[0]['bridge'])

    @mock.patch('oslo_concurrency.processutils.execute')
    def test_ip_mapping_pf9(self, mock_execute):
        mock_execute.return_value = \
            "Address                  HWtype  HWaddress           Flags Mask            Iface\n" \
            "10.4.47.82               ether   00:50:56:a9:04:fe   C                     br_lab", ""
        conn = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        self.mox.StubOutWithMock(conn, 'list_instance_uuids')
        self.mox.StubOutWithMock(conn, 'get_instance_info')
        conn.list_instance_uuids().AndReturn(['70c92b81-c3c1-4508-a1f4-ecbe86007a26'])
        conn.get_instance_info(mox.IgnoreArg()).AndReturn({'virtual_interfaces':
                                                               [{'mac_address': '00:50:56:a9:04:fe',
                                                                    'bridge': 'br_lab'}]})
        self.mox.ReplayAll()
        ips = conn.get_all_ip_mapping_pf9()
        self.assertIn('70c92b81-c3c1-4508-a1f4-ecbe86007a26', ips.keys())
        self.assertEqual('10.4.47.82',
                         ips['70c92b81-c3c1-4508-a1f4-ecbe86007a26'][0]['ip_address'])

    def test_stats_pf9(self):
        conn = libvirt_driver.LibvirtDriver(fake.FakeVirtAPI(), True)
        res_types = [resource_types.CPU, resource_types.DISK_TOTAL, resource_types.DISK_USED,
                     resource_types.MEMORY, resource_types.NETWORK_RECV_PKTS,
                     resource_types.NETWORK_RECV_RATE, resource_types.NETWORK_SEND_RATE,
                     resource_types.NETWORK_SENT_PKTS]
        res_hooks = conn.pf9_resource_hooks

        res_hooks[resource_types.CPU] = lambda: 50
        res_hooks[resource_types.MEMORY] = lambda: 40
        res_hooks[resource_types.DISK_TOTAL] = lambda: 10
        res_hooks[resource_types.DISK_USED] = lambda: 5
        res_hooks[resource_types.NETWORK_SENT_PKTS] = lambda: 11
        res_hooks[resource_types.NETWORK_RECV_PKTS] = lambda: 2
        res_hooks[resource_types.NETWORK_SEND_RATE] = lambda: 4
        res_hooks[resource_types.NETWORK_RECV_RATE] = lambda: 22

        stats = conn.get_host_stats_pf9(res_types, refresh=True)

        self.assertEqual(sorted(stats.keys()), sorted(res_types))

        for res_type in res_types:
            self.assertEqual(res_hooks[res_type](), stats[res_type])
    # PF9 unit tests end
