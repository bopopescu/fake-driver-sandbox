...
class LibvirtUtilsTestCase(test.NoDBTestCase):
    ...
    def test_xml_desc_parse_centos_pf9(self):
        data = pkgutil.get_data("nova.tests.unit.virt.libvirt", "test-centos.xml")
        info = libvirt_utils.parse_instance_xml(data)
        self.assertIsNotNone(info['instance_uuid'])
        self.assertIsNotNone(info['name'])
        self.assertIsNotNone(info['vcpus'])
        self.assertIsNotNone(info['memory_mb'])
        self.assertIsNotNone(info['virtual_interfaces'])
        self.assertIs(True, len(info['virtual_interfaces']) > 0)

    def test_xml_desc_parse_ubuntu_pf9(self):
        data = pkgutil.get_data("nova.tests.unit.virt.libvirt", "test-ubuntu.xml")
        info = libvirt_utils.parse_instance_xml(data)
        self.assertIsNotNone(info['instance_uuid'])
        self.assertIsNotNone(info['name'])
        self.assertIsNotNone(info['vcpus'])
...
"""
    ...
    @mock.patch.object(utils, 'execute')
    @testtools.skip('https://platform9.atlassian.net/browse/IAAS-2891')
