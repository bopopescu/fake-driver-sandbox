...
class ComputeManagerUnitTestCase(test.NoDBTestCase):
    ...
    @testtools.skip('https://platform9.atlassian.net/browse/IAAS-3988')
    ...
    def _test_sync_to_stop(self, db_power_state, vm_state, driver_power_state,
                           stop=True, force=False, shutdown_terminate=False):
        if driver_power_state in [power_state.RUNNING]:
            # Nova is not expected to call stop when driver state is active
            return

        instance = self._get_sync_instance(db_power_state, vm_state,
    ...
    # PF9 functions
    def test_evacuate_disabled(self):
        ...
        inst.host = 'not-' + our_host
        migration = objects.Migration(instance_uuid=inst.uuid)
        ...
        with contextlib.nested(
            ...
            mock.patch.object(self.compute, '_get_instances_on_driver',
                              return_value=[inst])
        ) as (migration_list, destroy_func,
              _get_instances_on_driver):
            migration_list.return_value = [migration]
            destroy_func.assert_not_called()
    ...
    def test_instance_not_found_on_driver_pf9(self, mock_delta):
        mock_delta.return_val = True
        with contextlib.nested(
            mock.patch.object(self.compute.driver, 'list_instance_uuids'),
            mock.patch.object(objects.InstanceList, 'get_by_host'),
            mock.patch.object(db, 'instance_get_by_uuid'),
            mock.patch.object(self.compute.conductor_api, 'instance_not_found')
        ) as (mock_list, mock_db_list, mock_get_by_uuid, mock_instance_not_found):
            mock_list.return_value = []
            instance = fake_instance.fake_instance_obj(self.context, host='fake-mini',
                                                       launched_at=timeutils.utcnow())

            instance.save()
            mock_db_list.return_value = [instance]
            mock_get_by_uuid.return_value = instance

            def _inst_not_found(context, host, uuids):
                instance.vm_state = vm_states.ERROR

            mock_instance_not_found.side_effect = _inst_not_found

            self.compute._remove_instances_not_on_driver(self.context)
            self.assertEqual(vm_states.ERROR, instance.vm_state)

    @mock.patch('oslo_utils.timeutils.is_older_than')
    @mock.patch('nova.objects.instance.Instance.refresh')
    def test_instances_not_found_on_driver_pf9(self, mock_refresh, mock_delta):
        instances = []
        mock_delta.return_val = True
        for i in range(0, 5):
            inst = fake_instance.fake_instance_obj(self.context, host='fake-mini',
                                                   launched_at=timeutils.utcnow())
        ...
        with contextlib.nested(
            ...
            mock.patch.object(objects.InstanceList, 'get_by_host'),
            mock.patch.object(self.compute.conductor_api, 'instance_not_found')
        ) as (mock_list, mock_get_by_host, mock_not_found):
            mock_list.return_value = [instances[2 * i]['uuid'] for i in range(0, 2)]
            mock_get_by_host.return_value = instances

            def _inst_not_found(context, host, uuids):
                for inst in instances:
                    if inst.uuid in uuids:
                        inst.vm_state = vm_states.ERROR

            mock_not_found.side_effect = _inst_not_found

            self.compute._remove_instances_not_on_driver(self.context)

            unchanged_instance_uuids = mock_list.return_value
            for inst in instances:
                if inst.uuid not in unchanged_instance_uuids:
                    self.assertEqual(vm_states.ERROR, inst.vm_state)
                else:
                    self.assertEqual(None, inst.vm_state)

    @mock.patch('oslo_utils.timeutils.is_older_than')
    @mock.patch('nova.objects.instance.Instance.refresh')
    def test_instances_not_on_driver_with_vm_state_pf9(self, mock_refresh, mock_delta):
        mock_delta.return_val = True
        instances = []

        for ts in [vm_states.SUSPENDED, vm_states.BUILDING, vm_states.RESCUED, vm_states.RESIZED]:
            inst = fake_instance.fake_instance_obj(self.context, host='fake-mini', vm_state=ts,
                                                   launched_at=timeutils.utcnow())
            instances.append(inst)
        ...
        with contextlib.nested(
            ...
            mock.patch.object(objects.InstanceList, 'get_by_host'),
            mock.patch.object(self.compute.conductor_api, 'instance_not_found')
        ) as (mock_list_uuids, mock_get_by_host, mock_not_found):
            mock_list_uuids.return_val = []
            mock_get_by_host.return_val = instances

            self.compute._remove_instances_not_on_driver(self.context)
            for inst in instances:
                self.assertNotEqual(vm_states.ERROR, inst.vm_state)

            self.assertFalse(mock_not_found.called)
