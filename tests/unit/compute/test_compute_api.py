...
class _ComputeAPIUnitTestMixIn(object):
    ...
    def test_create_quota_exceeded_messages(self):
        ...
        quotas = {'instances': 1, 'cores': 1, 'ram': 1, 'root_gb': 1} # PF9 change add root_gb
        usages = {r: {'in_use': 1, 'reserved': 1} for r in
                  ['instances', 'cores', 'ram', 'root_gb']} # PF9 change add root_gb
        ...
        for _unused in range(2):
            ...
            quota.QUOTAS.reserve(self.context, instances=40,
                                 ...
                                 root_gb=mox.IsA(int), # PF9 change
                                 project_id=mox.IgnoreArg(),
                                 user_id=mox.IgnoreArg(),
    ...
    def _test_delete(self, delete_type, **attrs):
        """ PF9 Lot of changes in this function. Merge with caution
        """
        ...
        # PF9 change to prevent deletions when host is down
        ...
        if self.cell_type != 'api':
            ...
            if inst.vm_state == vm_states.SOFT_DELETED:
                soft_delete = True
            # PF9 change: service_is_up is always invoked, even if vm_state == SHELVED_OFFLOADED
	    self.context.elevated().AndReturn(self.context)
	    ...
	    self.compute_api.servicegroup_api.service_is_up(
            ...
            if (inst.host == 'down-host' or
                ...
                # next function not invoked for down-host as part of PF9 change to prevent
                # deletion when host is down
    ...
    def test_check_instance_quota_exceeds_with_multiple_resources(self):
        # PF9 change add root_gb
        quotas = {'cores': 1, 'instances': 1, 'ram': 512, 'root_gb': 1}
        usages = {'cores': dict(in_use=1, reserved=0),
                  'instances': dict(in_use=1, reserved=0),
                  'ram': dict(in_use=512, reserved=0),
                  'root_gb': dict(in_use=1, reserved=0)}
        overs = ['cores', 'instances', 'ram', 'root_gb']
        over_quota_args = dict(quotas=quotas,
                               usages=usages,
                               overs=overs)
        ...
        with mock.patch.object(objects.Quotas, 'reserve', side_effect=e):
            try:
                ...
                self.assertEqual('cores, instances, ram, root_gb', e.kwargs['overs'])
                self.assertEqual('1, 1, 512, 1', e.kwargs['req'])
                self.assertEqual('1, 1, 512, 1', e.kwargs['used'])
                self.assertEqual('1, 1, 512, 1', e.kwargs['allowed'])
            else:
                self.fail("Exception not raised")
    ...
    def _create_instance_fault(self, fault, instance):
        fault_obj = objects.InstanceFault(context=self.context)
        fault_obj.instance_uuid = instance['uuid']
        fault_obj.deleted = False
        fault_obj.update(compute_utils.exception_to_dict(fault))
        return fault_obj

    @mock.patch.object(objects.Instance, 'get_by_uuid')
    @mock.patch.object(objects.Instance, 'save')
    @mock.patch.object(objects.InstanceFaultList, 'get_by_instance_uuids')
    def test_instance_register_prev_state_error(self, mock_get_faults, mock_save, mock_get_by_uuid):
        """
        PF9 function
        :param mock_get_by_uuid:
        :param mock_get_faults:
        :return:
        """
        inst = self._create_instance_obj(params=dict(vm_state=vm_states.ERROR, deleted=False,
                                                     display_name='fake'))
        inst_fault = self._create_instance_fault(
            exception.InstanceNotFoundOnHypervisorPF9(instance_id=inst['uuid']), inst)

        mock_get_by_uuid.return_value = inst
        mock_get_faults.return_value = [inst_fault]

        update = dict(instance_uuid=inst.uuid,
                      power_state=power_state.RUNNING,
                      name=inst.display_name,
                      vcpus=inst.vcpus,
                      memory_mb=inst.memory_mb)
        self.compute_api.instance_register(self.context.elevated(), 'fake-host', [update])
        self.assertEqual(vm_states.ACTIVE, inst.vm_state)
        self.assertEqual(True, inst_fault.deleted)


    @mock.patch.object(objects.Instance, 'get_by_uuid')
    @mock.patch.object(objects.Instance, 'save')
    @mock.patch.object(objects.InstanceFaultList, 'get_by_instance_uuids')
    def test_instance_register_multi_faults(self, mock_get_faults, mock_save, mock_get_by_uuid):
        """
        PF9 function
        :param mock_get_by_uuid:
        :param mock_get_faults:
        ...
        inst_fault = self._create_instance_fault(
            exception.InstanceNotFoundOnHypervisorPF9(instance_id=inst['uuid']), inst)
        inst_fault2 = self._create_instance_fault(
            exception.InstanceDeployFailure(reason='known'), inst)

        mock_get_by_uuid.return_value = inst
        mock_get_faults.return_value = [inst_fault, inst_fault2]
        ...
        self.assertEqual(True, inst_fault.deleted)
        self.assertEqual(False, inst_fault2.deleted)
