...
class StdTableOutputParserTestPF9(test.TestCase):
    """
    Miscellaneous test about parsing StdTableOutputParser
    """
    def setUp(self):
        super(StdTableOutputParserTestPF9, self).setUp()

    def test_get_all_brctl_output(self):
        data = pkgutil.get_data("nova.tests.unit.compute","test-brctl-output.txt")
        headers = ["bridge name", "bridge id", "STP enabled", "interfaces"]
        stdout_parser = compute_utils.StdTableOutputParser(data, delimiter='\t',
                                             headers=headers)
        ret_data = stdout_parser.get_all()
        self.assertEquals('docker0', ret_data[0]['bridge name'])
        self.assertEquals('virbr0', ret_data[1]['bridge name'])

    def test_get_all_arp_output(self):
        data = pkgutil.get_data("nova.tests.unit.compute","test-arp-output.txt")
        stdout_parser = compute_utils.StdTableOutputParser(data,
                                                           columns_desired=-1,
                                                           headers=None)
        ret_data = stdout_parser.get_all()
        print ret_data
        self.assertEquals('10.1.10.1', ret_data[0]['Address'])
        self.assertEquals('9c:d3:6d:c3:86:ab', ret_data[0]['HWaddress'])
        self.assertEquals('10.1.10.12', ret_data[2]['Address'])
        self.assertEquals('bc:ee:7b:c4:4f:d0', ret_data[2]['HWaddress'])
