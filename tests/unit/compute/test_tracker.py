...
class TestUpdateAvailableResources(BaseTestCase):
    ...
    def _update_available_resources(self):
        ...
        with mock.patch.object(self.rt, '_update') as update_mock:
            with  mock.patch('nova.compute.resource_tracker.ResourceTracker._get_all_ips', return_value = []):
...
class TestMoveClaim(BaseTestCase):
    def setUp(self):
        ...
        with contextlib.nested(
            ...
            mock.patch('nova.compute.resource_tracker.ResourceTracker._get_all_ips')
        ) as (inst_list_mock, migr_mock, all_ips_mock):
            inst_list_mock.return_value = objects.InstanceList(objects=[])
            migr_mock.return_value = objects.MigrationList(objects=[])
            all_ips_mock.return_value = []
