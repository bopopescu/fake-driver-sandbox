...
class HypervisorsController(object):
    ...
    def _delete(self, context, req, compute_node_id):
        self.host_api.compute_node_delete(context, compute_node_id)
    ...
    def delete(self, req, id):
        """Destroys hypervisor"""
        try:
            LOG.info(_('Deleting hypervisor %s'), str(id))
            self._delete(req.environ['nova.context'], req, id)
        except exception.NotFound:
            msg = _("Hypervisor could not be found")
            raise webob.exc.HTTPNotFound(explanation=msg)
