...
def network_dict(context, network):
    # PF9 added fields - description, range_start, range_end
    fields = ('id', 'cidr', 'netmask', 'gateway', 'broadcast', 'dns1', 'dns2',
              'cidr_v6', 'gateway_v6', 'label', 'netmask_v6', 'description',
...
class NetworkController(wsgi.Controller):
    ...
    # PF9: Add functions for network tenant association
    def _marshall_network_access_pf9(self, context, network_id):

        try:
            access_list = self.network_api.get_access_pf9(context,
                                                          network_id)
    ...
    def _extend_pf9(self, context, network):
        if 'id' in network:
            projects = self._marshall_network_access_pf9(context,
                                                         network['id'])
            network['project_access_pf9'] = projects
        else:
            LOG.warning('No id found')
    ...
    def index(self, req):
        ...
        # PF9 change for associating networks to tenants
        for network in result:
            self._extend_pf9(context, network)
    ...
    def show(self, req, id):
        ...
        try:
            ...
            # PF9 changes
            ret = network_dict(context, network)
            self._extend_pf9(context, ret)
        except exception.NetworkNotFound:
            msg = _("Network not found")
            raise exc.HTTPNotFound(explanation=msg)
        return {'network': ret}
    ...
    def update(self, req, id, body):
        ...
        params = body["network"]
        try:
            self.network_api.update(context, id, **params)
        except (exception.FixedIpAlreadyInUse,
                exception.FixedIpExists,
                exception.FixedIpRangeStartInvalid,
                exception.FixedIpRangeEndInvalid) as e:
            return exc.HTTPConflict(explanation=e.format_message())
