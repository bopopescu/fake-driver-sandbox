...
def compute_node_get_by_service_id_and_node(context, service_id, node):
    ...
    :param node: compute node within host to retrieve
    ...
    """
    return IMPL.compute_node_get_by_service_id_and_node(context, service_id, node)
...
def compute_node_get_all(context):
    ...
    """
    # Using PF9 query to populate pf9_stat, ip_info and pf9_networks properties
    return IMPL.compute_node_get_all_pf9(context)
...
def compute_node_update_pf9_stats(context, compute_id, values):
    """Set the given PF9 stats of a computeNode and update
       Raises ComputeHostNotFound if computeNode does not exist
    """
    return IMPL.compute_node_update_pf9_stats(context, compute_id, values)
...
def compute_node_get_all_networks_pf9(context, compute_id):
    return IMPL.compute_node_get_all_networks_pf9(context, compute_id)


def compute_node_associate_network_pf9(context, compute_id, network_id):
    return IMPL.compute_node_associate_network_pf9(context, compute_id, network_id)


def compute_node_disassociate_network_pf9(context, compute_id, network_id):
    IMPL.compute_node_disassociate_network_pf9(context, compute_id,
                                               network_id)
...
def fixed_ip_bulk_create_from_tuples(context, ips):
    """Create a lot of fixed ips from tuples of the form
    (network_id, address, reserved)
    """
    return IMPL.fixed_ip_bulk_create_from_tuples(context, ips)

def fixed_ip_reallocate(context, delete_ips, create_ips):
    """Delete and create lists of fixed IP addresses"""
    return IMPL.fixed_ip_reallocate(context, delete_ips, create_ips)
...
def network_get_access_pf9(context, network_id):
    """Get list of projects which are allowed to use a private network"""
    return IMPL.network_get_access_pf9(context, network_id)

def network_add_access_pf9(context, network_id, project_id):
    """Add project to list of authrized users of a private network"""
    IMPL.network_add_access_pf9(context, network_id, project_id)

def network_remove_access_pf9(context, network_id, project_id):
    """Remove project from list of authorized network users"""
    IMPL.network_remove_access_pf9(context, network_id, project_id)
...
def virtual_interface_delete(context, vif_id):
    ...
    IMPL.virtual_interface_delete(context, vif_id)
...
def instance_restore(context, instance_uuid):
    """Restore DB records of deleted instances
    """
    return IMPL.instance_restore(context, instance_uuid)
...
def instance_get_all_by_network_pf9(context, network_id):
    return IMPL.instance_get_all_by_network_pf9(context,
                                                network_id)


def instance_associate_networks_pf9(context, instance_id, network_ids):
    return IMPL.instance_associate_networks_pf9(context, instance_id,
                                                network_ids)


def instance_disassociate_networks_pf9(context, instance_id, network_ids):
    IMPL.instance_disassociate_networks_pf9(context, instance_id, network_ids)
...
def network_create_with_host_mapping_pf9(context, host, values):
    return IMPL.network_create_with_host_mapping_pf9(context, host, values)

def network_check_and_create_host_mapping_pf9(context, host, bridge_name):
    IMPL.network_check_and_create_host_mapping_pf9(context, host,
                                                   bridge_name)
def network_delete_fixed_ips(context, network_id):
    """
    PF9 method: Delete free fixed IPs associated with network
    :param context:
    :param network_id:
    :return:
    """
    IMPL.network_delete_fixed_ips(context, network_id)

def network_cleanup(context, host, reported_bridge_names, node=None):
    """
    PF9: Cleans up networks from nova DB which have been deleted on hypervisor
    """
    return IMPL.network_cleanup(context, host, reported_bridge_names, node)
...
def network_get_all_by_bridges(context, bridges, project_only="allow_none"):
    ...
    :param context:
    :param bridges:
    :return: The networks by the bridge
    """
    return IMPL.network_get_all_by_bridges(context, bridges, project_only)


def network_get_all_by_instance_pf9(context, instance_id):
    return IMPL.network_get_all_by_instance_pf9(context,
                                                instance_id)
...
def block_device_mapping_get_all_by_instance_uuids(context, instance_uuids, use_subordinate=False):
    ...
    return IMPL.block_device_mapping_get_all_by_instance_uuids(context, instance_uuids, use_subordinate)
