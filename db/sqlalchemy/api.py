...
def model_query_network_pf9(context, args=None, session=None,
                            use_subordinate=False, read_deleted=None,
                            project_only=False):
    """
    Query helper to return networks owned/shared within projects
    This function will query Networks table twice to get owned and shared
    networks separately and then perform a union of the 2 results.
    ...
    map_model = models.NetworkProjectsPf9
    ...
    if 'no' == read_deleted:
        ...
        raise ValueError(_("Unrecognized read_deleted value '%s'")
                           % read_deleted)
    q1 = sqlalchemyutils.model_query(base_model, session, **query_kwargs)
    if nova.context.is_user_context(context) and project_only:
        q2 = sqlalchemyutils.model_query(base_model, session, **query_kwargs).\
            join(map_model, base_model.id == map_model.network_id).filter(
                map_model.project_id == context.project_id)

        if project_only == 'allow_none':
            q1 = q1.filter(or_(base_model.project_id == context.project_id,
...
def _sync_instances(context, project_id, user_id, session):
    # PF9: Storage quotas
    return dict(zip(('instances', 'cores', 'ram', 'root_gb'),
...
def service_destroy(context, service_id):
    ...
    with session.begin():
        ...
        query = model_query(context, models.ComputeNode, session=session).\
                    filter(or_(models.ComputeNode.service_id == service_id,
                               models.ComputeNode.host == service['host']))
        nodes = query.all()
        query.soft_delete(synchronize_session=False)

        # PF9 change: remove auxiliary data for the compute node (should be only one) for this service_id
        if nodes is None or len(nodes) is 0:
            LOG.warning('No compute node record found for service %s being deleted', service_id)
            return
        LOG.info('Deleting auxiliary data for node %s', str(nodes[0]))
        for table in [models.ComputeNodesExt, models.ComputeNodesIPInfo,
                      models.ComputeNodesPf9Stat,
                      models.ComputeNodeNetworksPf9]:
            result = model_query(context, table,
                                 session=session).\
                                 filter_by(host_id=nodes[0].id)
            if not result:
                LOG.warning('Can not find compute node record for %s in %s',
                            str(nodes[0].id), str(table))
                # It is possible that some of these records may not exist when we
                # we are trying to delete record so move on
                continue
            result.soft_delete(synchronize_session=False)
...
def _compute_node_get(context, compute_id, session=None):
    # PF9: Host-network association
    result = model_query(context, models.ComputeNode, session=session).\
            filter_by(id=compute_id).\
            options(joinedload('pf9_ip_info')).\
            options(joinedload('pf9_stat')).\
            options(joinedload('pf9_networks')).\
            first()
...
# PF9 function to return the specific node associated with this service ID
def compute_node_get_by_service_id_and_node(context, service_id, node):
    result = model_query(context, models.ComputeNode, read_deleted='no').\
        filter_by(service_id=service_id).\
        filter_by(hypervisor_hostname=node).\
        first()
...
# TODO: PF9: Remove this function when we have implemented the different API for querying
#       stats, ip and network information for all the nodes.
def compute_node_get_all_pf9(context):
    ...
    # Retrieve ComputeNode
    compute_node = models.ComputeNode.__table__
    pf9_ip_info = models.ComputeNodesIPInfo.__table__
    pf9_stat = models.ComputeNodesPf9Stat.__table__
    pf9_nw = models.ComputeNodeNetworksPf9.__table__
    with engine.begin() as conn:
        ...
        pf9_ip_query = sql.select(filter_columns(pf9_ip_info)).\
                        where(pf9_ip_info.c.deleted == 0).\
                        order_by(pf9_ip_info.c.host_id)
        pf9_ip_info_rows = conn.execute(pf9_ip_query).fetchall()

        pf9_stat_query = sql.select(filter_columns(pf9_stat)).\
                            where(pf9_stat.c.deleted == 0).\
                            order_by(pf9_stat.c.host_id)
        pf9_stat_rows = conn.execute(pf9_stat_query).fetchall()

        pf9_nw_query = sql.select(filter_columns(pf9_nw)).\
            where(pf9_nw.c.deleted == 0).order_by(pf9_nw.c.host_id)
        pf9_nw_rows = conn.execute(pf9_nw_query).fetchall()
    ...
    def _get_next(iter):
        """
        PF9: A simple wrapper on iterator next to not throw exception,
        but return a tuple of (negative number, empty string), meant to work along
        with other commonly used tuples later in this function
        """
        try:
            return iter.next()
    ...
    def _assign_prop_to_matching_node(prop_name, node, curr_prop, prop_iter):
        """
        PF9: A function that assigns "property" to the node after
        finding a matching node from the node_prop_iter
        :param prop_name: property to assign to node, like 'ip_info'
        :param node: the node object
        :param curr_prop: current node-id, property-value tuple
        :param prop_iter: iterator to go through if the current-node-id doesn't match
           node['id']
        :return: tuple of next node-id, property-value
        """

        # First find the matching node_id if any, moving to next in prop iter
        # if the current one doesn't match
        while node['id'] > curr_prop[0] and curr_prop[0] > 0:
            curr_prop = _get_next(prop_iter)
        if node['id'] == curr_prop[0]:
            node[prop_name] = list(curr_prop[1])
            curr_prop = _get_next(prop_iter)
        return curr_prop
    ...
    # NOTE(rparikh): This whole function needs to be replaced by 1 or 2 queries or
    # stored procedures, these in memory joins can get very expensive when we get
    # to 1000 number of nodes.
    compute_nodes.sort(key=lambda node: node['id'])
    compute_nodes_iter = iter(compute_nodes)

    pf9_stat_list = [dict(proxy.items()) for proxy in pf9_stat_rows]
    compute_nodes_pf9_stats = itertools.groupby(pf9_stat_list,
                                                lambda s: s['host_id'])

    pf9_ip_info_list = [dict(proxy.items()) for proxy in pf9_ip_info_rows]
    compute_nodes_ip_info = itertools.groupby(pf9_ip_info_list,
                                              lambda s: s['host_id'])

    pf9_nw_rows = [dict(proxy.items()) for proxy in pf9_nw_rows]
    compute_nodes_networks = itertools.groupby(pf9_nw_rows,
                                               lambda s: s['host_id'])

    curr_node_ip = _get_next(compute_nodes_ip_info)
    curr_node_pf9_stat = _get_next(compute_nodes_pf9_stats)
    curr_node_nw = _get_next(compute_nodes_networks)

    # Perform an in memory join, this loop exploits the fact that
    # compute_node_iter and all other properties (ip_info, stats..)
    # are all sorted by node['id'] and does the join in linear time
    for node in compute_nodes_iter:
        # Initialize all PF9 properties in case the DB does not have corresponding data yet
        node['pf9_stat'] = list()
        node['pf9_ip_info'] = list()
        node['pf9_networks'] = list()

        curr_node_pf9_stat = _assign_prop_to_matching_node('pf9_stat', node,
                                                           curr_node_pf9_stat,
                                                           compute_nodes_pf9_stats)
        curr_node_ip = _assign_prop_to_matching_node('pf9_ip_info', node,
                                                     curr_node_ip,
                                                     compute_nodes_ip_info)
        curr_node_nw = _assign_prop_to_matching_node('pf9_networks', node,
                                                     curr_node_nw,
                                                     compute_nodes_networks)

    return compute_nodes


def compute_node_get_all_networks_pf9(context, compute_id):

    return model_query(context, models.ComputeNodeNetworksPf9).\
        filter_by(host_id=compute_id).all()

def compute_node_associate_network_pf9(context, compute_id, network_id):
    model = models.ComputeNodeNetworksPf9
    session = get_session()

    orm_obj = None
    with session.begin():
        result = model_query(context, model, session=session).\
            filter(model.host_id == compute_id).\
            filter(model.network_id == network_id).first()

        if result:
            raise exception.NetworkAssociationExists(compute_id=compute_id,
                                                     network_id=network_id)
        orm_obj = models.ComputeNodeNetworksPf9()
        orm_obj.host_id = compute_id
        orm_obj.network_id = network_id
        orm_obj.save(session=session)
    return orm_obj

def compute_node_disassociate_network_pf9(context, compute_id, network_id):
    model = models.ComputeNodeNetworksPf9
    session = get_session()
    ...
    with session.begin():
        ...
        if not result:
            raise exception.NetworkAssociationNotFound(compute_id=compute_id,
                                                       network_id=network_id)
        result.soft_delete(session=session)
...
def _prep_ip_infos(values):
    out_infos = []

    infos = values.get('pf9_ip_info', [])

    for info in infos:
        out_info = models.ComputeNodesIPInfo()
        out_info['interface_name'] = info['interface_name']
        out_info['ip'] = info['ip']
        out_infos.append(out_info)

    values['pf9_ip_info'] = out_infos


def _prep_pf9_stat(values):
    results = []
    pf9_stats = values.get('pf9_stat', {})

    for k, v in pf9_stats.iteritems():
        stat = models.ComputeNodesPf9Stat()
        stat['key'] = k
        stat['value'] = v
        results.append(stat)

    values['pf9_stat'] = results
...
def compute_node_create(context, values):
    ...
    compute_node_ref=None
    session = get_session()
    # PF9: Run the creation of a compute node to prevent race condition
    # where two compute nodes with the same service ID is added to the
    # compute node table and both are active (deleted=0).
    # A de-auth and re-auth creates new service ID and
    # corresponding compute_node entry. A hostname change on the other hand
    # will cause the previous compute_node entry to be marked deleted.
    # Refer bug: IAAS-2397
    with session.begin():
        try:
            result = compute_node_get_by_service_id_and_node(
                      context, values['service_id'], values['hypervisor_hostname'])
            compute_node_ref = result
        except (exception.ServiceNotFound, KeyError) as e:
                # compute node not found.or hypervisor_hostname not given.
                # KeyError here for hypervisor_hostname - some unit tests do
                # not pass hypervisor_hostname. 
    # PF9 end
            _prep_ip_infos(values)
            _prep_pf9_stat(values)
...
def _update_pf9_stats(context, stats_update, compute_id, session):
    existing = model_query(context, models.ComputeNodesPf9Stat, session=session,
            read_deleted='no').filter_by(host_id=compute_id).all()
    statmap = dict()
    ...
    if existing:
        for row in existing:
            if row.key in old_stats.keys():
                row.value = old_stats[row.key].value
...
def compute_node_update(context, compute_id, values):
    ...
    """
    #FIXME update extended IP addresses, pf9 extended information
    pf9_ip_info = values.pop('pf9_ip_info', [])
    pf9_stats = values.pop('pf9_stat', {})
    pf9_networks = values.pop('pf9_networks', {})

    session = get_session()
    with session.begin():
        _update_ip_info(context, pf9_ip_info, compute_id, session)
...
def compute_node_update_pf9_stats(context, compute_host, values):
    """Updates the ComputeNode stats with most recent data."""
    session = get_session()

    with session.begin():
        compute_node = compute_node_get_all_by_host(context, compute_host)[0]
        compute_id = compute_node.id
        _update_pf9_stats(context, values, compute_id, session)


def _update_ip_info(context, ip_info, compute_id, session):
    """
    Handle ip address change, removal of interface
    """
    existing = model_query(context, models.ComputeNodesIPInfo, session=session,
            read_deleted='no').filter_by(host_id=compute_id).all()

    #TODO add support for adding interfaces
    host_ip_info = dict()
    for if_info in ip_info:
        host_ip_info[if_info['interface_name']] = if_info['ip']

    # Pf9: Find out new rows to be added
    # first copy all the ip_info
    new_ip_info = dict(host_ip_info)

    for row in existing:
        if_name = row.interface_name
        if if_name in host_ip_info.keys():
            if row.ip != host_ip_info[if_name]:
                row.ip = host_ip_info[if_name]
            # PF9: Since there is already an entry for if_name, we don't
            # want to add it, so remove it from the new_ip_info
            del new_ip_info[if_name]
        else:
            row.soft_delete(session=session)

    # PF9: Now add that to the table
    for ip_info in new_ip_info.iteritems():
        node_ip_info = models.ComputeNodesIPInfo()
        node_ip_info['interface_name'] = ip_info[0]
        node_ip_info['ip'] = ip_info[1]
        node_ip_info['host_id'] = compute_id
        node_ip_info.save(session=session)
...
@require_context
def fixed_ip_bulk_create_from_tuples(context, ips):
    session = get_session()
    with session.begin():
        _fixed_ip_bulk_create_from_tuples(session, ips)


def _fixed_ip_create_single(session, ip):
...
def _fixed_ip_bulk_create_from_tuples(session, ips):
    for network_id, address, reserved in ips:
        ip = {'network_id': network_id,
              'address': address,
              'reserved': reserved}
        _fixed_ip_create_single(session, ip)


@require_context
def fixed_ip_reallocate(context, delete_ips, create_ips):
    all_ips = delete_ips + create_ips
    session = get_session()
    with session.begin():
        for network_id, address, reserved in all_ips:
            model = model_query(context, models.FixedIp, session=session,
                                 read_deleted="no").\
            ...
            if model['allocated']:
                ...
                raise exception.FixedIpAlreadyInUse(address=address,
                                                    instance_uuid=instance_uuid)
            model.soft_delete(session)
        _fixed_ip_bulk_create_from_tuples(session, create_ips)

@require_context
def fixed_ip_disassociate(context, address, session=None):
    # PF9 change
...
@require_context
def fixed_ip_get_by_network(context, network_id, session=None):
    """
    PF9 function
    :param context:
    :param network_id:
...
@require_context
def fixed_ips_by_virtual_interface(context, vif_id, session=None):
    # PF9 change
    session = session or get_session()
    result = model_query(context, models.FixedIp, session=session, read_deleted="no").\
...
@require_context
def fixed_ip_update(context, address, values, session=None):
    # PF9 start
    session = session or get_session()
    with session.begin(subtransactions=True):
        _fixed_ip_get_by_address(context, address, session=session).\
...
def virtual_interface_delete(context, vif_id):
    """
    PF9 function: Delete virtual interface record from database
    :param context:
    :param vif_id:
    :return:
    """
    session = get_session()
    with session.begin():
        vif_ref = _virtual_interface_query(context, session).filter_by(id=vif_id).first()
        fixed_ips = fixed_ips_by_virtual_interface(context, vif_id, session=session)
        for fip in fixed_ips:
            fixed_ip_update(context, fip['address'],
                            dict(allocated=False, instance_uuid=None, virtual_interface_id=None),
                            session=session)
        inst_ref = instance_get_by_uuid(context, vif_ref['instance_uuid'])
        instance_disassociate_networks_pf9(context, inst_ref['id'], [vif_ref['network_id']])
        _virtual_interface_query(context, session=session).filter_by(id=vif_id).soft_delete()
...
def instance_create(context, values):
    ...
    with session.begin():
        ...
        try:
            networks = [network_get_by_uuid(context, u, session=session)
                        for u in net_uuids]
            network_ids = [n['id'] for n in networks]
            _instance_associate_networks_pf9(context, instance_ref['id'],
                                             network_ids, session=session)

        except exception.NoNetworksFound as nf:
            LOG.warning(_('Requested networks not found for instance, error:'
                          '{e}'.format(e=nf)))
        # PF9 end
...
@require_context
@oslo_db_api.wrap_db_retry(max_retries=5, retry_on_deadlock=True)
def _instance_destroy(context, instance_uuid, session, constraint=None):
    ...
    model_query(context, models.SecurityGroupInstanceAssociation,
                session=session).filter_by(instance_uuid=instance_uuid).\
        soft_delete()
    ...
    model_query(context, models.InstanceSystemMetadata, session=session).\
                filter_by(instance_uuid=instance_uuid).\
                soft_delete()
    model_query(context, models.InstanceNetworksPf9, session=session).\
        filter_by(instance_id=instance_ref['id']).soft_delete()
...
def instance_destroy(context, instance_uuid, constraint=None):
    session = get_session()
    with session.begin():
        return _instance_destroy(context, instance_uuid, session,
                                 constraint=constraint)


@require_context
def instance_restore(context, instance_uuid):
    session = get_session()

    with session.begin():
        _instance_restore(context, instance_uuid, session=session)


def _instance_restore(context, instance_uuid, session=None):
    """Restore those deleted instances in DB
    """
    if uuidutils.is_uuid_like(instance_uuid):
        # First check if instance is deleted
        instance_ref = model_query(context, models.Instance,
                                   session=session).\
            filter_by(uuid=instance_uuid).first()

        if not instance_ref:
            raise exception.InstanceNotFound(instance_id=instance_uuid)

        if not instance_ref['deleted']:
            raise exception.InstanceExists(name=instance_uuid)

    else:
        raise exception.InvalidUUID(instance_uuid)

    LOG.info('restoring instance %s', instance_uuid)

    instance_ref.undelete(session=session)

    for table in [models.SecurityGroupInstanceAssociation,
                  models.InstanceInfoCache, models.InstanceMetadata]:
        result = model_query(context, table, session=session, read_deleted="yes").\
                                filter_by(instance_uuid=instance_uuid).first()
        if result:
            result.undelete(session=session)

    result = model_query(context, models.InstanceNetworksPf9, session=session,
                         read_deleted="yes").\
    ...
    if result:
        for r in result.all():
            try:
                _network_get(context, r['network_id'], session=session)
                r.undelete(session=session)
            except exception.NetworkNotFound:
                LOG.info(_('Network with ID %s has been deleted, not restoring'
                         ' instance mapping'), r['network_id'])
...
@require_context
def instance_get_all_by_network_pf9(context, network_id,
                                    session=None):
    return model_query(context, models.InstanceNetworksPf9,
                       session=session).\
        filter_by(network_id=network_id).all()

@require_context
def instance_associate_networks_pf9(context, instance_id, network_ids):
    session = get_session()
    with session.begin():
        return _instance_associate_networks_pf9(context, instance_id,
                                                network_ids,
                                                session)


def _instance_associate_networks_pf9(context, instance_id, network_ids,
                                    session):
    mappings = network_get_all_by_instance_pf9(context, instance_id,
                                               session=session)
    existing_network_ids = set([m['network_id'] for m in mappings])

    new_mappings = []
    for n_id in network_ids:
        if n_id not in existing_network_ids:
            model = models.InstanceNetworksPf9()
            model.network_id = n_id
            model.instance_id = instance_id
            model.save(session=session)
            new_mappings.append(model_query(context, models.InstanceNetworksPf9,
                                            session=session).\
                                            filter_by(instance_id=instance_id).\
                                            filter_by(network_id=n_id).first())
    return new_mappings

@require_context
def instance_disassociate_networks_pf9(context, instance_id, network_ids, session=None):
    session = session or get_session()
    n_ids = set([n['id'] for n in
    network_get_all_by_instance_pf9(context, instance_id, session=session)])
    ids_to_purge = set(network_ids)
    for n_id in ids_to_purge.intersection(n_ids):
        model_query(context, models.InstanceNetworksPf9, session=session).\
            filter_by(instance_id=instance_id).\
            filter_by(network_id=n_id).soft_delete()
...
def _validate_instance_network_access_pf9(context, instance_uuid, project_id,
                                          session):
    """
    Make sure new project of an instance has access to networks the instance
    is connected to
    """
    instance_ref = _instance_get_by_uuid(context, instance_uuid, session)
    if instance_ref['project_id'] == project_id:
        return
    network_ids = [n['network_id'] for n in
                   network_get_all_by_instance_pf9(context, instance_ref['id'],
                                               session=session)]
    network_uuids = [_network_get(context, n_id, session=session)['uuid']
                     for n_id in network_ids]
    for uuid in network_uuids:
        project_ids = network_get_access_pf9(context, uuid,
                                             session=session)
        if project_id not in project_ids:
            LOG.error(_("Network {n} is not shared with project {prj}".\
                        format(n=uuid, prj=project_id)))
            raise exception.InstanceNetworksNotAccessiblePf9(
                project_id=project_id, instance_id=instance_ref['id'],
                network_id=uuid)
...
def _instance_update(context, session, instance_uuid, values, expected,
    ...
    if "project_id" in values:
        ...
        _validate_instance_network_access_pf9(context, instance_uuid,
                                              project_id, session)
...
def network_create_safe(context, values):
    ...
    # PF9 change
    network_ref['project_id'] = context.project_id
...
def network_delete_safe(context, network_id):
    """
    PF9 added pf9 network->project and network->host association
    """
    session = get_session()
    result = model_query(context, models.FixedIp, session=session,
                         read_deleted="no").\
    ...
    result = model_query(context, models.InstanceNetworksPf9, session=session,
                         read_deleted="no").filter_by(network_id=network_id).\
        count()
    ...
    LOG.info('Deleting network {net_id}, '
               'purging PF9 info'.format(net_id=network_id))

    model_query(context, models.NetworkProjectsPf9, session=session,
                read_deleted="no").\
        filter_by(network_id=network_id).soft_delete()

    model_query(context, models.ComputeNodeNetworksPf9, session=session,
                read_deleted="no").\
        filter_by(network_id=network_id).soft_delete()

    network_ref.soft_delete(session=session)


def _compute_node_get_by_service_host(context, host, session=None):
    # PF9: Added deleted check
    session = session or get_session()
    query = session.query(models.ComputeNode).join(models.Service).\
        filter(models.Service.host == host).\
        filter(models.ComputeNode.deleted == 0)
    return query.first()


def _compute_node_get_by_service_host_and_node(context, host, session=None, node=None):
    # PF9: Added deleted check
    session = session or get_session()
    query = session.query(models.ComputeNode).join(models.Service).\
        filter(models.Service.host == host).\
        filter(models.ComputeNode.deleted == 0).\
        filter(models.ComputeNode.hypervisor_hostname == node)
    return query.first()

def network_create_with_host_mapping_pf9(context, host, values):
    session = get_session()
    with session.begin():
        # Compute node should exist at this point
        compute_node = _compute_node_get_by_service_host(context,
                                                         host,
                                                         session=session)
        if not compute_node:
            LOG.error('Compute node {node} not found'.format(node=host))
            raise exception.ComputeHostNotFound(host=host)

        network_ref = models.Network()
        network_ref['uuid'] = uuidutils.generate_uuid()
        network_ref['project_id'] = context.project_id or \
                                    CONF.PF9.pf9_project_id
        network_ref.update(values)

        network_ref.save(session=session)

        nw_to_host_mapping = models.ComputeNodeNetworksPf9()
        nw_to_host_mapping['network_id'] = network_ref['id']
        nw_to_host_mapping['host_id'] = compute_node['id']

        nw_to_host_mapping.save(session=session)

        return network_ref


def network_check_and_create_host_mapping_pf9(context, host, bridge_name):
    session = get_session()
    with session.begin():
        # Compute node should exist at this point
        compute_node = _compute_node_get_by_service_host(context,
        ...
        nets = network_get_all_by_bridges(context, [bridge_name],
                                          session=session)

        if not nets:
            LOG.warning('Network with bridge {bridge} does not exist, '
                        'ignoring'.format(bridge=bridge_name))
            return

        # Assume bridge names are unique
        net = nets[0]
        result = model_query(context, models.ComputeNodeNetworksPf9,
                             session=session).\
            filter_by(network_id=net['id']).\
            filter_by(host_id=compute_node['id']).first()

        if not result:
            cn_model = models.ComputeNodeNetworksPf9()
            cn_model.host_id = compute_node['id']
            cn_model.network_id = net['id']
            cn_model.save(session=session)
...
def _network_get(context, network_id, session=None, project_only='allow_none'):
    result = model_query_network_pf9(context, session=session,
                         project_only=project_only).\
                         filter(models.Network.id == network_id).\
                         first()
...
def _get_unique_networks(networks):
    """
    PF9 function
    """
    net_ids = set()
...
def network_get_all(context, project_only):
    result = model_query_network_pf9(context, read_deleted="no",
                                    project_only=project_only).all()

    if not result:
        raise exception.NoNetworksFound()

    return _get_unique_networks(result)

@require_context
def network_get_all_by_bridges(context, bridges, project_only='allow_none',
                               session=None):
    # PF9: modified function.
    # Short cut no bridges

    if not bridges:
        raise exception.NoNetworksFound()

    session = session or get_session()
    query = model_query_network_pf9(context, read_deleted="no",
                                    project_only=project_only,
                                    session=session)

    if bridges and len(bridges) > 0:
        query = query.filter(models.Network.bridge.in_(bridges))
...
def network_get_all_by_uuids(context, network_uuids, project_only):
    result = model_query_network_pf9(context, read_deleted="no",
                                     project_only=project_only).\
        filter(models.Network.uuid.in_(network_uuids)).all()
...
@require_context
def network_get_all_by_instance_pf9(context, instance_id,
                                session=None):
    return model_query(context, models.InstanceNetworksPf9,
                       session=session).\
...
def network_get_by_uuid(context, uuid, session=None):
    """
    PF9: Changed function
    """

    result = _network_get_query(context, session=session).\
        filter(models.Network.uuid == uuid).first()
    if not result:
...
@require_context
def network_cleanup(context, host, reported_bridge_names, node):
    """
    PF9: Cleans up networks from nova DB which have been deleted on hypervisor
    """
    session = get_session()
    with session.begin():
        # Compute node should exist at this point
        compute_node = compute_node_get_all_by_host(context, host)[0]

        if not compute_node:
            raise exception.ComputeHostNotFound(host=host)

        # 1. Get all networks associated with this host
        associated_networks = model_query(context, models.ComputeNodeNetworksPf9,
                                     session=session).\
            filter_by(host_id=compute_node['id'])

        associated_ids = [n['network_id'] for n in associated_networks.all()]

        # 2. Get all networks reported by nova-compute in this call
        try :
            reported_networks = network_get_all_by_bridges(context,
                                                          reported_bridge_names,
                                                          session=session)
        except exception.NoNetworksFound:
            reported_networks = []

        reported_net_ids = [n['id'] for n in reported_networks]

        for net_id in associated_ids:
            # Find a network which is in DB, but not reported by nova-compute.
            # Delete association. If that network is no longer associated with
            # a host, delete it from nova.
            if net_id not in reported_net_ids:
                association = model_query(context, models.ComputeNodeNetworksPf9,
                                          session=session).\
                    filter_by(network_id=net_id).\
                    filter_by(host_id=compute_node['id']).first()

                if association:
                    association.soft_delete(session=session)
                    res = model_query(context, models.ComputeNodeNetworksPf9,
                                      session=session).\
                        filter_by(network_id=net_id).all()


@require_context
def network_get_access_pf9(context, network_id, session=None):

    # Check if network_id is valid, if not throw NetworkNotFound
    net = network_get_by_uuid(context, network_id, session=session)
    query = model_query(context, models.NetworkProjectsPf9, session=session).\
        filter_by(network_id=net['id'])
    result = query.all()

    ret = [net['project_id']] if net['project_id'] is not None else []
    if not result:
        return ret

    return [r['project_id'] for r in result] + ret

@require_context
def network_add_access_pf9(context, network_id, project_id):
    session = get_session()
    with session.begin():
        access_list = network_get_access_pf9(context, network_id,
                                             session=session)

        if project_id in access_list:
            raise exception.NetworkAccessExists(network_id=network_id,
                                                project_id=project_id)
        net = network_get_by_uuid(context, network_id, session)

        nw_to_prj_map = models.NetworkProjectsPf9()
        nw_to_prj_map['network_id'] = net['id']
        nw_to_prj_map['project_id'] = project_id
        nw_to_prj_map['deleted'] = 0
        nw_to_prj_map.save(session=session)

@require_context
def network_remove_access_pf9(context, network_id, project_id):
    def _network_in_use(context, nw_id, prj_id, session):
        inst = models.Instance
        inst_nw = models.InstanceNetworksPf9
        query = session.query(inst).join(inst_nw).\
            filter(inst_nw.deleted == 0)
        query = query.filter(inst_nw.network_id == nw_id)
        query = query.filter(inst.project_id == prj_id)
        return query.count()

    session = get_session()
    with session.begin():
        net = network_get_by_uuid(context, network_id, session)
        access_list = network_get_access_pf9(context, net['uuid'],
                                             session=session)

        if project_id not in access_list:
            raise exception.NetworkAccessNotFound(network_id=network_id,
                                                  project_id=project_id)
        if _network_in_use(context, net['id'], project_id, session):
            raise exception.NetworkInUse(network_id=network_id)

        model_query(context, models.NetworkProjectsPf9, session=session).\
            filter_by(network_id=net['id']).\
            filter_by(project_id=project_id).soft_delete()

@require_context
def network_delete_fixed_ips(context, network_id):
    """
    PF9 function
    :param context:
    :param network_id:
    ...
    session = get_session()
    with session.begin(subtransactions=True):
        ips = fixed_ip_get_by_network(context, network_id, session=session)
        for ip in ips:
            model_query(context, models.FixedIp, session=session).filter_by(id=ip['id']).soft_delete()
...
@require_context
def block_device_mapping_get_all_by_instance_uuids(context, instance_uuids, use_subordinate=False):
    if not instance_uuids:
        return []
