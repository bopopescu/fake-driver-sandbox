...
class ComputeNodesIPInfo(BASE, NovaBase):
    """
    Platform 9 specific compute node information
    """
    __tablename__ = 'compute_nodes_pf9_ip_info'
    __table_args__ = (
        Index('ix_compute_nodes_pf9_ip_info_host_id', 'host_id'),
        Index('compute_nodes_pf9_ip_info_host_id_and_deleted_idx',
              'host_id', 'deleted')
    )

    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey('compute_nodes.id'),
                     nullable=False)
    interface_name = Column(String(128))
    ip = Column(types.IPAddress)

    primary_join = ('and_(ComputeNodesIPInfo.host_id == ComputeNode.id, '
                    'ComputeNodesIPInfo.deleted == 0)')
    ip_info = orm.relationship('ComputeNode', backref='pf9_ip_info',
                               primaryjoin=primary_join)

    def __str__(self):
        return "{%d: %s = %s}" % (self.host_id, self.interface_name,
                                  self.ip)


# TODO: pooja: remove the pf9_ext model as it is not being used anywhere.
class ComputeNodesExt(BASE, NovaBase):
    """
    Platform 9 specific
    """
    __tablename__ = 'compute_nodes_pf9_ext'
    __table_args__ = (
        Index('ix_compute_nodes_pf9_ext_host_id', 'host_id'),
        Index('compute_nodes_pf9_ext_host_id_and_deleted_idx',
              'host_id', 'deleted')
    )
    ...
    primary_join = ('and_(ComputeNodesExt.host_id == ComputeNode.id, '
                    'ComputeNodesExt.deleted == 0)')

    ext = orm.relationship('ComputeNode', backref='pf9_ext',
                           primaryjoin=primary_join)

    def __str__(self):
        return "{%d: %s}" % (self.host_id, self.host_name)


class ComputeNodesPf9Stat(BASE, NovaBase):
    """
    Platform9 specific stats
    """

    __tablename__ = 'compute_nodes_pf9_stats'

    __table_args__ = (
        Index('ix_compute_nodes_pf9_stats_host_id', 'host_id'),
        Index('compute_nodes_pf9_stats_host_id_and_deleted_idx',
              'host_id', 'deleted')
    )

    id = Column(Integer, primary_key=True)
    host_id = Column(Integer, ForeignKey('compute_nodes.id'), nullable=False)

    key = Column(String(128))
    value = Column(String(256))

    primary_join = ('and_(ComputeNodesPf9Stat.host_id == ComputeNode.id, '
                    'ComputeNodesPf9Stat.deleted == 0)')
    pf9_stat = orm.relationship('ComputeNode', backref='pf9_stat',
                            primaryjoin=primary_join)

    def __str__(self):
        return "{%d: key=%s, value=%s}" % (self.id, self.key, self.value)
...
class InstanceExtra(BASE, NovaBase):
    ...
    instance = orm.relationship(Instance,
                            backref=orm.backref('extra',
                                                uselist=False),
                            foreign_keys=instance_uuid,
                            # PF9: Change the instance extra join with
                            # instances to not include deleted records
                            primaryjoin='and_(InstanceExtra.deleted == 0,'
                                        'InstanceExtra.instance_uuid == Instance.uuid)')
...
class Network(BASE, NovaBase):
    ...
    # Platform 9 enhancements
    description = Column(String(255))
    range_start = Column(types.IPAddress())
    range_end = Column(types.IPAddress())
...
class NetworkProjectsPf9(BASE, NovaBase):
    """Represents mapping between networks and projects"""
    __tablename__ = 'network_projects_pf9'

    id = Column(Integer, primary_key=True, nullable=False)
    network_id = Column(Integer, ForeignKey('networks.id'))
    project_id = Column(String(255))

    mapping = orm.relationship('Network',
                           foreign_keys=network_id,
                           primaryjoin='and_(NetworkProjectsPf9.network_id =='
                                       'Network.id, Network.deleted == 0)')


class ComputeNodeNetworksPf9(BASE, NovaBase):
    """Represents mapping between compute node and networks"""
    __tablename__ = 'compute_node_networks_pf9'

    id = Column(Integer, primary_key=True, nullable=False)
    network_id = Column(Integer, ForeignKey('networks.id'))
    nw_mapping = orm.relationship('Network',
                           foreign_keys=network_id,
                           primaryjoin='and_(ComputeNodeNetworksPf9.network_id '
                                       '== Network.id, Network.deleted == 0)')
    host_id = Column(Integer, ForeignKey('compute_nodes.id'))
    host_mapping = orm.relationship('ComputeNode', backref='pf9_networks',
                           primaryjoin='and_(ComputeNodeNetworksPf9.host_id =='
                                       'ComputeNode.id, '
                                       'ComputeNode.deleted == 0)')

class InstanceNetworksPf9(BASE, NovaBase):
    """Represents mapping between instance and networks"""
    __tablename__ = 'instance_networks_pf9'
    id = Column(Integer, primary_key=True, nullable=False)
    instance_id = Column(Integer, ForeignKey('instances.id'))
    inst_mapping = orm.relationship('Instance',
                                foreign_keys=instance_id,
                                primaryjoin='and_(InstanceNetworksPf9.instance_id'
                                            ' == Instance.id, Instance.deleted '
                                            '== 0)')
    network_id = Column(Integer, ForeignKey('networks.id'))
    nw_mapping = orm.relationship('Network',
                               foreign_keys=network_id,
                               primaryjoin='and_(InstanceNetworksPf9.network_id'
                                           ' == Network.id, Network.deleted == '
                                           '0)')
