...
# PF9: user keystoneclient to fetch glance endpoint
...
glance_opts = [
    ...
    cfg.StrOpt('region_name',
                default='RegionOne',
                help='PF9: Region name of this compute host. Used to find the '
                     'glance endpoint in this region.')
    ]
...
# PF9: import keystone_authtoken settings so we can access endpoints
...
def _create_glance_client(context, host, port, use_ssl, path='', version=1):
    ...
    if netutils.is_valid_ipv6(host):
        ...
        host = '[%s]' % host
    endpoint = '%s://%s:%s%s' % (scheme, host, port, path)
...
def get_api_servers():
    ...
    for api_server in configured_servers:
        ...
        if host[0] == '[' and host[-1] == ']':
            host = host[1:-1]
        use_ssl = (o.scheme == 'https')
        port = o.port or (443 if use_ssl else 80)
        api_servers.append((host, port, use_ssl, o.path))
    random.shuffle(api_servers)
    return itertools.cycle(api_servers)


def _get_alt_context_pf9(context):
    alt_context = copy.deepcopy(context)
    alt_context.auth_token = str(alt_context.auth_token)
    return alt_context


def _get_image_service_endpoint(context):
    """
    PF9: Use the keystone service catalog to fetch the glance internal
    endpoint.
    """
    identity_uri = CONF.keystone_authtoken.identity_uri
    if not identity_uri.endswith('/v2.0'):
        identity_uri += '/v2.0'
    ...
    LOG.info('Using endpoint %s for image service', glance_endpoint)
    try:
        parsed = urlparse.urlparse(glance_endpoint)
        use_ssl = (parsed.scheme == 'https')
        port = parsed.port or (443 if use_ssl else 80)
        tuples = [(parsed.hostname, port, use_ssl, parsed.path)]
    except ValueError:
        LOG.exception('Failed to parse endpoint URL: %s', glance_endpoint)
    return itertools.cycle(tuples)
    ...
    def __init__(self, context=None, host=None, port=None, use_ssl=False,
        ...
        if host is not None:
            self.client = self._create_static_client(context,
                                                     host, port,
                                                     use_ssl, path, version)
        else:
            self.client = None
        self.api_servers = None

    def _create_static_client(self, context, host, port, use_ssl, path, version):
        ...
        return _create_glance_client(context,
                                     self.host, self.port,
                                     self.use_ssl, self.path, self.version)

    def _create_onetime_client(self, context, version):
        """Create a client that will be used for one call."""
        # PF9: if no glance api servers are configured, use the endpoints
        # in the context from the auth token. This will be the case
        # on-premise in ostackhost.
        if CONF.glance.api_servers:
            self.api_servers = get_api_servers()
        else:
            self.api_servers = _get_image_service_endpoint(context)

        self.host, self.port, self.use_ssl, self.path = self.api_servers.next()
        return _create_glance_client(context,
                                     self.host, self.port,
                                     self.use_ssl, self.path, version)

    def call(self, context, version, method, *args, **kwargs):
        """Call a glance client method.  If we get a connection error,
        retry the request according to CONF.glance.num_retries.
        """
        alt_context = _get_alt_context_pf9(context)
        ...
        for attempt in range(1, num_attempts + 1):
            client = self.client or self._create_onetime_client(alt_context,
                                                                version)
            try:
